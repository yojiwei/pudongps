...
import com.share.exchange.ConstantList;
import com.share.exchange.DataDeal;
import com.util.CTools;

public class CDataImpl extends CDataControl implements IDataControl,
	 ...
	 * @author yojiwei
	 */
	public long addNewValue(int pPrimaryKeyType) {
		long id; // 当前记录ID
		dataContent = new Hashtable();
		dataType = new Hashtable();
		/*
		...
		id = getMaxId(strTableName); // 给当前ID赋值
		currentId = id; // 设置当前ID
		setValue(strPrimaryFieldName, Long.toString(pPrimaryKeyType), CDataImpl.LONG); // 把最新的ID插入Hashtable
		return id;
	 ...
	 * @author yojiwei
	 */
	public long addNewValue(String tableName, String fieldName, int pPrimaryKeyType) {
		setTableName(tableName);// 设置表
		setPrimaryFieldName(fieldName);// 设置主字段名
		return addNewValue(pPrimaryKeyType);
	}
	...
	public boolean update() {
		...
		if (dataContent != null && dataType != null) // 存储数据的hashtable不能为空
		{
			...
			if (mode == CDataImpl.NULL) {
				return false; // 如果没有设置类型，退出
			}

			if (mode == CDataImpl.ADDNEW) // 如果是新增
			{
				/*
				 * 开始组合sql语句
				 */
				...
				updateSql = updateSql
						+ tempSql.substring(0, tempSql.length() - 1);
				if (primaryKeyType == CDataImpl.PRIMARY_KEY_IS_VARCHAR) {
...
//		String blobData = null;
	...
	public boolean setSequence(String paraName, String sequenceFieldName,
		...
		Enumeration enmn = request.getParameterNames();
		dataCn.beginTrans();

		try {
			while (enmn.hasMoreElements()) {
				sp = (String) enmn.nextElement();
				if (sp.length() > l) {
...
//		return splitPageOpt(sql,intPageSize,1);
	}
...
//			rsPage.absolute((page - 1) * pageSize + 1); // 定位记录
	...
	private void setInputContent(HttpServletRequest request) {
		inputContent = ""; // 初识化为空
		Enumeration enmn = request.getParameterNames(); // 把所有的参数放入enmn
		while (enmn.hasMoreElements()) {
			String paraName = (String) enmn.nextElement();
	...
	public static void main(String[] args) {
		...
		dImpl.splitPageOpt(
				"select cw_subject,cw_applytime from tb_connwork where cp_id='o1' order by cw_applytime desc", 10, 4);
		System.out.println(Calendar.getInstance().get(Calendar.MILLISECOND));
		try {
			System.out.println(dImpl.rsPage.getFetchSize());
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		System.out.println(Calendar.getInstance().get(Calendar.MILLISECOND));
		dImpl.splitPage("select cw_subject from tb_connwork where cp_id='o1'",
				10, 4);
		System.out.println(Calendar.getInstance().get(Calendar.MILLISECOND));
		try {
			System.out.println(dImpl.rsPage.getFetchSize());
