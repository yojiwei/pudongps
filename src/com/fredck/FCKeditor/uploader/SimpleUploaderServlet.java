...
public class SimpleUploaderServlet extends HttpServlet {
	 ...
	 */
	 public void init() throws ServletException {
		...
		if(baseDir==null)
			baseDir="/UserFiles/";
			//baseDir = Messages.getString("imghttppath");
		
		String realBaseDir=getServletContext().getRealPath(baseDir);
		
		//String realBaseDir = Messages.getString("imgpath");
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//String currentDirPath = Messages.getString("imgpath");
		//String currentPath = Messages.getString("imghttppath");
		...
		if(enabled) {		
			...
			try {
				...
				if(session.getAttribute("temppath")!=null)
					tempPath = (String)session.getAttribute("temppath");
				else{
					SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
					
					tempPath = format.format(new Date()) + Integer.toString((int)(Math.random()*100000));
					session.setAttribute("temppath",tempPath);
				}
				currentDirPath = currentDirPath + "\\" + tempPath;
				File dir = new File(currentDirPath);
				if(!dir.exists()) dir.mkdir();
				File pathToSave=new File(currentDirPath,fileName);			
				fileUrl=currentPath + "/" + tempPath +"/"+fileName;
				if(extIsAllowed(typeStr,ext)) {
					int counter=1;
					while(pathToSave.exists()){
						newName=nameWithoutExt+"("+counter+")"+"."+ext;
						fileUrl=currentPath + "/" + tempPath +"/"+newName;
						retVal="201";
		...
		out.println("window.parent.OnUploadCompleted("+retVal+",'"+fileUrl+"','"+newName+"','"+errorMessage+"','" + tempPath + "');");
		out.println("</script>");
		out.flush();
		out.close();
