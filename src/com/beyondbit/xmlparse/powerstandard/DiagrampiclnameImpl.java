...
public class DiagrampiclnameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Diagrampiclname {
    ...
    private static DiagrampiclnameImpl prototype = null;

    public static void setPrototype(DiagrampiclnameImpl prototype) {
        DiagrampiclnameImpl.prototype = prototype;
    }
    public static DiagrampiclnameImpl newInstance() {
        try {
            return (prototype!=null)?(DiagrampiclnameImpl)prototype.clone(): new DiagrampiclnameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public DiagrampiclnameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Diagrampiclname unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Diagrampiclname unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Diagrampiclname unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Diagrampiclname unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Diagrampiclname unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Diagrampiclname unmarshal(Reader reader, boolean validate) throws IOException {
        DiagrampiclnameImpl diagrampiclname = DiagrampiclnameImpl.newInstance();
        diagrampiclname.setValidating(validate);
        diagrampiclname.setCurrentUNode(diagrampiclname);
        diagrampiclname.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(diagrampiclname);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", diagrampiclname);

            // Register content handler
            parser.setContentHandler(diagrampiclname);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("diagrampiclname")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
