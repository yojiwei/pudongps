...
public class AuseridImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Auserid {
    ...
    private static AuseridImpl prototype = null;

    public static void setPrototype(AuseridImpl prototype) {
        AuseridImpl.prototype = prototype;
    }
    public static AuseridImpl newInstance() {
        try {
            return (prototype!=null)?(AuseridImpl)prototype.clone(): new AuseridImpl();
    ...
    public static Auserid unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Auserid unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Auserid unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Auserid unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Auserid unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Auserid unmarshal(Reader reader, boolean validate) throws IOException {
        AuseridImpl auserid = AuseridImpl.newInstance();
        auserid.setValidating(validate);
        auserid.setCurrentUNode(auserid);
        auserid.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(auserid);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", auserid);

            // Register content handler
            parser.setContentHandler(auserid);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("auserid")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
