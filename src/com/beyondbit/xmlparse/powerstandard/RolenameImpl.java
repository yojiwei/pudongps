...
public class RolenameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Rolename {
    ...
    private static RolenameImpl prototype = null;

    public static void setPrototype(RolenameImpl prototype) {
        RolenameImpl.prototype = prototype;
    }
    public static RolenameImpl newInstance() {
        try {
            return (prototype!=null)?(RolenameImpl)prototype.clone(): new RolenameImpl();
    ...
    public static Rolename unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Rolename unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Rolename unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Rolename unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Rolename unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Rolename unmarshal(Reader reader, boolean validate) throws IOException {
        RolenameImpl rolename = RolenameImpl.newInstance();
        rolename.setValidating(validate);
        rolename.setCurrentUNode(rolename);
        rolename.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(rolename);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", rolename);

            // Register content handler
            parser.setContentHandler(rolename);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("rolename")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
