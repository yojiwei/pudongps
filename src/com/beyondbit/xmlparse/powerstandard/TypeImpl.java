...
public class TypeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Type {
    ...
    private static TypeImpl prototype = null;

    public static void setPrototype(TypeImpl prototype) {
        TypeImpl.prototype = prototype;
    }
    public static TypeImpl newInstance() {
        try {
            return (prototype!=null)?(TypeImpl)prototype.clone(): new TypeImpl();
    ...
    public static Type unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Type unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Type unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Type unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Type unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Type unmarshal(Reader reader, boolean validate) throws IOException {
        TypeImpl type = TypeImpl.newInstance();
        type.setValidating(validate);
        type.setCurrentUNode(type);
        type.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(type);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", type);

            // Register content handler
            parser.setContentHandler(type);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("type")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
