...
public class AudiodesnameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Audiodesname {
    ...
    private static AudiodesnameImpl prototype = null;

    public static void setPrototype(AudiodesnameImpl prototype) {
        AudiodesnameImpl.prototype = prototype;
    }
    public static AudiodesnameImpl newInstance() {
        try {
            return (prototype!=null)?(AudiodesnameImpl)prototype.clone(): new AudiodesnameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public AudiodesnameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Audiodesname unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Audiodesname unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Audiodesname unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Audiodesname unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Audiodesname unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Audiodesname unmarshal(Reader reader, boolean validate) throws IOException {
        AudiodesnameImpl audiodesname = AudiodesnameImpl.newInstance();
        audiodesname.setValidating(validate);
        audiodesname.setCurrentUNode(audiodesname);
        audiodesname.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(audiodesname);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", audiodesname);

            // Register content handler
            parser.setContentHandler(audiodesname);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("audiodesname")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
