...
public class DesImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Des {
    ...
    private static DesImpl prototype = null;

    public static void setPrototype(DesImpl prototype) {
        DesImpl.prototype = prototype;
    }
    public static DesImpl newInstance() {
        try {
            return (prototype!=null)?(DesImpl)prototype.clone(): new DesImpl();
    ...
    public static Des unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Des unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Des unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Des unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Des unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Des unmarshal(Reader reader, boolean validate) throws IOException {
        DesImpl des = DesImpl.newInstance();
        des.setValidating(validate);
        des.setCurrentUNode(des);
        des.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(des);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", des);

            // Register content handler
            parser.setContentHandler(des);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("des")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
