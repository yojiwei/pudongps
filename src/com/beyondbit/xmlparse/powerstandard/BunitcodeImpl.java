...
public class BunitcodeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Bunitcode {
    ...
    private static BunitcodeImpl prototype = null;

    public static void setPrototype(BunitcodeImpl prototype) {
        BunitcodeImpl.prototype = prototype;
    }
    public static BunitcodeImpl newInstance() {
        try {
            return (prototype!=null)?(BunitcodeImpl)prototype.clone(): new BunitcodeImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public BunitcodeImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Bunitcode unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Bunitcode unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Bunitcode unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Bunitcode unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Bunitcode unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Bunitcode unmarshal(Reader reader, boolean validate) throws IOException {
        BunitcodeImpl bunitcode = BunitcodeImpl.newInstance();
        bunitcode.setValidating(validate);
        bunitcode.setCurrentUNode(bunitcode);
        bunitcode.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(bunitcode);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", bunitcode);

            // Register content handler
            parser.setContentHandler(bunitcode);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("bunitcode")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
