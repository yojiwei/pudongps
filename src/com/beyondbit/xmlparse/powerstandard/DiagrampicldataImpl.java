...
public class DiagrampicldataImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Diagrampicldata {
    ...
    private static DiagrampicldataImpl prototype = null;

    public static void setPrototype(DiagrampicldataImpl prototype) {
        DiagrampicldataImpl.prototype = prototype;
    }
    public static DiagrampicldataImpl newInstance() {
        try {
            return (prototype!=null)?(DiagrampicldataImpl)prototype.clone(): new DiagrampicldataImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public DiagrampicldataImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Diagrampicldata unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Diagrampicldata unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Diagrampicldata unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Diagrampicldata unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Diagrampicldata unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Diagrampicldata unmarshal(Reader reader, boolean validate) throws IOException {
        DiagrampicldataImpl diagrampicldata = DiagrampicldataImpl.newInstance();
        diagrampicldata.setValidating(validate);
        diagrampicldata.setCurrentUNode(diagrampicldata);
        diagrampicldata.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(diagrampicldata);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", diagrampicldata);

            // Register content handler
            parser.setContentHandler(diagrampicldata);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("diagrampicldata")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
