...
public class CatenameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Catename {
    ...
    private static CatenameImpl prototype = null;

    public static void setPrototype(CatenameImpl prototype) {
        CatenameImpl.prototype = prototype;
    }
    public static CatenameImpl newInstance() {
        try {
            return (prototype!=null)?(CatenameImpl)prototype.clone(): new CatenameImpl();
    ...
    public static Catename unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Catename unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Catename unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Catename unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Catename unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Catename unmarshal(Reader reader, boolean validate) throws IOException {
        CatenameImpl catename = CatenameImpl.newInstance();
        catename.setValidating(validate);
        catename.setCurrentUNode(catename);
        catename.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(catename);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", catename);

            // Register content handler
            parser.setContentHandler(catename);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("catename")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
