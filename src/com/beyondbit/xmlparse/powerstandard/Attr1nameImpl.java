...
public class Attr1nameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Attr1name {
    ...
    private static Attr1nameImpl prototype = null;

    public static void setPrototype(Attr1nameImpl prototype) {
        Attr1nameImpl.prototype = prototype;
    }
    public static Attr1nameImpl newInstance() {
        try {
            return (prototype!=null)?(Attr1nameImpl)prototype.clone(): new Attr1nameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public Attr1nameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Attr1name unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Attr1name unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Attr1name unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Attr1name unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Attr1name unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Attr1name unmarshal(Reader reader, boolean validate) throws IOException {
        Attr1nameImpl attr1name = Attr1nameImpl.newInstance();
        attr1name.setValidating(validate);
        attr1name.setCurrentUNode(attr1name);
        attr1name.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(attr1name);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", attr1name);

            // Register content handler
            parser.setContentHandler(attr1name);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("attr1name")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
