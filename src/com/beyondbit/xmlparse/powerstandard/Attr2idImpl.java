...
public class Attr2idImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Attr2id {
    ...
    private static Attr2idImpl prototype = null;

    public static void setPrototype(Attr2idImpl prototype) {
        Attr2idImpl.prototype = prototype;
    }
    public static Attr2idImpl newInstance() {
        try {
            return (prototype!=null)?(Attr2idImpl)prototype.clone(): new Attr2idImpl();
    ...
    public static Attr2id unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Attr2id unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Attr2id unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Attr2id unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Attr2id unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Attr2id unmarshal(Reader reader, boolean validate) throws IOException {
        Attr2idImpl attr2id = Attr2idImpl.newInstance();
        attr2id.setValidating(validate);
        attr2id.setCurrentUNode(attr2id);
        attr2id.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(attr2id);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", attr2id);

            // Register content handler
            parser.setContentHandler(attr2id);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("attr2id")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
