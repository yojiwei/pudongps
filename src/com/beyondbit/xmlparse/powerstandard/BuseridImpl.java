...
public class BuseridImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Buserid {
    ...
    private static BuseridImpl prototype = null;

    public static void setPrototype(BuseridImpl prototype) {
        BuseridImpl.prototype = prototype;
    }
    public static BuseridImpl newInstance() {
        try {
            return (prototype!=null)?(BuseridImpl)prototype.clone(): new BuseridImpl();
    ...
    public static Buserid unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Buserid unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Buserid unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Buserid unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Buserid unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Buserid unmarshal(Reader reader, boolean validate) throws IOException {
        BuseridImpl buserid = BuseridImpl.newInstance();
        buserid.setValidating(validate);
        buserid.setCurrentUNode(buserid);
        buserid.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(buserid);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", buserid);

            // Register content handler
            parser.setContentHandler(buserid);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("buserid")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
