...
public class EffectdateImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Effectdate {
    ...
    private static EffectdateImpl prototype = null;

    public static void setPrototype(EffectdateImpl prototype) {
        EffectdateImpl.prototype = prototype;
    }
    public static EffectdateImpl newInstance() {
        try {
            return (prototype!=null)?(EffectdateImpl)prototype.clone(): new EffectdateImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public EffectdateImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Effectdate unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Effectdate unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Effectdate unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Effectdate unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Effectdate unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Effectdate unmarshal(Reader reader, boolean validate) throws IOException {
        EffectdateImpl effectdate = EffectdateImpl.newInstance();
        effectdate.setValidating(validate);
        effectdate.setCurrentUNode(effectdate);
        effectdate.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(effectdate);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", effectdate);

            // Register content handler
            parser.setContentHandler(effectdate);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("effectdate")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
