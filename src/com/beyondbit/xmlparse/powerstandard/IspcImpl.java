...
public class IspcImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Ispc {
    ...
    private static IspcImpl prototype = null;

    public static void setPrototype(IspcImpl prototype) {
        IspcImpl.prototype = prototype;
    }
    public static IspcImpl newInstance() {
        try {
            return (prototype!=null)?(IspcImpl)prototype.clone(): new IspcImpl();
    ...
    public static Ispc unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Ispc unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Ispc unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Ispc unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Ispc unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Ispc unmarshal(Reader reader, boolean validate) throws IOException {
        IspcImpl ispc = IspcImpl.newInstance();
        ispc.setValidating(validate);
        ispc.setCurrentUNode(ispc);
        ispc.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(ispc);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", ispc);

            // Register content handler
            parser.setContentHandler(ispc);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("ispc")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
