...
public class DiagrampicsnameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Diagrampicsname {
    ...
    private static DiagrampicsnameImpl prototype = null;

    public static void setPrototype(DiagrampicsnameImpl prototype) {
        DiagrampicsnameImpl.prototype = prototype;
    }
    public static DiagrampicsnameImpl newInstance() {
        try {
            return (prototype!=null)?(DiagrampicsnameImpl)prototype.clone(): new DiagrampicsnameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public DiagrampicsnameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Diagrampicsname unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Diagrampicsname unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Diagrampicsname unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Diagrampicsname unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Diagrampicsname unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Diagrampicsname unmarshal(Reader reader, boolean validate) throws IOException {
        DiagrampicsnameImpl diagrampicsname = DiagrampicsnameImpl.newInstance();
        diagrampicsname.setValidating(validate);
        diagrampicsname.setCurrentUNode(diagrampicsname);
        diagrampicsname.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(diagrampicsname);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", diagrampicsname);

            // Register content handler
            parser.setContentHandler(diagrampicsname);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("diagrampicsname")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
