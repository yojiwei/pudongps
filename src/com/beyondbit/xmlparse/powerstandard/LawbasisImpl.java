...
public class LawbasisImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Lawbasis {
    ...
    private static LawbasisImpl prototype = null;

    public static void setPrototype(LawbasisImpl prototype) {
        LawbasisImpl.prototype = prototype;
    }
    public static LawbasisImpl newInstance() {
        try {
            return (prototype!=null)?(LawbasisImpl)prototype.clone(): new LawbasisImpl();
    ...
    public static Lawbasis unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Lawbasis unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Lawbasis unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Lawbasis unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Lawbasis unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Lawbasis unmarshal(Reader reader, boolean validate) throws IOException {
        LawbasisImpl lawbasis = LawbasisImpl.newInstance();
        lawbasis.setValidating(validate);
        lawbasis.setCurrentUNode(lawbasis);
        lawbasis.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(lawbasis);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", lawbasis);

            // Register content handler
            parser.setContentHandler(lawbasis);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("lawbasis")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
