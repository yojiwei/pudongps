...
public class BjobImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Bjob {
    ...
    private static BjobImpl prototype = null;

    public static void setPrototype(BjobImpl prototype) {
        BjobImpl.prototype = prototype;
    }
    public static BjobImpl newInstance() {
        try {
            return (prototype!=null)?(BjobImpl)prototype.clone(): new BjobImpl();
    ...
    public static Bjob unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Bjob unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Bjob unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Bjob unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Bjob unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Bjob unmarshal(Reader reader, boolean validate) throws IOException {
        BjobImpl bjob = BjobImpl.newInstance();
        bjob.setValidating(validate);
        bjob.setCurrentUNode(bjob);
        bjob.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(bjob);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", bjob);

            // Register content handler
            parser.setContentHandler(bjob);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("bjob")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
