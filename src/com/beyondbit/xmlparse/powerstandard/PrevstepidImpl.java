...
public class PrevstepidImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Prevstepid {
    ...
    private static PrevstepidImpl prototype = null;

    public static void setPrototype(PrevstepidImpl prototype) {
        PrevstepidImpl.prototype = prototype;
    }
    public static PrevstepidImpl newInstance() {
        try {
            return (prototype!=null)?(PrevstepidImpl)prototype.clone(): new PrevstepidImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public PrevstepidImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Prevstepid unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Prevstepid unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Prevstepid unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Prevstepid unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Prevstepid unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Prevstepid unmarshal(Reader reader, boolean validate) throws IOException {
        PrevstepidImpl prevstepid = PrevstepidImpl.newInstance();
        prevstepid.setValidating(validate);
        prevstepid.setCurrentUNode(prevstepid);
        prevstepid.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(prevstepid);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", prevstepid);

            // Register content handler
            parser.setContentHandler(prevstepid);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("prevstepid")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
