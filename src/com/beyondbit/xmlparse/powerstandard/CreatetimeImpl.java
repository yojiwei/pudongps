...
public class CreatetimeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Createtime {
    ...
    private static CreatetimeImpl prototype = null;

    public static void setPrototype(CreatetimeImpl prototype) {
        CreatetimeImpl.prototype = prototype;
    }
    public static CreatetimeImpl newInstance() {
        try {
            return (prototype!=null)?(CreatetimeImpl)prototype.clone(): new CreatetimeImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public CreatetimeImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Createtime unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Createtime unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Createtime unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Createtime unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Createtime unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Createtime unmarshal(Reader reader, boolean validate) throws IOException {
        CreatetimeImpl createtime = CreatetimeImpl.newInstance();
        createtime.setValidating(validate);
        createtime.setCurrentUNode(createtime);
        createtime.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(createtime);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", createtime);

            // Register content handler
            parser.setContentHandler(createtime);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("createtime")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
