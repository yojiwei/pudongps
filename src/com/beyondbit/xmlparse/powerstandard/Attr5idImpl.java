...
public class Attr5idImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Attr5id {
    ...
    private static Attr5idImpl prototype = null;

    public static void setPrototype(Attr5idImpl prototype) {
        Attr5idImpl.prototype = prototype;
    }
    public static Attr5idImpl newInstance() {
        try {
            return (prototype!=null)?(Attr5idImpl)prototype.clone(): new Attr5idImpl();
    ...
    public static Attr5id unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Attr5id unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Attr5id unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Attr5id unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Attr5id unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Attr5id unmarshal(Reader reader, boolean validate) throws IOException {
        Attr5idImpl attr5id = Attr5idImpl.newInstance();
        attr5id.setValidating(validate);
        attr5id.setCurrentUNode(attr5id);
        attr5id.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(attr5id);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", attr5id);

            // Register content handler
            parser.setContentHandler(attr5id);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("attr5id")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
