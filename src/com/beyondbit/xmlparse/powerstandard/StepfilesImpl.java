...
public class StepfilesImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Stepfiles {

    private List psfileList;
    ...
    private static StepfilesImpl prototype = null;

    public static void setPrototype(StepfilesImpl prototype) {
        StepfilesImpl.prototype = prototype;
    }
    public static StepfilesImpl newInstance() {
        try {
            return (prototype!=null)?(StepfilesImpl)prototype.clone(): new StepfilesImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public StepfilesImpl() {
        psfileList = new LinkedList();
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public List getPsfileList() {
        return psfileList;
    }

    public void setPsfileList(List psfileList) {
        this.psfileList = psfileList;
    }

    public void addPsfile(Psfile psfile) {
        psfileList.add(psfile);
    }

    public void removePsfile(Psfile psfile) {
        psfileList.remove(psfile);
    }
    ...
    protected void writeXMLRepresentation(Writer writer,
        ...
        for (Iterator i=psfileList.iterator(); i.hasNext(); ) {
            PsfileImpl psfile = (PsfileImpl)i.next();
            psfile.writeXMLRepresentation(writer,
    ...
    public static Stepfiles unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Stepfiles unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Stepfiles unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Stepfiles unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Stepfiles unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Stepfiles unmarshal(Reader reader, boolean validate) throws IOException {
        StepfilesImpl stepfiles = StepfilesImpl.newInstance();
        stepfiles.setValidating(validate);
        stepfiles.setCurrentUNode(stepfiles);
        stepfiles.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(stepfiles);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", stepfiles);

            // Register content handler
            parser.setContentHandler(stepfiles);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("stepfiles")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
            ...
            // Delegate handling
            if (localName.equalsIgnoreCase("psfile")) {
                PsfileImpl psfile = PsfileImpl.newInstance();
                current = getCurrentUNode();
                psfile.setParentUNode(current);
                psfile.setCurrentUNode(psfile);
                this.setCurrentUNode(psfile);
                psfile.startElement(namespaceURI, localName, qName, atts);
                // Add this value in
                psfileList.add(psfile);
                return;
            }
