...
public class Powerstd_idImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Powerstd_id {
    ...
    private static Powerstd_idImpl prototype = null;

    public static void setPrototype(Powerstd_idImpl prototype) {
        Powerstd_idImpl.prototype = prototype;
    }
    public static Powerstd_idImpl newInstance() {
        try {
            return (prototype!=null)?(Powerstd_idImpl)prototype.clone(): new Powerstd_idImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public Powerstd_idImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Powerstd_id unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Powerstd_id unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Powerstd_id unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Powerstd_id unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Powerstd_id unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Powerstd_id unmarshal(Reader reader, boolean validate) throws IOException {
        Powerstd_idImpl powerstd_id = Powerstd_idImpl.newInstance();
        powerstd_id.setValidating(validate);
        powerstd_id.setCurrentUNode(powerstd_id);
        powerstd_id.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(powerstd_id);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", powerstd_id);

            // Register content handler
            parser.setContentHandler(powerstd_id);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("powerstd_id")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
