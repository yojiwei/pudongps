...
public class Attr5nameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Attr5name {
    ...
    private static Attr5nameImpl prototype = null;

    public static void setPrototype(Attr5nameImpl prototype) {
        Attr5nameImpl.prototype = prototype;
    }
    public static Attr5nameImpl newInstance() {
        try {
            return (prototype!=null)?(Attr5nameImpl)prototype.clone(): new Attr5nameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public Attr5nameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Attr5name unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Attr5name unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Attr5name unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Attr5name unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Attr5name unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Attr5name unmarshal(Reader reader, boolean validate) throws IOException {
        Attr5nameImpl attr5name = Attr5nameImpl.newInstance();
        attr5name.setValidating(validate);
        attr5name.setCurrentUNode(attr5name);
        attr5name.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(attr5name);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", attr5name);

            // Register content handler
            parser.setContentHandler(attr5name);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("attr5name")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
