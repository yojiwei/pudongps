...
public class IsinputImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Isinput {
    ...
    private static IsinputImpl prototype = null;

    public static void setPrototype(IsinputImpl prototype) {
        IsinputImpl.prototype = prototype;
    }
    public static IsinputImpl newInstance() {
        try {
            return (prototype!=null)?(IsinputImpl)prototype.clone(): new IsinputImpl();
    ...
    public static Isinput unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Isinput unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Isinput unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Isinput unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Isinput unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Isinput unmarshal(Reader reader, boolean validate) throws IOException {
        IsinputImpl isinput = IsinputImpl.newInstance();
        isinput.setValidating(validate);
        isinput.setCurrentUNode(isinput);
        isinput.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(isinput);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", isinput);

            // Register content handler
            parser.setContentHandler(isinput);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("isinput")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
