...
public class DiagrampicltypeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Diagrampicltype {
    ...
    private static DiagrampicltypeImpl prototype = null;

    public static void setPrototype(DiagrampicltypeImpl prototype) {
        DiagrampicltypeImpl.prototype = prototype;
    }
    public static DiagrampicltypeImpl newInstance() {
        try {
            return (prototype!=null)?(DiagrampicltypeImpl)prototype.clone(): new DiagrampicltypeImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public DiagrampicltypeImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Diagrampicltype unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Diagrampicltype unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Diagrampicltype unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Diagrampicltype unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Diagrampicltype unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Diagrampicltype unmarshal(Reader reader, boolean validate) throws IOException {
        DiagrampicltypeImpl diagrampicltype = DiagrampicltypeImpl.newInstance();
        diagrampicltype.setValidating(validate);
        diagrampicltype.setCurrentUNode(diagrampicltype);
        diagrampicltype.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(diagrampicltype);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", diagrampicltype);

            // Register content handler
            parser.setContentHandler(diagrampicltype);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("diagrampicltype")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
