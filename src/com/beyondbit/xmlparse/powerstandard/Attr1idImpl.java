...
public class Attr1idImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Attr1id {
    ...
    private static Attr1idImpl prototype = null;

    public static void setPrototype(Attr1idImpl prototype) {
        Attr1idImpl.prototype = prototype;
    }
    public static Attr1idImpl newInstance() {
        try {
            return (prototype!=null)?(Attr1idImpl)prototype.clone(): new Attr1idImpl();
    ...
    public static Attr1id unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Attr1id unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Attr1id unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Attr1id unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Attr1id unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Attr1id unmarshal(Reader reader, boolean validate) throws IOException {
        Attr1idImpl attr1id = Attr1idImpl.newInstance();
        attr1id.setValidating(validate);
        attr1id.setCurrentUNode(attr1id);
        attr1id.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(attr1id);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", attr1id);

            // Register content handler
            parser.setContentHandler(attr1id);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("attr1id")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
