...
public class BasisesImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Basises {
    ...
    private static BasisesImpl prototype = null;

    public static void setPrototype(BasisesImpl prototype) {
        BasisesImpl.prototype = prototype;
    }
    public static BasisesImpl newInstance() {
        try {
            return (prototype!=null)?(BasisesImpl)prototype.clone(): new BasisesImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public BasisesImpl() {
        basisList = new LinkedList();
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public void removeBasis(Basis basis) {
    ...
    protected void writeXMLRepresentation(Writer writer,
        ...
        for (Iterator i=basisList.iterator(); i.hasNext(); ) {
            BasisImpl basis = (BasisImpl)i.next();
            basis.writeXMLRepresentation(writer,
    ...
    public static Basises unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Basises unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Basises unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Basises unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Basises unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Basises unmarshal(Reader reader, boolean validate) throws IOException {
        BasisesImpl basises = BasisesImpl.newInstance();
        basises.setValidating(validate);
        basises.setCurrentUNode(basises);
        basises.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(basises);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", basises);

            // Register content handler
            parser.setContentHandler(basises);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("basises")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
            ...
            // Delegate handling
            if (localName.equalsIgnoreCase("basis")) {
                BasisImpl basis = BasisImpl.newInstance();
                current = getCurrentUNode();
                basis.setParentUNode(current);
                basis.setCurrentUNode(basis);
                this.setCurrentUNode(basis);
                basis.startElement(namespaceURI, localName, qName, atts);
                // Add this value in
                basisList.add(basis);
                return;
            }
