...
public class CheckdateImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Checkdate {
    ...
    private static CheckdateImpl prototype = null;

    public static void setPrototype(CheckdateImpl prototype) {
        CheckdateImpl.prototype = prototype;
    }
    public static CheckdateImpl newInstance() {
        try {
            return (prototype!=null)?(CheckdateImpl)prototype.clone(): new CheckdateImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public CheckdateImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Checkdate unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Checkdate unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Checkdate unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Checkdate unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Checkdate unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Checkdate unmarshal(Reader reader, boolean validate) throws IOException {
        CheckdateImpl checkdate = CheckdateImpl.newInstance();
        checkdate.setValidating(validate);
        checkdate.setCurrentUNode(checkdate);
        checkdate.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(checkdate);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", checkdate);

            // Register content handler
            parser.setContentHandler(checkdate);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("checkdate")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
