...
public class FilecodeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Filecode {
    ...
    private static FilecodeImpl prototype = null;

    public static void setPrototype(FilecodeImpl prototype) {
        FilecodeImpl.prototype = prototype;
    }
    public static FilecodeImpl newInstance() {
        try {
            return (prototype!=null)?(FilecodeImpl)prototype.clone(): new FilecodeImpl();
    ...
    public static Filecode unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Filecode unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Filecode unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Filecode unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Filecode unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Filecode unmarshal(Reader reader, boolean validate) throws IOException {
        FilecodeImpl filecode = FilecodeImpl.newInstance();
        filecode.setValidating(validate);
        filecode.setCurrentUNode(filecode);
        filecode.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(filecode);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", filecode);

            // Register content handler
            parser.setContentHandler(filecode);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("filecode")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
