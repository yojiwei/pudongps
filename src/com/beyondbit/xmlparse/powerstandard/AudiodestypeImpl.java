...
public class AudiodestypeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Audiodestype {
    ...
    private static AudiodestypeImpl prototype = null;

    public static void setPrototype(AudiodestypeImpl prototype) {
        AudiodestypeImpl.prototype = prototype;
    }
    public static AudiodestypeImpl newInstance() {
        try {
            return (prototype!=null)?(AudiodestypeImpl)prototype.clone(): new AudiodestypeImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public AudiodestypeImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Audiodestype unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Audiodestype unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Audiodestype unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Audiodestype unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Audiodestype unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Audiodestype unmarshal(Reader reader, boolean validate) throws IOException {
        AudiodestypeImpl audiodestype = AudiodestypeImpl.newInstance();
        audiodestype.setValidating(validate);
        audiodestype.setCurrentUNode(audiodestype);
        audiodestype.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(audiodestype);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", audiodestype);

            // Register content handler
            parser.setContentHandler(audiodestype);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("audiodestype")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
