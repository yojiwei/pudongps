...
public class AjobImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Ajob {
    ...
    private static AjobImpl prototype = null;

    public static void setPrototype(AjobImpl prototype) {
        AjobImpl.prototype = prototype;
    }
    public static AjobImpl newInstance() {
        try {
            return (prototype!=null)?(AjobImpl)prototype.clone(): new AjobImpl();
    ...
    public static Ajob unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Ajob unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Ajob unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Ajob unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Ajob unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Ajob unmarshal(Reader reader, boolean validate) throws IOException {
        AjobImpl ajob = AjobImpl.newInstance();
        ajob.setValidating(validate);
        ajob.setCurrentUNode(ajob);
        ajob.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(ajob);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", ajob);

            // Register content handler
            parser.setContentHandler(ajob);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("ajob")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
