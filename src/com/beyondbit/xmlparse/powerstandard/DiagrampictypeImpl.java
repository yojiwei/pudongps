...
public class DiagrampictypeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Diagrampictype {
    ...
    private static DiagrampictypeImpl prototype = null;

    public static void setPrototype(DiagrampictypeImpl prototype) {
        DiagrampictypeImpl.prototype = prototype;
    }
    public static DiagrampictypeImpl newInstance() {
        try {
            return (prototype!=null)?(DiagrampictypeImpl)prototype.clone(): new DiagrampictypeImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public DiagrampictypeImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Diagrampictype unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Diagrampictype unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Diagrampictype unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Diagrampictype unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Diagrampictype unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Diagrampictype unmarshal(Reader reader, boolean validate) throws IOException {
        DiagrampictypeImpl diagrampictype = DiagrampictypeImpl.newInstance();
        diagrampictype.setValidating(validate);
        diagrampictype.setCurrentUNode(diagrampictype);
        diagrampictype.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(diagrampictype);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", diagrampictype);

            // Register content handler
            parser.setContentHandler(diagrampictype);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("diagrampictype")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
