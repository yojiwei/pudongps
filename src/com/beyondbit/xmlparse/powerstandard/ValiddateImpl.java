...
public class ValiddateImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Validdate {
    ...
    private static ValiddateImpl prototype = null;

    public static void setPrototype(ValiddateImpl prototype) {
        ValiddateImpl.prototype = prototype;
    }
    public static ValiddateImpl newInstance() {
        try {
            return (prototype!=null)?(ValiddateImpl)prototype.clone(): new ValiddateImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public ValiddateImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Validdate unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Validdate unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Validdate unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Validdate unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Validdate unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Validdate unmarshal(Reader reader, boolean validate) throws IOException {
        ValiddateImpl validdate = ValiddateImpl.newInstance();
        validdate.setValidating(validate);
        validdate.setCurrentUNode(validdate);
        validdate.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(validdate);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", validdate);

            // Register content handler
            parser.setContentHandler(validdate);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("validdate")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
