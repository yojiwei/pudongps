...
public class BusernameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Busername {
    ...
    private static BusernameImpl prototype = null;

    public static void setPrototype(BusernameImpl prototype) {
        BusernameImpl.prototype = prototype;
    }
    public static BusernameImpl newInstance() {
        try {
            return (prototype!=null)?(BusernameImpl)prototype.clone(): new BusernameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public BusernameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Busername unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Busername unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Busername unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Busername unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Busername unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Busername unmarshal(Reader reader, boolean validate) throws IOException {
        BusernameImpl busername = BusernameImpl.newInstance();
        busername.setValidating(validate);
        busername.setCurrentUNode(busername);
        busername.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(busername);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", busername);

            // Register content handler
            parser.setContentHandler(busername);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("busername")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
