...
public class CreateuseridImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Createuserid {
    ...
    private static CreateuseridImpl prototype = null;

    public static void setPrototype(CreateuseridImpl prototype) {
        CreateuseridImpl.prototype = prototype;
    }
    public static CreateuseridImpl newInstance() {
        try {
            return (prototype!=null)?(CreateuseridImpl)prototype.clone(): new CreateuseridImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public CreateuseridImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Createuserid unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Createuserid unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Createuserid unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Createuserid unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Createuserid unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Createuserid unmarshal(Reader reader, boolean validate) throws IOException {
        CreateuseridImpl createuserid = CreateuseridImpl.newInstance();
        createuserid.setValidating(validate);
        createuserid.setCurrentUNode(createuserid);
        createuserid.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(createuserid);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", createuserid);

            // Register content handler
            parser.setContentHandler(createuserid);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("createuserid")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
