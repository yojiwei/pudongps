...
public class UnitnameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Unitname {
    ...
    private static UnitnameImpl prototype = null;

    public static void setPrototype(UnitnameImpl prototype) {
        UnitnameImpl.prototype = prototype;
    }
    public static UnitnameImpl newInstance() {
        try {
            return (prototype!=null)?(UnitnameImpl)prototype.clone(): new UnitnameImpl();
    ...
    public static Unitname unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Unitname unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Unitname unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Unitname unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Unitname unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Unitname unmarshal(Reader reader, boolean validate) throws IOException {
        UnitnameImpl unitname = UnitnameImpl.newInstance();
        unitname.setValidating(validate);
        unitname.setCurrentUNode(unitname);
        unitname.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(unitname);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", unitname);

            // Register content handler
            parser.setContentHandler(unitname);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("unitname")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
