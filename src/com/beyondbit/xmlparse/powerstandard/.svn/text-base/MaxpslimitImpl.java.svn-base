...
public class MaxpslimitImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Maxpslimit {
    ...
    private static MaxpslimitImpl prototype = null;

    public static void setPrototype(MaxpslimitImpl prototype) {
        MaxpslimitImpl.prototype = prototype;
    }
    public static MaxpslimitImpl newInstance() {
        try {
            return (prototype!=null)?(MaxpslimitImpl)prototype.clone(): new MaxpslimitImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public MaxpslimitImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Maxpslimit unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Maxpslimit unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Maxpslimit unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Maxpslimit unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Maxpslimit unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Maxpslimit unmarshal(Reader reader, boolean validate) throws IOException {
        MaxpslimitImpl maxpslimit = MaxpslimitImpl.newInstance();
        maxpslimit.setValidating(validate);
        maxpslimit.setCurrentUNode(maxpslimit);
        maxpslimit.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(maxpslimit);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", maxpslimit);

            // Register content handler
            parser.setContentHandler(maxpslimit);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("maxpslimit")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
