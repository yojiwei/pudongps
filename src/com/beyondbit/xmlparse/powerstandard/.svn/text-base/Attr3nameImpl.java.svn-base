...
public class Attr3nameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Attr3name {
    ...
    private static Attr3nameImpl prototype = null;

    public static void setPrototype(Attr3nameImpl prototype) {
        Attr3nameImpl.prototype = prototype;
    }
    public static Attr3nameImpl newInstance() {
        try {
            return (prototype!=null)?(Attr3nameImpl)prototype.clone(): new Attr3nameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public Attr3nameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Attr3name unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Attr3name unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Attr3name unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Attr3name unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Attr3name unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Attr3name unmarshal(Reader reader, boolean validate) throws IOException {
        Attr3nameImpl attr3name = Attr3nameImpl.newInstance();
        attr3name.setValidating(validate);
        attr3name.setCurrentUNode(attr3name);
        attr3name.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(attr3name);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", attr3name);

            // Register content handler
            parser.setContentHandler(attr3name);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("attr3name")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
