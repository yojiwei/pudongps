...
public class PslimitImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Pslimit {
    ...
    private static PslimitImpl prototype = null;

    public static void setPrototype(PslimitImpl prototype) {
        PslimitImpl.prototype = prototype;
    }
    public static PslimitImpl newInstance() {
        try {
            return (prototype!=null)?(PslimitImpl)prototype.clone(): new PslimitImpl();
    ...
    public static Pslimit unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Pslimit unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Pslimit unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Pslimit unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Pslimit unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Pslimit unmarshal(Reader reader, boolean validate) throws IOException {
        PslimitImpl pslimit = PslimitImpl.newInstance();
        pslimit.setValidating(validate);
        pslimit.setCurrentUNode(pslimit);
        pslimit.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(pslimit);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", pslimit);

            // Register content handler
            parser.setContentHandler(pslimit);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("pslimit")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
