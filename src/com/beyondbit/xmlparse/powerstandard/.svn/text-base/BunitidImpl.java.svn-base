...
public class BunitidImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Bunitid {
    ...
    private static BunitidImpl prototype = null;

    public static void setPrototype(BunitidImpl prototype) {
        BunitidImpl.prototype = prototype;
    }
    public static BunitidImpl newInstance() {
        try {
            return (prototype!=null)?(BunitidImpl)prototype.clone(): new BunitidImpl();
    ...
    public static Bunitid unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Bunitid unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Bunitid unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Bunitid unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Bunitid unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Bunitid unmarshal(Reader reader, boolean validate) throws IOException {
        BunitidImpl bunitid = BunitidImpl.newInstance();
        bunitid.setValidating(validate);
        bunitid.setCurrentUNode(bunitid);
        bunitid.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(bunitid);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", bunitid);

            // Register content handler
            parser.setContentHandler(bunitid);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("bunitid")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
