...
public class CheckuseridImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Checkuserid {
    ...
    private static CheckuseridImpl prototype = null;

    public static void setPrototype(CheckuseridImpl prototype) {
        CheckuseridImpl.prototype = prototype;
    }
    public static CheckuseridImpl newInstance() {
        try {
            return (prototype!=null)?(CheckuseridImpl)prototype.clone(): new CheckuseridImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public CheckuseridImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Checkuserid unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Checkuserid unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Checkuserid unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Checkuserid unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Checkuserid unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Checkuserid unmarshal(Reader reader, boolean validate) throws IOException {
        CheckuseridImpl checkuserid = CheckuseridImpl.newInstance();
        checkuserid.setValidating(validate);
        checkuserid.setCurrentUNode(checkuserid);
        checkuserid.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(checkuserid);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", checkuserid);

            // Register content handler
            parser.setContentHandler(checkuserid);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("checkuserid")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
