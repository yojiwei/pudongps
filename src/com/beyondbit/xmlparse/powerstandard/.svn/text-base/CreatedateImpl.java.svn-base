...
public class CreatedateImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Createdate {
    ...
    private static CreatedateImpl prototype = null;

    public static void setPrototype(CreatedateImpl prototype) {
        CreatedateImpl.prototype = prototype;
    }
    public static CreatedateImpl newInstance() {
        try {
            return (prototype!=null)?(CreatedateImpl)prototype.clone(): new CreatedateImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public CreatedateImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Createdate unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Createdate unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Createdate unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Createdate unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Createdate unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Createdate unmarshal(Reader reader, boolean validate) throws IOException {
        CreatedateImpl createdate = CreatedateImpl.newInstance();
        createdate.setValidating(validate);
        createdate.setCurrentUNode(createdate);
        createdate.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(createdate);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", createdate);

            // Register content handler
            parser.setContentHandler(createdate);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("createdate")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
