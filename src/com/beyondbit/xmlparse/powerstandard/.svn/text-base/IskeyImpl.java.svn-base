...
public class IskeyImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Iskey {
    ...
    private static IskeyImpl prototype = null;

    public static void setPrototype(IskeyImpl prototype) {
        IskeyImpl.prototype = prototype;
    }
    public static IskeyImpl newInstance() {
        try {
            return (prototype!=null)?(IskeyImpl)prototype.clone(): new IskeyImpl();
    ...
    public static Iskey unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Iskey unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Iskey unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Iskey unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Iskey unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Iskey unmarshal(Reader reader, boolean validate) throws IOException {
        IskeyImpl iskey = IskeyImpl.newInstance();
        iskey.setValidating(validate);
        iskey.setCurrentUNode(iskey);
        iskey.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(iskey);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", iskey);

            // Register content handler
            parser.setContentHandler(iskey);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("iskey")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
