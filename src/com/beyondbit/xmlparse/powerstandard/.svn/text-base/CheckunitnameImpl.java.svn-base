...
public class CheckunitnameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Checkunitname {
    ...
    private static CheckunitnameImpl prototype = null;

    public static void setPrototype(CheckunitnameImpl prototype) {
        CheckunitnameImpl.prototype = prototype;
    }
    public static CheckunitnameImpl newInstance() {
        try {
            return (prototype!=null)?(CheckunitnameImpl)prototype.clone(): new CheckunitnameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public CheckunitnameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Checkunitname unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Checkunitname unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Checkunitname unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Checkunitname unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Checkunitname unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Checkunitname unmarshal(Reader reader, boolean validate) throws IOException {
        CheckunitnameImpl checkunitname = CheckunitnameImpl.newInstance();
        checkunitname.setValidating(validate);
        checkunitname.setCurrentUNode(checkunitname);
        checkunitname.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(checkunitname);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", checkunitname);

            // Register content handler
            parser.setContentHandler(checkunitname);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("checkunitname")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
