...
public class CreateunitidImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Createunitid {
    ...
    private static CreateunitidImpl prototype = null;

    public static void setPrototype(CreateunitidImpl prototype) {
        CreateunitidImpl.prototype = prototype;
    }
    public static CreateunitidImpl newInstance() {
        try {
            return (prototype!=null)?(CreateunitidImpl)prototype.clone(): new CreateunitidImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public CreateunitidImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Createunitid unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Createunitid unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Createunitid unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Createunitid unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Createunitid unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Createunitid unmarshal(Reader reader, boolean validate) throws IOException {
        CreateunitidImpl createunitid = CreateunitidImpl.newInstance();
        createunitid.setValidating(validate);
        createunitid.setCurrentUNode(createunitid);
        createunitid.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(createunitid);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", createunitid);

            // Register content handler
            parser.setContentHandler(createunitid);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("createunitid")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
