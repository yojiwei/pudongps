...
public class NameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Name {
    ...
    private static NameImpl prototype = null;

    public static void setPrototype(NameImpl prototype) {
        NameImpl.prototype = prototype;
    }
    public static NameImpl newInstance() {
        try {
            return (prototype!=null)?(NameImpl)prototype.clone(): new NameImpl();
    ...
    public static Name unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Name unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Name unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Name unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Name unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Name unmarshal(Reader reader, boolean validate) throws IOException {
        NameImpl name = NameImpl.newInstance();
        name.setValidating(validate);
        name.setCurrentUNode(name);
        name.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(name);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", name);

            // Register content handler
            parser.setContentHandler(name);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("name")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
