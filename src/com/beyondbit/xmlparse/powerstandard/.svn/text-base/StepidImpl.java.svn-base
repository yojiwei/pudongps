...
public class StepidImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Stepid {
    ...
    private static StepidImpl prototype = null;

    public static void setPrototype(StepidImpl prototype) {
        StepidImpl.prototype = prototype;
    }
    public static StepidImpl newInstance() {
        try {
            return (prototype!=null)?(StepidImpl)prototype.clone(): new StepidImpl();
    ...
    public static Stepid unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Stepid unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Stepid unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Stepid unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Stepid unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Stepid unmarshal(Reader reader, boolean validate) throws IOException {
        StepidImpl stepid = StepidImpl.newInstance();
        stepid.setValidating(validate);
        stepid.setCurrentUNode(stepid);
        stepid.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(stepid);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", stepid);

            // Register content handler
            parser.setContentHandler(stepid);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("stepid")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
