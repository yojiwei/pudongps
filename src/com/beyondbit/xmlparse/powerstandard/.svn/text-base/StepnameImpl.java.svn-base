...
public class StepnameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Stepname {
    ...
    private static StepnameImpl prototype = null;

    public static void setPrototype(StepnameImpl prototype) {
        StepnameImpl.prototype = prototype;
    }
    public static StepnameImpl newInstance() {
        try {
            return (prototype!=null)?(StepnameImpl)prototype.clone(): new StepnameImpl();
    ...
    public static Stepname unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Stepname unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Stepname unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Stepname unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Stepname unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Stepname unmarshal(Reader reader, boolean validate) throws IOException {
        StepnameImpl stepname = StepnameImpl.newInstance();
        stepname.setValidating(validate);
        stepname.setCurrentUNode(stepname);
        stepname.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(stepname);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", stepname);

            // Register content handler
            parser.setContentHandler(stepname);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("stepname")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
