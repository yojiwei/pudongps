...
public class CreateunitcodeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Createunitcode {
    ...
    private static CreateunitcodeImpl prototype = null;

    public static void setPrototype(CreateunitcodeImpl prototype) {
        CreateunitcodeImpl.prototype = prototype;
    }
    public static CreateunitcodeImpl newInstance() {
        try {
            return (prototype!=null)?(CreateunitcodeImpl)prototype.clone(): new CreateunitcodeImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public CreateunitcodeImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Createunitcode unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Createunitcode unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Createunitcode unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Createunitcode unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Createunitcode unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Createunitcode unmarshal(Reader reader, boolean validate) throws IOException {
        CreateunitcodeImpl createunitcode = CreateunitcodeImpl.newInstance();
        createunitcode.setValidating(validate);
        createunitcode.setCurrentUNode(createunitcode);
        createunitcode.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(createunitcode);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", createunitcode);

            // Register content handler
            parser.setContentHandler(createunitcode);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("createunitcode")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
