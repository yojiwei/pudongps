...
public class CheckusernameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Checkusername {
    ...
    private static CheckusernameImpl prototype = null;

    public static void setPrototype(CheckusernameImpl prototype) {
        CheckusernameImpl.prototype = prototype;
    }
    public static CheckusernameImpl newInstance() {
        try {
            return (prototype!=null)?(CheckusernameImpl)prototype.clone(): new CheckusernameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public CheckusernameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Checkusername unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Checkusername unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Checkusername unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Checkusername unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Checkusername unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Checkusername unmarshal(Reader reader, boolean validate) throws IOException {
        CheckusernameImpl checkusername = CheckusernameImpl.newInstance();
        checkusername.setValidating(validate);
        checkusername.setCurrentUNode(checkusername);
        checkusername.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(checkusername);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", checkusername);

            // Register content handler
            parser.setContentHandler(checkusername);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("checkusername")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
