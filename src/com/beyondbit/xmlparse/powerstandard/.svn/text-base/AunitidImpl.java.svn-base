...
public class AunitidImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Aunitid {
    ...
    private static AunitidImpl prototype = null;

    public static void setPrototype(AunitidImpl prototype) {
        AunitidImpl.prototype = prototype;
    }
    public static AunitidImpl newInstance() {
        try {
            return (prototype!=null)?(AunitidImpl)prototype.clone(): new AunitidImpl();
    ...
    public static Aunitid unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Aunitid unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Aunitid unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Aunitid unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Aunitid unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Aunitid unmarshal(Reader reader, boolean validate) throws IOException {
        AunitidImpl aunitid = AunitidImpl.newInstance();
        aunitid.setValidating(validate);
        aunitid.setCurrentUNode(aunitid);
        aunitid.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(aunitid);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", aunitid);

            // Register content handler
            parser.setContentHandler(aunitid);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("aunitid")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
