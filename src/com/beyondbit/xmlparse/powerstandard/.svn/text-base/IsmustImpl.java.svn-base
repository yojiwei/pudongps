...
public class IsmustImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Ismust {
    ...
    private static IsmustImpl prototype = null;

    public static void setPrototype(IsmustImpl prototype) {
        IsmustImpl.prototype = prototype;
    }
    public static IsmustImpl newInstance() {
        try {
            return (prototype!=null)?(IsmustImpl)prototype.clone(): new IsmustImpl();
    ...
    public static Ismust unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Ismust unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Ismust unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Ismust unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Ismust unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Ismust unmarshal(Reader reader, boolean validate) throws IOException {
        IsmustImpl ismust = IsmustImpl.newInstance();
        ismust.setValidating(validate);
        ismust.setCurrentUNode(ismust);
        ismust.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(ismust);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", ismust);

            // Register content handler
            parser.setContentHandler(ismust);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("ismust")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
