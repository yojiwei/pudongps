...
public class NextstepImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Nextstep {
    ...
    private static NextstepImpl prototype = null;

    public static void setPrototype(NextstepImpl prototype) {
        NextstepImpl.prototype = prototype;
    }
    public static NextstepImpl newInstance() {
        try {
            return (prototype!=null)?(NextstepImpl)prototype.clone(): new NextstepImpl();
    ...
    public static Nextstep unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Nextstep unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Nextstep unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Nextstep unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Nextstep unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Nextstep unmarshal(Reader reader, boolean validate) throws IOException {
        NextstepImpl nextstep = NextstepImpl.newInstance();
        nextstep.setValidating(validate);
        nextstep.setCurrentUNode(nextstep);
        nextstep.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(nextstep);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", nextstep);

            // Register content handler
            parser.setContentHandler(nextstep);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("nextstep")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
