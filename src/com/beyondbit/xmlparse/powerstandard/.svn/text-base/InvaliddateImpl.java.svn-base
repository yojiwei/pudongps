...
public class InvaliddateImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Invaliddate {
    ...
    private static InvaliddateImpl prototype = null;

    public static void setPrototype(InvaliddateImpl prototype) {
        InvaliddateImpl.prototype = prototype;
    }
    public static InvaliddateImpl newInstance() {
        try {
            return (prototype!=null)?(InvaliddateImpl)prototype.clone(): new InvaliddateImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public InvaliddateImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Invaliddate unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Invaliddate unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Invaliddate unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Invaliddate unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Invaliddate unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Invaliddate unmarshal(Reader reader, boolean validate) throws IOException {
        InvaliddateImpl invaliddate = InvaliddateImpl.newInstance();
        invaliddate.setValidating(validate);
        invaliddate.setCurrentUNode(invaliddate);
        invaliddate.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(invaliddate);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", invaliddate);

            // Register content handler
            parser.setContentHandler(invaliddate);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("invaliddate")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
