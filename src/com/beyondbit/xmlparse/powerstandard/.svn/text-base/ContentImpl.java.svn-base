...
public class ContentImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Content {
    ...
    private static ContentImpl prototype = null;

    public static void setPrototype(ContentImpl prototype) {
        ContentImpl.prototype = prototype;
    }
    public static ContentImpl newInstance() {
        try {
            return (prototype!=null)?(ContentImpl)prototype.clone(): new ContentImpl();
    ...
    public static Content unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Content unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Content unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Content unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Content unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Content unmarshal(Reader reader, boolean validate) throws IOException {
        ContentImpl content = ContentImpl.newInstance();
        content.setValidating(validate);
        content.setCurrentUNode(content);
        content.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(content);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", content);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("content")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
