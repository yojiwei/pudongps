...
public class NextstepidImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Nextstepid {
    ...
    private static NextstepidImpl prototype = null;

    public static void setPrototype(NextstepidImpl prototype) {
        NextstepidImpl.prototype = prototype;
    }
    public static NextstepidImpl newInstance() {
        try {
            return (prototype!=null)?(NextstepidImpl)prototype.clone(): new NextstepidImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public NextstepidImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Nextstepid unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Nextstepid unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Nextstepid unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Nextstepid unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Nextstepid unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Nextstepid unmarshal(Reader reader, boolean validate) throws IOException {
        NextstepidImpl nextstepid = NextstepidImpl.newInstance();
        nextstepid.setValidating(validate);
        nextstepid.setCurrentUNode(nextstepid);
        nextstepid.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(nextstepid);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", nextstepid);

            // Register content handler
            parser.setContentHandler(nextstepid);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("nextstepid")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
