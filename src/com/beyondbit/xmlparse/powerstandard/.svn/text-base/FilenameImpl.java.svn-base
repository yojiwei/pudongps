...
public class FilenameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Filename {
    ...
    private static FilenameImpl prototype = null;

    public static void setPrototype(FilenameImpl prototype) {
        FilenameImpl.prototype = prototype;
    }
    public static FilenameImpl newInstance() {
        try {
            return (prototype!=null)?(FilenameImpl)prototype.clone(): new FilenameImpl();
    ...
    public static Filename unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Filename unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Filename unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Filename unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Filename unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Filename unmarshal(Reader reader, boolean validate) throws IOException {
        FilenameImpl filename = FilenameImpl.newInstance();
        filename.setValidating(validate);
        filename.setCurrentUNode(filename);
        filename.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(filename);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", filename);

            // Register content handler
            parser.setContentHandler(filename);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("filename")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
