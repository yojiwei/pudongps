...
public class PsnameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Psname {
    ...
    private static PsnameImpl prototype = null;

    public static void setPrototype(PsnameImpl prototype) {
        PsnameImpl.prototype = prototype;
    }
    public static PsnameImpl newInstance() {
        try {
            return (prototype!=null)?(PsnameImpl)prototype.clone(): new PsnameImpl();
    ...
    public static Psname unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Psname unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Psname unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Psname unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Psname unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Psname unmarshal(Reader reader, boolean validate) throws IOException {
        PsnameImpl psname = PsnameImpl.newInstance();
        psname.setValidating(validate);
        psname.setCurrentUNode(psname);
        psname.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(psname);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", psname);

            // Register content handler
            parser.setContentHandler(psname);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("psname")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
