...
public class CreateusernameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Createusername {
    ...
    private static CreateusernameImpl prototype = null;

    public static void setPrototype(CreateusernameImpl prototype) {
        CreateusernameImpl.prototype = prototype;
    }
    public static CreateusernameImpl newInstance() {
        try {
            return (prototype!=null)?(CreateusernameImpl)prototype.clone(): new CreateusernameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public CreateusernameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Createusername unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Createusername unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Createusername unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Createusername unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Createusername unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Createusername unmarshal(Reader reader, boolean validate) throws IOException {
        CreateusernameImpl createusername = CreateusernameImpl.newInstance();
        createusername.setValidating(validate);
        createusername.setCurrentUNode(createusername);
        createusername.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(createusername);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", createusername);

            // Register content handler
            parser.setContentHandler(createusername);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("createusername")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
