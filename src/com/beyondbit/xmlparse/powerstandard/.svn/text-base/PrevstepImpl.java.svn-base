...
public class PrevstepImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Prevstep {
    ...
    private static PrevstepImpl prototype = null;

    public static void setPrototype(PrevstepImpl prototype) {
        PrevstepImpl.prototype = prototype;
    }
    public static PrevstepImpl newInstance() {
        try {
            return (prototype!=null)?(PrevstepImpl)prototype.clone(): new PrevstepImpl();
    ...
    public static Prevstep unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Prevstep unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Prevstep unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Prevstep unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Prevstep unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Prevstep unmarshal(Reader reader, boolean validate) throws IOException {
        PrevstepImpl prevstep = PrevstepImpl.newInstance();
        prevstep.setValidating(validate);
        prevstep.setCurrentUNode(prevstep);
        prevstep.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(prevstep);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", prevstep);

            // Register content handler
            parser.setContentHandler(prevstep);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("prevstep")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
