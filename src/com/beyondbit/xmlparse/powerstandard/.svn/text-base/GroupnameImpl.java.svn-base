...
public class GroupnameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Groupname {
    ...
    private static GroupnameImpl prototype = null;

    public static void setPrototype(GroupnameImpl prototype) {
        GroupnameImpl.prototype = prototype;
    }
    public static GroupnameImpl newInstance() {
        try {
            return (prototype!=null)?(GroupnameImpl)prototype.clone(): new GroupnameImpl();
    ...
    public static Groupname unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Groupname unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Groupname unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Groupname unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Groupname unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Groupname unmarshal(Reader reader, boolean validate) throws IOException {
        GroupnameImpl groupname = GroupnameImpl.newInstance();
        groupname.setValidating(validate);
        groupname.setCurrentUNode(groupname);
        groupname.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(groupname);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", groupname);

            // Register content handler
            parser.setContentHandler(groupname);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("groupname")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
