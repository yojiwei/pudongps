...
public class CheckunitidImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Checkunitid {
    ...
    private static CheckunitidImpl prototype = null;

    public static void setPrototype(CheckunitidImpl prototype) {
        CheckunitidImpl.prototype = prototype;
    }
    public static CheckunitidImpl newInstance() {
        try {
            return (prototype!=null)?(CheckunitidImpl)prototype.clone(): new CheckunitidImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public CheckunitidImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Checkunitid unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Checkunitid unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Checkunitid unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Checkunitid unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Checkunitid unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Checkunitid unmarshal(Reader reader, boolean validate) throws IOException {
        CheckunitidImpl checkunitid = CheckunitidImpl.newInstance();
        checkunitid.setValidating(validate);
        checkunitid.setCurrentUNode(checkunitid);
        checkunitid.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(checkunitid);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", checkunitid);

            // Register content handler
            parser.setContentHandler(checkunitid);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("checkunitid")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
