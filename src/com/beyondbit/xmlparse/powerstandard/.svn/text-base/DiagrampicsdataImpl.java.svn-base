...
public class DiagrampicsdataImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Diagrampicsdata {
    ...
    private static DiagrampicsdataImpl prototype = null;

    public static void setPrototype(DiagrampicsdataImpl prototype) {
        DiagrampicsdataImpl.prototype = prototype;
    }
    public static DiagrampicsdataImpl newInstance() {
        try {
            return (prototype!=null)?(DiagrampicsdataImpl)prototype.clone(): new DiagrampicsdataImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public DiagrampicsdataImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Diagrampicsdata unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Diagrampicsdata unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Diagrampicsdata unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Diagrampicsdata unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Diagrampicsdata unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Diagrampicsdata unmarshal(Reader reader, boolean validate) throws IOException {
        DiagrampicsdataImpl diagrampicsdata = DiagrampicsdataImpl.newInstance();
        diagrampicsdata.setValidating(validate);
        diagrampicsdata.setCurrentUNode(diagrampicsdata);
        diagrampicsdata.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(diagrampicsdata);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", diagrampicsdata);

            // Register content handler
            parser.setContentHandler(diagrampicsdata);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("diagrampicsdata")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
