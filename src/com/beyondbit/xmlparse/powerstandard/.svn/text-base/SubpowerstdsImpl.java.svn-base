...
public class SubpowerstdsImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Subpowerstds {

    private List subpowerstdList;
    ...
    private static SubpowerstdsImpl prototype = null;

    public static void setPrototype(SubpowerstdsImpl prototype) {
        SubpowerstdsImpl.prototype = prototype;
    }
    public static SubpowerstdsImpl newInstance() {
        try {
            return (prototype!=null)?(SubpowerstdsImpl)prototype.clone(): new SubpowerstdsImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public SubpowerstdsImpl() {
        subpowerstdList = new LinkedList();
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public List getSubpowerstdList() {
        return subpowerstdList;
    }

    public void setSubpowerstdList(List subpowerstdList) {
        this.subpowerstdList = subpowerstdList;
    }

    public void addSubpowerstd(Subpowerstd subpowerstd) {
        subpowerstdList.add(subpowerstd);
    }

    public void removeSubpowerstd(Subpowerstd subpowerstd) {
        subpowerstdList.remove(subpowerstd);
    }
    ...
    protected void writeXMLRepresentation(Writer writer,
        ...
        for (Iterator i=subpowerstdList.iterator(); i.hasNext(); ) {
            SubpowerstdImpl subpowerstd = (SubpowerstdImpl)i.next();
            subpowerstd.writeXMLRepresentation(writer,
    ...
    public static Subpowerstds unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Subpowerstds unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Subpowerstds unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Subpowerstds unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Subpowerstds unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Subpowerstds unmarshal(Reader reader, boolean validate) throws IOException {
        SubpowerstdsImpl subpowerstds = SubpowerstdsImpl.newInstance();
        subpowerstds.setValidating(validate);
        subpowerstds.setCurrentUNode(subpowerstds);
        subpowerstds.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(subpowerstds);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", subpowerstds);

            // Register content handler
            parser.setContentHandler(subpowerstds);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("subpowerstds")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
            ...
            // Delegate handling
            if (localName.equalsIgnoreCase("subpowerstd")) {
                SubpowerstdImpl subpowerstd = SubpowerstdImpl.newInstance();
                current = getCurrentUNode();
                subpowerstd.setParentUNode(current);
                subpowerstd.setCurrentUNode(subpowerstd);
                this.setCurrentUNode(subpowerstd);
                subpowerstd.startElement(namespaceURI, localName, qName, atts);
                // Add this value in
                subpowerstdList.add(subpowerstd);
                return;
            }
