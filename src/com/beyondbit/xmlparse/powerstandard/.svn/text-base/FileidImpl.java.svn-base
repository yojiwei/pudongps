...
public class FileidImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Fileid {
    ...
    private static FileidImpl prototype = null;

    public static void setPrototype(FileidImpl prototype) {
        FileidImpl.prototype = prototype;
    }
    public static FileidImpl newInstance() {
        try {
            return (prototype!=null)?(FileidImpl)prototype.clone(): new FileidImpl();
    ...
    public static Fileid unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Fileid unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Fileid unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Fileid unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Fileid unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Fileid unmarshal(Reader reader, boolean validate) throws IOException {
        FileidImpl fileid = FileidImpl.newInstance();
        fileid.setValidating(validate);
        fileid.setCurrentUNode(fileid);
        fileid.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(fileid);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", fileid);

            // Register content handler
            parser.setContentHandler(fileid);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("fileid")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
