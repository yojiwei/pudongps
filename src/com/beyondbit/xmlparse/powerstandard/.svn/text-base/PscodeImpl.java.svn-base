...
public class PscodeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Pscode {
    ...
    private static PscodeImpl prototype = null;

    public static void setPrototype(PscodeImpl prototype) {
        PscodeImpl.prototype = prototype;
    }
    public static PscodeImpl newInstance() {
        try {
            return (prototype!=null)?(PscodeImpl)prototype.clone(): new PscodeImpl();
    ...
    public static Pscode unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Pscode unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Pscode unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Pscode unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Pscode unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Pscode unmarshal(Reader reader, boolean validate) throws IOException {
        PscodeImpl pscode = PscodeImpl.newInstance();
        pscode.setValidating(validate);
        pscode.setCurrentUNode(pscode);
        pscode.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(pscode);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", pscode);

            // Register content handler
            parser.setContentHandler(pscode);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("pscode")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
