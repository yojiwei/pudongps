...
public class LawbasisnameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Lawbasisname {
    ...
    private static LawbasisnameImpl prototype = null;

    public static void setPrototype(LawbasisnameImpl prototype) {
        LawbasisnameImpl.prototype = prototype;
    }
    public static LawbasisnameImpl newInstance() {
        try {
            return (prototype!=null)?(LawbasisnameImpl)prototype.clone(): new LawbasisnameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public LawbasisnameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Lawbasisname unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Lawbasisname unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Lawbasisname unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Lawbasisname unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Lawbasisname unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Lawbasisname unmarshal(Reader reader, boolean validate) throws IOException {
        LawbasisnameImpl lawbasisname = LawbasisnameImpl.newInstance();
        lawbasisname.setValidating(validate);
        lawbasisname.setCurrentUNode(lawbasisname);
        lawbasisname.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(lawbasisname);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", lawbasisname);

            // Register content handler
            parser.setContentHandler(lawbasisname);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("lawbasisname")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
