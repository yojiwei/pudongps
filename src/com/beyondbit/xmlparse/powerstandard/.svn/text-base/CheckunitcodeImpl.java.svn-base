...
public class CheckunitcodeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Checkunitcode {
    ...
    private static CheckunitcodeImpl prototype = null;

    public static void setPrototype(CheckunitcodeImpl prototype) {
        CheckunitcodeImpl.prototype = prototype;
    }
    public static CheckunitcodeImpl newInstance() {
        try {
            return (prototype!=null)?(CheckunitcodeImpl)prototype.clone(): new CheckunitcodeImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public CheckunitcodeImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Checkunitcode unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Checkunitcode unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Checkunitcode unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Checkunitcode unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Checkunitcode unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Checkunitcode unmarshal(Reader reader, boolean validate) throws IOException {
        CheckunitcodeImpl checkunitcode = CheckunitcodeImpl.newInstance();
        checkunitcode.setValidating(validate);
        checkunitcode.setCurrentUNode(checkunitcode);
        checkunitcode.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(checkunitcode);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", checkunitcode);

            // Register content handler
            parser.setContentHandler(checkunitcode);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("checkunitcode")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
