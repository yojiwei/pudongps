...
public class PowerstdstepsImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Powerstdsteps {

    private List powerstdstepList;
    ...
    private static PowerstdstepsImpl prototype = null;

    public static void setPrototype(PowerstdstepsImpl prototype) {
        PowerstdstepsImpl.prototype = prototype;
    }
    public static PowerstdstepsImpl newInstance() {
        try {
            return (prototype!=null)?(PowerstdstepsImpl)prototype.clone(): new PowerstdstepsImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public PowerstdstepsImpl() {
        powerstdstepList = new LinkedList();
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public List getPowerstdstepList() {
        return powerstdstepList;
    }

    public void setPowerstdstepList(List powerstdstepList) {
        this.powerstdstepList = powerstdstepList;
    }

    public void addPowerstdstep(Powerstdstep powerstdstep) {
        powerstdstepList.add(powerstdstep);
    }

    public void removePowerstdstep(Powerstdstep powerstdstep) {
        powerstdstepList.remove(powerstdstep);
    }
    ...
    protected void writeXMLRepresentation(Writer writer,
        ...
        for (Iterator i=powerstdstepList.iterator(); i.hasNext(); ) {
            PowerstdstepImpl powerstdstep = (PowerstdstepImpl)i.next();
            powerstdstep.writeXMLRepresentation(writer,
    ...
    public static Powerstdsteps unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Powerstdsteps unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Powerstdsteps unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Powerstdsteps unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Powerstdsteps unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Powerstdsteps unmarshal(Reader reader, boolean validate) throws IOException {
        PowerstdstepsImpl powerstdsteps = PowerstdstepsImpl.newInstance();
        powerstdsteps.setValidating(validate);
        powerstdsteps.setCurrentUNode(powerstdsteps);
        powerstdsteps.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(powerstdsteps);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", powerstdsteps);

            // Register content handler
            parser.setContentHandler(powerstdsteps);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("powerstdsteps")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
            ...
            // Delegate handling
            if (localName.equalsIgnoreCase("powerstdstep")) {
                PowerstdstepImpl powerstdstep = PowerstdstepImpl.newInstance();
                current = getCurrentUNode();
                powerstdstep.setParentUNode(current);
                powerstdstep.setCurrentUNode(powerstdstep);
                this.setCurrentUNode(powerstdstep);
                powerstdstep.startElement(namespaceURI, localName, qName, atts);
                // Add this value in
                powerstdstepList.add(powerstdstep);
                return;
            }
