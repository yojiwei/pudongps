...
public class Attr2nameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Attr2name {
    ...
    private static Attr2nameImpl prototype = null;

    public static void setPrototype(Attr2nameImpl prototype) {
        Attr2nameImpl.prototype = prototype;
    }
    public static Attr2nameImpl newInstance() {
        try {
            return (prototype!=null)?(Attr2nameImpl)prototype.clone(): new Attr2nameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public Attr2nameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Attr2name unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Attr2name unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Attr2name unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Attr2name unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Attr2name unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Attr2name unmarshal(Reader reader, boolean validate) throws IOException {
        Attr2nameImpl attr2name = Attr2nameImpl.newInstance();
        attr2name.setValidating(validate);
        attr2name.setCurrentUNode(attr2name);
        attr2name.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(attr2name);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", attr2name);

            // Register content handler
            parser.setContentHandler(attr2name);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("attr2name")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
