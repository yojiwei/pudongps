...
public class OpenscopeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Openscope {
    ...
    private static OpenscopeImpl prototype = null;

    public static void setPrototype(OpenscopeImpl prototype) {
        OpenscopeImpl.prototype = prototype;
    }
    public static OpenscopeImpl newInstance() {
        try {
            return (prototype!=null)?(OpenscopeImpl)prototype.clone(): new OpenscopeImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public OpenscopeImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Openscope unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Openscope unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Openscope unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Openscope unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Openscope unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Openscope unmarshal(Reader reader, boolean validate) throws IOException {
        OpenscopeImpl openscope = OpenscopeImpl.newInstance();
        openscope.setValidating(validate);
        openscope.setCurrentUNode(openscope);
        openscope.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(openscope);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", openscope);

            // Register content handler
            parser.setContentHandler(openscope);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("openscope")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
