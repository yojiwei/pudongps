...
public class AusernameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Ausername {
    ...
    private static AusernameImpl prototype = null;

    public static void setPrototype(AusernameImpl prototype) {
        AusernameImpl.prototype = prototype;
    }
    public static AusernameImpl newInstance() {
        try {
            return (prototype!=null)?(AusernameImpl)prototype.clone(): new AusernameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public AusernameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Ausername unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Ausername unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Ausername unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Ausername unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Ausername unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Ausername unmarshal(Reader reader, boolean validate) throws IOException {
        AusernameImpl ausername = AusernameImpl.newInstance();
        ausername.setValidating(validate);
        ausername.setCurrentUNode(ausername);
        ausername.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(ausername);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", ausername);

            // Register content handler
            parser.setContentHandler(ausername);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("ausername")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
