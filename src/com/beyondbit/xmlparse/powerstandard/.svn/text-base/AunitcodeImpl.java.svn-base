...
public class AunitcodeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Aunitcode {
    ...
    private static AunitcodeImpl prototype = null;

    public static void setPrototype(AunitcodeImpl prototype) {
        AunitcodeImpl.prototype = prototype;
    }
    public static AunitcodeImpl newInstance() {
        try {
            return (prototype!=null)?(AunitcodeImpl)prototype.clone(): new AunitcodeImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public AunitcodeImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Aunitcode unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Aunitcode unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Aunitcode unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Aunitcode unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Aunitcode unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Aunitcode unmarshal(Reader reader, boolean validate) throws IOException {
        AunitcodeImpl aunitcode = AunitcodeImpl.newInstance();
        aunitcode.setValidating(validate);
        aunitcode.setCurrentUNode(aunitcode);
        aunitcode.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(aunitcode);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", aunitcode);

            // Register content handler
            parser.setContentHandler(aunitcode);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("aunitcode")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
