...
public class CreateunitnameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Createunitname {
    ...
    private static CreateunitnameImpl prototype = null;

    public static void setPrototype(CreateunitnameImpl prototype) {
        CreateunitnameImpl.prototype = prototype;
    }
    public static CreateunitnameImpl newInstance() {
        try {
            return (prototype!=null)?(CreateunitnameImpl)prototype.clone(): new CreateunitnameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public CreateunitnameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Createunitname unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Createunitname unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Createunitname unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Createunitname unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Createunitname unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Createunitname unmarshal(Reader reader, boolean validate) throws IOException {
        CreateunitnameImpl createunitname = CreateunitnameImpl.newInstance();
        createunitname.setValidating(validate);
        createunitname.setCurrentUNode(createunitname);
        createunitname.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(createunitname);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", createunitname);

            // Register content handler
            parser.setContentHandler(createunitname);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("createunitname")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
