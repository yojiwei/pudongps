...
public class SfidImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Sfid {
    ...
    private static SfidImpl prototype = null;

    public static void setPrototype(SfidImpl prototype) {
        SfidImpl.prototype = prototype;
    }
    public static SfidImpl newInstance() {
        try {
            return (prototype!=null)?(SfidImpl)prototype.clone(): new SfidImpl();
    ...
    public static Sfid unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Sfid unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Sfid unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Sfid unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Sfid unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Sfid unmarshal(Reader reader, boolean validate) throws IOException {
        SfidImpl sfid = SfidImpl.newInstance();
        sfid.setValidating(validate);
        sfid.setCurrentUNode(sfid);
        sfid.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(sfid);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", sfid);

            // Register content handler
            parser.setContentHandler(sfid);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("sfid")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
