...
public class AunitnameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Aunitname {
    ...
    private static AunitnameImpl prototype = null;

    public static void setPrototype(AunitnameImpl prototype) {
        AunitnameImpl.prototype = prototype;
    }
    public static AunitnameImpl newInstance() {
        try {
            return (prototype!=null)?(AunitnameImpl)prototype.clone(): new AunitnameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public AunitnameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Aunitname unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Aunitname unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Aunitname unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Aunitname unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Aunitname unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Aunitname unmarshal(Reader reader, boolean validate) throws IOException {
        AunitnameImpl aunitname = AunitnameImpl.newInstance();
        aunitname.setValidating(validate);
        aunitname.setCurrentUNode(aunitname);
        aunitname.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(aunitname);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", aunitname);

            // Register content handler
            parser.setContentHandler(aunitname);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("aunitname")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
