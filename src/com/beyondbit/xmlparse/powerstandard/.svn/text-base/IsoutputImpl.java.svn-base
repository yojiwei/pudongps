...
public class IsoutputImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Isoutput {
    ...
    private static IsoutputImpl prototype = null;

    public static void setPrototype(IsoutputImpl prototype) {
        IsoutputImpl.prototype = prototype;
    }
    public static IsoutputImpl newInstance() {
        try {
            return (prototype!=null)?(IsoutputImpl)prototype.clone(): new IsoutputImpl();
    ...
    public static Isoutput unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Isoutput unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Isoutput unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Isoutput unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Isoutput unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Isoutput unmarshal(Reader reader, boolean validate) throws IOException {
        IsoutputImpl isoutput = IsoutputImpl.newInstance();
        isoutput.setValidating(validate);
        isoutput.setCurrentUNode(isoutput);
        isoutput.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(isoutput);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", isoutput);

            // Register content handler
            parser.setContentHandler(isoutput);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("isoutput")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
