...
public class CateidImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Cateid {
    ...
    private static CateidImpl prototype = null;

    public static void setPrototype(CateidImpl prototype) {
        CateidImpl.prototype = prototype;
    }
    public static CateidImpl newInstance() {
        try {
            return (prototype!=null)?(CateidImpl)prototype.clone(): new CateidImpl();
    ...
    public static Cateid unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Cateid unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Cateid unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Cateid unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Cateid unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Cateid unmarshal(Reader reader, boolean validate) throws IOException {
        CateidImpl cateid = CateidImpl.newInstance();
        cateid.setValidating(validate);
        cateid.setCurrentUNode(cateid);
        cateid.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(cateid);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", cateid);

            // Register content handler
            parser.setContentHandler(cateid);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("cateid")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
