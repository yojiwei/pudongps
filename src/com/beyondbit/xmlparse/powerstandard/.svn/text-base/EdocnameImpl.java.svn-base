...
public class EdocnameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Edocname {
    ...
    private static EdocnameImpl prototype = null;

    public static void setPrototype(EdocnameImpl prototype) {
        EdocnameImpl.prototype = prototype;
    }
    public static EdocnameImpl newInstance() {
        try {
            return (prototype!=null)?(EdocnameImpl)prototype.clone(): new EdocnameImpl();
    ...
    public static Edocname unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Edocname unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Edocname unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Edocname unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Edocname unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Edocname unmarshal(Reader reader, boolean validate) throws IOException {
        EdocnameImpl edocname = EdocnameImpl.newInstance();
        edocname.setValidating(validate);
        edocname.setCurrentUNode(edocname);
        edocname.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(edocname);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", edocname);

            // Register content handler
            parser.setContentHandler(edocname);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("edocname")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
