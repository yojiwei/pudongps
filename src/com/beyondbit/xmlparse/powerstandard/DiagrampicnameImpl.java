...
public class DiagrampicnameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Diagrampicname {
    ...
    private static DiagrampicnameImpl prototype = null;

    public static void setPrototype(DiagrampicnameImpl prototype) {
        DiagrampicnameImpl.prototype = prototype;
    }
    public static DiagrampicnameImpl newInstance() {
        try {
            return (prototype!=null)?(DiagrampicnameImpl)prototype.clone(): new DiagrampicnameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public DiagrampicnameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Diagrampicname unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Diagrampicname unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Diagrampicname unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Diagrampicname unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Diagrampicname unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Diagrampicname unmarshal(Reader reader, boolean validate) throws IOException {
        DiagrampicnameImpl diagrampicname = DiagrampicnameImpl.newInstance();
        diagrampicname.setValidating(validate);
        diagrampicname.setCurrentUNode(diagrampicname);
        diagrampicname.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(diagrampicname);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", diagrampicname);

            // Register content handler
            parser.setContentHandler(diagrampicname);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("diagrampicname")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
