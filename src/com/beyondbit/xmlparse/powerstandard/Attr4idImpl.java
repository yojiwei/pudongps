...
public class Attr4idImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Attr4id {
    ...
    private static Attr4idImpl prototype = null;

    public static void setPrototype(Attr4idImpl prototype) {
        Attr4idImpl.prototype = prototype;
    }
    public static Attr4idImpl newInstance() {
        try {
            return (prototype!=null)?(Attr4idImpl)prototype.clone(): new Attr4idImpl();
    ...
    public static Attr4id unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Attr4id unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Attr4id unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Attr4id unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Attr4id unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Attr4id unmarshal(Reader reader, boolean validate) throws IOException {
        Attr4idImpl attr4id = Attr4idImpl.newInstance();
        attr4id.setValidating(validate);
        attr4id.setCurrentUNode(attr4id);
        attr4id.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(attr4id);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", attr4id);

            // Register content handler
            parser.setContentHandler(attr4id);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("attr4id")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
