...
public class StateImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, State {
    ...
    private static StateImpl prototype = null;

    public static void setPrototype(StateImpl prototype) {
        StateImpl.prototype = prototype;
    }
    public static StateImpl newInstance() {
        try {
            return (prototype!=null)?(StateImpl)prototype.clone(): new StateImpl();
    ...
    public static State unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static State unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static State unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static State unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static State unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static State unmarshal(Reader reader, boolean validate) throws IOException {
        StateImpl state = StateImpl.newInstance();
        state.setValidating(validate);
        state.setCurrentUNode(state);
        state.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", state);

            // Register content handler
            parser.setContentHandler(state);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("state")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
