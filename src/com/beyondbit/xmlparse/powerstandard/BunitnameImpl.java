...
public class BunitnameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Bunitname {
    ...
    private static BunitnameImpl prototype = null;

    public static void setPrototype(BunitnameImpl prototype) {
        BunitnameImpl.prototype = prototype;
    }
    public static BunitnameImpl newInstance() {
        try {
            return (prototype!=null)?(BunitnameImpl)prototype.clone(): new BunitnameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public BunitnameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Bunitname unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Bunitname unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Bunitname unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Bunitname unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Bunitname unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Bunitname unmarshal(Reader reader, boolean validate) throws IOException {
        BunitnameImpl bunitname = BunitnameImpl.newInstance();
        bunitname.setValidating(validate);
        bunitname.setCurrentUNode(bunitname);
        bunitname.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(bunitname);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", bunitname);

            // Register content handler
            parser.setContentHandler(bunitname);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("bunitname")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
