...
public class PslimitunitImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Pslimitunit {
    ...
    private static PslimitunitImpl prototype = null;

    public static void setPrototype(PslimitunitImpl prototype) {
        PslimitunitImpl.prototype = prototype;
    }
    public static PslimitunitImpl newInstance() {
        try {
            return (prototype!=null)?(PslimitunitImpl)prototype.clone(): new PslimitunitImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public PslimitunitImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Pslimitunit unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Pslimitunit unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Pslimitunit unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Pslimitunit unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Pslimitunit unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Pslimitunit unmarshal(Reader reader, boolean validate) throws IOException {
        PslimitunitImpl pslimitunit = PslimitunitImpl.newInstance();
        pslimitunit.setValidating(validate);
        pslimitunit.setCurrentUNode(pslimitunit);
        pslimitunit.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(pslimitunit);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", pslimitunit);

            // Register content handler
            parser.setContentHandler(pslimitunit);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("pslimitunit")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
