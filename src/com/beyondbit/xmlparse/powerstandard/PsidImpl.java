...
public class PsidImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Psid {
    ...
    private static PsidImpl prototype = null;

    public static void setPrototype(PsidImpl prototype) {
        PsidImpl.prototype = prototype;
    }
    public static PsidImpl newInstance() {
        try {
            return (prototype!=null)?(PsidImpl)prototype.clone(): new PsidImpl();
    ...
    public static Psid unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Psid unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Psid unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Psid unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Psid unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Psid unmarshal(Reader reader, boolean validate) throws IOException {
        PsidImpl psid = PsidImpl.newInstance();
        psid.setValidating(validate);
        psid.setCurrentUNode(psid);
        psid.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(psid);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", psid);

            // Register content handler
            parser.setContentHandler(psid);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("psid")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
