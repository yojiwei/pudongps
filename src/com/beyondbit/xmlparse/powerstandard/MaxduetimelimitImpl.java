...
public class MaxduetimelimitImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Maxduetimelimit {
    ...
    private static MaxduetimelimitImpl prototype = null;

    public static void setPrototype(MaxduetimelimitImpl prototype) {
        MaxduetimelimitImpl.prototype = prototype;
    }
    public static MaxduetimelimitImpl newInstance() {
        try {
            return (prototype!=null)?(MaxduetimelimitImpl)prototype.clone(): new MaxduetimelimitImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public MaxduetimelimitImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Maxduetimelimit unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Maxduetimelimit unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Maxduetimelimit unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Maxduetimelimit unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Maxduetimelimit unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Maxduetimelimit unmarshal(Reader reader, boolean validate) throws IOException {
        MaxduetimelimitImpl maxduetimelimit = MaxduetimelimitImpl.newInstance();
        maxduetimelimit.setValidating(validate);
        maxduetimelimit.setCurrentUNode(maxduetimelimit);
        maxduetimelimit.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(maxduetimelimit);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", maxduetimelimit);

            // Register content handler
            parser.setContentHandler(maxduetimelimit);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("maxduetimelimit")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
