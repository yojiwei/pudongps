...
public class IdImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Id {
    ...
    private static IdImpl prototype = null;

    public static void setPrototype(IdImpl prototype) {
        IdImpl.prototype = prototype;
    }
    public static IdImpl newInstance() {
        try {
            return (prototype!=null)?(IdImpl)prototype.clone(): new IdImpl();
    ...
    public static Id unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Id unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Id unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Id unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Id unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Id unmarshal(Reader reader, boolean validate) throws IOException {
        IdImpl id = IdImpl.newInstance();
        id.setValidating(validate);
        id.setCurrentUNode(id);
        id.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(id);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", id);

            // Register content handler
            parser.setContentHandler(id);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("id")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
