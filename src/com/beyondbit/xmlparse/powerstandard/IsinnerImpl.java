...
public class IsinnerImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Isinner {
    ...
    private static IsinnerImpl prototype = null;

    public static void setPrototype(IsinnerImpl prototype) {
        IsinnerImpl.prototype = prototype;
    }
    public static IsinnerImpl newInstance() {
        try {
            return (prototype!=null)?(IsinnerImpl)prototype.clone(): new IsinnerImpl();
    ...
    public static Isinner unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Isinner unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Isinner unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Isinner unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Isinner unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Isinner unmarshal(Reader reader, boolean validate) throws IOException {
        IsinnerImpl isinner = IsinnerImpl.newInstance();
        isinner.setValidating(validate);
        isinner.setCurrentUNode(isinner);
        isinner.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(isinner);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", isinner);

            // Register content handler
            parser.setContentHandler(isinner);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("isinner")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
