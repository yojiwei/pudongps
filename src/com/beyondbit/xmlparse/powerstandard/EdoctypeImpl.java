...
public class EdoctypeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Edoctype {
    ...
    private static EdoctypeImpl prototype = null;

    public static void setPrototype(EdoctypeImpl prototype) {
        EdoctypeImpl.prototype = prototype;
    }
    public static EdoctypeImpl newInstance() {
        try {
            return (prototype!=null)?(EdoctypeImpl)prototype.clone(): new EdoctypeImpl();
    ...
    public static Edoctype unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Edoctype unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Edoctype unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Edoctype unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Edoctype unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Edoctype unmarshal(Reader reader, boolean validate) throws IOException {
        EdoctypeImpl edoctype = EdoctypeImpl.newInstance();
        edoctype.setValidating(validate);
        edoctype.setCurrentUNode(edoctype);
        edoctype.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(edoctype);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", edoctype);

            // Register content handler
            parser.setContentHandler(edoctype);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("edoctype")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
