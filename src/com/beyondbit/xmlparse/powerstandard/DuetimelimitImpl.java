...
public class DuetimelimitImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Duetimelimit {
    ...
    private static DuetimelimitImpl prototype = null;

    public static void setPrototype(DuetimelimitImpl prototype) {
        DuetimelimitImpl.prototype = prototype;
    }
    public static DuetimelimitImpl newInstance() {
        try {
            return (prototype!=null)?(DuetimelimitImpl)prototype.clone(): new DuetimelimitImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public DuetimelimitImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Duetimelimit unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Duetimelimit unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Duetimelimit unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Duetimelimit unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Duetimelimit unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Duetimelimit unmarshal(Reader reader, boolean validate) throws IOException {
        DuetimelimitImpl duetimelimit = DuetimelimitImpl.newInstance();
        duetimelimit.setValidating(validate);
        duetimelimit.setCurrentUNode(duetimelimit);
        duetimelimit.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(duetimelimit);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", duetimelimit);

            // Register content handler
            parser.setContentHandler(duetimelimit);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("duetimelimit")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
