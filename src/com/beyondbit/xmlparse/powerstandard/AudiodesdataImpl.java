...
public class AudiodesdataImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Audiodesdata {
    ...
    private static AudiodesdataImpl prototype = null;

    public static void setPrototype(AudiodesdataImpl prototype) {
        AudiodesdataImpl.prototype = prototype;
    }
    public static AudiodesdataImpl newInstance() {
        try {
            return (prototype!=null)?(AudiodesdataImpl)prototype.clone(): new AudiodesdataImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public AudiodesdataImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static Audiodesdata unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Audiodesdata unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Audiodesdata unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Audiodesdata unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Audiodesdata unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Audiodesdata unmarshal(Reader reader, boolean validate) throws IOException {
        AudiodesdataImpl audiodesdata = AudiodesdataImpl.newInstance();
        audiodesdata.setValidating(validate);
        audiodesdata.setCurrentUNode(audiodesdata);
        audiodesdata.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(audiodesdata);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", audiodesdata);

            // Register content handler
            parser.setContentHandler(audiodesdata);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("audiodesdata")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
