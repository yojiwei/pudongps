...
public class PowerstdsImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Powerstds {

    private List powerstdList;
    ...
    private static PowerstdsImpl prototype = null;

    public static void setPrototype(PowerstdsImpl prototype) {
        PowerstdsImpl.prototype = prototype;
    }
    public static PowerstdsImpl newInstance() {
        try {
            return (prototype!=null)?(PowerstdsImpl)prototype.clone(): new PowerstdsImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public PowerstdsImpl() {
        powerstdList = new LinkedList();
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public List getPowerstdList() {
        return powerstdList;
    }

    public void setPowerstdList(List powerstdList) {
        this.powerstdList = powerstdList;
    }

    public void addPowerstd(Powerstd powerstd) {
        powerstdList.add(powerstd);
    }

    public void removePowerstd(Powerstd powerstd) {
        powerstdList.remove(powerstd);
    }
    ...
    protected void writeXMLRepresentation(Writer writer,
        ...
        for (Iterator i=powerstdList.iterator(); i.hasNext(); ) {
            PowerstdImpl powerstd = (PowerstdImpl)i.next();
            powerstd.writeXMLRepresentation(writer,
    ...
    public static Powerstds unmarshal(Reader reader, boolean validate) throws IOException {
        PowerstdsImpl powerstds = PowerstdsImpl.newInstance();
        powerstds.setValidating(validate);
        powerstds.setCurrentUNode(powerstds);
        powerstds.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(powerstds);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", powerstds);

            // Register content handler
            parser.setContentHandler(powerstds);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equalsIgnoreCase("powerstds")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
            ...
            // Delegate handling
            if (localName.equalsIgnoreCase("powerstd")) {
                PowerstdImpl powerstd = PowerstdImpl.newInstance();
                current = getCurrentUNode();
                powerstd.setParentUNode(current);
                powerstd.setCurrentUNode(powerstd);
                this.setCurrentUNode(powerstd);
                powerstd.startElement(namespaceURI, localName, qName, atts);
                // Add this value in
                powerstdList.add(powerstd);
                return;
            }
