...
public class UploadDateImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, UploadDate {
    ...
    private static UploadDateImpl prototype = null;

    public static void setPrototype(UploadDateImpl prototype) {
        UploadDateImpl.prototype = prototype;
    }
    public static UploadDateImpl newInstance() {
        try {
            return (prototype!=null)?(UploadDateImpl)prototype.clone(): new UploadDateImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public UploadDateImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static UploadDate unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static UploadDate unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static UploadDate unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static UploadDate unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static UploadDate unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static UploadDate unmarshal(Reader reader, boolean validate) throws IOException {
        UploadDateImpl uploadDate = UploadDateImpl.newInstance();
        uploadDate.setValidating(validate);
        uploadDate.setCurrentUNode(uploadDate);
        uploadDate.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(uploadDate);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", uploadDate);

            // Register content handler
            parser.setContentHandler(uploadDate);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("UploadDate")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
