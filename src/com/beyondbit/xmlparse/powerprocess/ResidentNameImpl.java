...
public class ResidentNameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, ResidentName {
    ...
    private static ResidentNameImpl prototype = null;

    public static void setPrototype(ResidentNameImpl prototype) {
        ResidentNameImpl.prototype = prototype;
    }
    public static ResidentNameImpl newInstance() {
        try {
            return (prototype!=null)?(ResidentNameImpl)prototype.clone(): new ResidentNameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public ResidentNameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static ResidentName unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static ResidentName unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static ResidentName unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static ResidentName unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static ResidentName unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static ResidentName unmarshal(Reader reader, boolean validate) throws IOException {
        ResidentNameImpl residentName = ResidentNameImpl.newInstance();
        residentName.setValidating(validate);
        residentName.setCurrentUNode(residentName);
        residentName.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(residentName);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", residentName);

            // Register content handler
            parser.setContentHandler(residentName);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("ResidentName")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
