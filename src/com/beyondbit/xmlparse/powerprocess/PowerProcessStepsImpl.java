...
public class PowerProcessStepsImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, PowerProcessSteps {

    private List PowerProcessStepList;
    ...
    private static PowerProcessStepsImpl prototype = null;

    public static void setPrototype(PowerProcessStepsImpl prototype) {
        PowerProcessStepsImpl.prototype = prototype;
    }
    public static PowerProcessStepsImpl newInstance() {
        try {
            return (prototype!=null)?(PowerProcessStepsImpl)prototype.clone(): new PowerProcessStepsImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public PowerProcessStepsImpl() {
        PowerProcessStepList = new LinkedList();
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public List getPowerProcessStepList() {
        return PowerProcessStepList;
    }

    public void setPowerProcessStepList(List PowerProcessStepList) {
        this.PowerProcessStepList = PowerProcessStepList;
    }

    public void addPowerProcessStep(PowerProcessStep PowerProcessStep) {
        PowerProcessStepList.add(PowerProcessStep);
    }

    public void removePowerProcessStep(PowerProcessStep PowerProcessStep) {
        PowerProcessStepList.remove(PowerProcessStep);
    }
    ...
    protected void writeXMLRepresentation(Writer writer,
        ...
        writer.write(indent);
        writer.write("<PowerProcessSteps");
        ...
        for (Iterator i=PowerProcessStepList.iterator(); i.hasNext(); ) {
            PowerProcessStepImpl powerProcessStep = (PowerProcessStepImpl)i.next();
            powerProcessStep.writeXMLRepresentation(writer,
    ...
    public static PowerProcessSteps unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static PowerProcessSteps unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static PowerProcessSteps unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static PowerProcessSteps unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static PowerProcessSteps unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static PowerProcessSteps unmarshal(Reader reader, boolean validate) throws IOException {
        PowerProcessStepsImpl powerProcessSteps = PowerProcessStepsImpl.newInstance();
        powerProcessSteps.setValidating(validate);
        powerProcessSteps.setCurrentUNode(powerProcessSteps);
        powerProcessSteps.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(powerProcessSteps);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", powerProcessSteps);

            // Register content handler
            parser.setContentHandler(powerProcessSteps);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("PowerProcessSteps")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
            ...
            // Delegate handling
            if (localName.equals("PowerProcessStep")) {
                PowerProcessStepImpl powerProcessStep = PowerProcessStepImpl.newInstance();
                current = getCurrentUNode();
                powerProcessStep.setParentUNode(current);
                powerProcessStep.setCurrentUNode(powerProcessStep);
                this.setCurrentUNode(powerProcessStep);
                powerProcessStep.startElement(namespaceURI, localName, qName, atts);
                // Add this value in
                PowerProcessStepList.add(powerProcessStep);
                return;
            }
