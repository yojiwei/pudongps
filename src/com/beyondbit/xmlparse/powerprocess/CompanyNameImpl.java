...
public class CompanyNameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, CompanyName {
    ...
    private static CompanyNameImpl prototype = null;

    public static void setPrototype(CompanyNameImpl prototype) {
        CompanyNameImpl.prototype = prototype;
    }
    public static CompanyNameImpl newInstance() {
        try {
            return (prototype!=null)?(CompanyNameImpl)prototype.clone(): new CompanyNameImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public CompanyNameImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static CompanyName unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static CompanyName unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static CompanyName unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static CompanyName unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static CompanyName unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static CompanyName unmarshal(Reader reader, boolean validate) throws IOException {
        CompanyNameImpl companyName = CompanyNameImpl.newInstance();
        companyName.setValidating(validate);
        companyName.setCurrentUNode(companyName);
        companyName.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(companyName);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", companyName);

            // Register content handler
            parser.setContentHandler(companyName);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("CompanyName")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
