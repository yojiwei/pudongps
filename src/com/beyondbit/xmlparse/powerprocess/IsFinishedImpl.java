...
public class IsFinishedImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, IsFinished {
    ...
    private static IsFinishedImpl prototype = null;

    public static void setPrototype(IsFinishedImpl prototype) {
        IsFinishedImpl.prototype = prototype;
    }
    public static IsFinishedImpl newInstance() {
        try {
            return (prototype!=null)?(IsFinishedImpl)prototype.clone(): new IsFinishedImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public IsFinishedImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static IsFinished unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static IsFinished unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static IsFinished unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static IsFinished unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static IsFinished unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static IsFinished unmarshal(Reader reader, boolean validate) throws IOException {
        IsFinishedImpl isFinished = IsFinishedImpl.newInstance();
        isFinished.setValidating(validate);
        isFinished.setCurrentUNode(isFinished);
        isFinished.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(isFinished);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", isFinished);

            // Register content handler
            parser.setContentHandler(isFinished);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("IsFinished")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
