...
public class PPSIDImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, PPSID {
    ...
    private static PPSIDImpl prototype = null;

    public static void setPrototype(PPSIDImpl prototype) {
        PPSIDImpl.prototype = prototype;
    }
    public static PPSIDImpl newInstance() {
        try {
            return (prototype!=null)?(PPSIDImpl)prototype.clone(): new PPSIDImpl();
    ...
    public static PPSID unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static PPSID unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static PPSID unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static PPSID unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static PPSID unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static PPSID unmarshal(Reader reader, boolean validate) throws IOException {
        PPSIDImpl pPSID = PPSIDImpl.newInstance();
        pPSID.setValidating(validate);
        pPSID.setCurrentUNode(pPSID);
        pPSID.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(pPSID);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", pPSID);

            // Register content handler
            parser.setContentHandler(pPSID);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("PPSID")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
