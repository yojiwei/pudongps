...
public class PowerProcessListUnmarshaller {
    ...
    public static PowerProcessList unmarshal(Reader reader, boolean validate) throws IOException {
        ...
        if (entityResolver != null) {
            PowerProcessListImpl.setEntityResolver(entityResolver);
        }

        // Set the error handler, if needed
        if (errorHandler != null) {
            PowerProcessListImpl.setErrorHandler(errorHandler);
        } else {
            if (validate) {
                PowerProcessListImpl.setErrorHandler(new PowerProcessListDefaultErrorHandler());
            }
        }

        // Unmarshal using the implementation class
        return PowerProcessListImpl.unmarshal(reader, validate);
    }

}


class PowerProcessListDefaultErrorHandler implements ErrorHandler {
