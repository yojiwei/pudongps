...
public class BeginDateImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, BeginDate {
    ...
    private static BeginDateImpl prototype = null;

    public static void setPrototype(BeginDateImpl prototype) {
        BeginDateImpl.prototype = prototype;
    }
    public static BeginDateImpl newInstance() {
        try {
            return (prototype!=null)?(BeginDateImpl)prototype.clone(): new BeginDateImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public BeginDateImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static BeginDate unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static BeginDate unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static BeginDate unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static BeginDate unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static BeginDate unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static BeginDate unmarshal(Reader reader, boolean validate) throws IOException {
        BeginDateImpl beginDate = BeginDateImpl.newInstance();
        beginDate.setValidating(validate);
        beginDate.setCurrentUNode(beginDate);
        beginDate.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(beginDate);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", beginDate);

            // Register content handler
            parser.setContentHandler(beginDate);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("BeginDate")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
