...
public class PowerProcessListImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, PowerProcessList {

    private List PowerProcessList;
    ...
    private static PowerProcessListImpl prototype = null;

    public static void setPrototype(PowerProcessListImpl prototype) {
        PowerProcessListImpl.prototype = prototype;
    }
    public static PowerProcessListImpl newInstance() {
        try {
            return (prototype!=null)?(PowerProcessListImpl)prototype.clone(): new PowerProcessListImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public PowerProcessListImpl() {
        PowerProcessList = new LinkedList();
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public List getPowerProcessList() {
        return PowerProcessList;
    }

    public void setPowerProcessList(List PowerProcessList) {
        this.PowerProcessList = PowerProcessList;
    }

    public void addPowerProcess(PowerProcess PowerProcess) {
        PowerProcessList.add(PowerProcess);
    }

    public void removePowerProcess(PowerProcess PowerProcess) {
        PowerProcessList.remove(PowerProcess);
    }
    ...
    protected void writeXMLRepresentation(Writer writer,
        ...
        for (Iterator i=PowerProcessList.iterator(); i.hasNext(); ) {
            PowerProcessImpl powerProcess = (PowerProcessImpl)i.next();
            powerProcess.writeXMLRepresentation(writer,
    ...
    public static PowerProcessList unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static PowerProcessList unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static PowerProcessList unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static PowerProcessList unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static PowerProcessList unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static PowerProcessList unmarshal(Reader reader, boolean validate) throws IOException {
        PowerProcessListImpl powerProcessList = PowerProcessListImpl.newInstance();
        powerProcessList.setValidating(validate);
        powerProcessList.setCurrentUNode(powerProcessList);
        powerProcessList.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(powerProcessList);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", powerProcessList);

            // Register content handler
            parser.setContentHandler(powerProcessList);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("PowerProcessList")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
            ...
            // Delegate handling
            if (localName.equals("PowerProcess")) {
                PowerProcessImpl powerProcess = PowerProcessImpl.newInstance();
                current = getCurrentUNode();
                powerProcess.setParentUNode(current);
                powerProcess.setCurrentUNode(powerProcess);
                this.setCurrentUNode(powerProcess);
                powerProcess.startElement(namespaceURI, localName, qName, atts);
                // Add this value in
                PowerProcessList.add(powerProcess);
                return;
            }
