...
public class SuperviseDESImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, SuperviseDES {
    ...
    private static SuperviseDESImpl prototype = null;

    public static void setPrototype(SuperviseDESImpl prototype) {
        SuperviseDESImpl.prototype = prototype;
    }
    public static SuperviseDESImpl newInstance() {
        try {
            return (prototype!=null)?(SuperviseDESImpl)prototype.clone(): new SuperviseDESImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public SuperviseDESImpl() {
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static SuperviseDES unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static SuperviseDES unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static SuperviseDES unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static SuperviseDES unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static SuperviseDES unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static SuperviseDES unmarshal(Reader reader, boolean validate) throws IOException {
        SuperviseDESImpl superviseDES = SuperviseDESImpl.newInstance();
        superviseDES.setValidating(validate);
        superviseDES.setCurrentUNode(superviseDES);
        superviseDES.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(superviseDES);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", superviseDES);

            // Register content handler
            parser.setContentHandler(superviseDES);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("SuperviseDES")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
