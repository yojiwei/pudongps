...
public class EndDateImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, EndDate {
    ...
    private static EndDateImpl prototype = null;

    public static void setPrototype(EndDateImpl prototype) {
        EndDateImpl.prototype = prototype;
    }
    public static EndDateImpl newInstance() {
        try {
            return (prototype!=null)?(EndDateImpl)prototype.clone(): new EndDateImpl();
    ...
    public static EndDate unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static EndDate unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static EndDate unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static EndDate unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static EndDate unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static EndDate unmarshal(Reader reader, boolean validate) throws IOException {
        EndDateImpl endDate = EndDateImpl.newInstance();
        endDate.setValidating(validate);
        endDate.setCurrentUNode(endDate);
        endDate.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(endDate);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", endDate);

            // Register content handler
            parser.setContentHandler(endDate);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("EndDate")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
