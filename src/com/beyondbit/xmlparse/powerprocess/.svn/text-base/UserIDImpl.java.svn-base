...
public class UserIDImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, UserID {
    ...
    private static UserIDImpl prototype = null;

    public static void setPrototype(UserIDImpl prototype) {
        UserIDImpl.prototype = prototype;
    }
    public static UserIDImpl newInstance() {
        try {
            return (prototype!=null)?(UserIDImpl)prototype.clone(): new UserIDImpl();
    ...
    public static UserID unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static UserID unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static UserID unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static UserID unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static UserID unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static UserID unmarshal(Reader reader, boolean validate) throws IOException {
        UserIDImpl userID = UserIDImpl.newInstance();
        userID.setValidating(validate);
        userID.setCurrentUNode(userID);
        userID.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(userID);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", userID);

            // Register content handler
            parser.setContentHandler(userID);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("UserID")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
