...
public class SequenceImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Sequence {
    ...
    private static SequenceImpl prototype = null;

    public static void setPrototype(SequenceImpl prototype) {
        SequenceImpl.prototype = prototype;
    }
    public static SequenceImpl newInstance() {
        try {
            return (prototype!=null)?(SequenceImpl)prototype.clone(): new SequenceImpl();
    ...
    public static Sequence unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Sequence unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Sequence unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Sequence unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Sequence unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Sequence unmarshal(Reader reader, boolean validate) throws IOException {
        SequenceImpl sequence = SequenceImpl.newInstance();
        sequence.setValidating(validate);
        sequence.setCurrentUNode(sequence);
        sequence.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(sequence);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", sequence);

            // Register content handler
            parser.setContentHandler(sequence);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("Sequence")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
