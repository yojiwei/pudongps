...
public class PSStepIDImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, PSStepID {
    ...
    private static PSStepIDImpl prototype = null;

    public static void setPrototype(PSStepIDImpl prototype) {
        PSStepIDImpl.prototype = prototype;
    }
    public static PSStepIDImpl newInstance() {
        try {
            return (prototype!=null)?(PSStepIDImpl)prototype.clone(): new PSStepIDImpl();
    ...
    public static PSStepID unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static PSStepID unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static PSStepID unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static PSStepID unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static PSStepID unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static PSStepID unmarshal(Reader reader, boolean validate) throws IOException {
        PSStepIDImpl pSStepID = PSStepIDImpl.newInstance();
        pSStepID.setValidating(validate);
        pSStepID.setCurrentUNode(pSStepID);
        pSStepID.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(pSStepID);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", pSStepID);

            // Register content handler
            parser.setContentHandler(pSStepID);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("PSStepID")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
