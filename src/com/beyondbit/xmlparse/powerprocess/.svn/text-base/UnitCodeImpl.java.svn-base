...
public class UnitCodeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, UnitCode {
    ...
    private static UnitCodeImpl prototype = null;

    public static void setPrototype(UnitCodeImpl prototype) {
        UnitCodeImpl.prototype = prototype;
    }
    public static UnitCodeImpl newInstance() {
        try {
            return (prototype!=null)?(UnitCodeImpl)prototype.clone(): new UnitCodeImpl();
    ...
    public static UnitCode unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static UnitCode unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static UnitCode unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static UnitCode unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static UnitCode unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static UnitCode unmarshal(Reader reader, boolean validate) throws IOException {
        UnitCodeImpl unitCode = UnitCodeImpl.newInstance();
        unitCode.setValidating(validate);
        unitCode.setCurrentUNode(unitCode);
        unitCode.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(unitCode);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", unitCode);

            // Register content handler
            parser.setContentHandler(unitCode);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("UnitCode")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
