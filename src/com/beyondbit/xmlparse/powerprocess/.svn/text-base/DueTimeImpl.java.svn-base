...
public class DueTimeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, DueTime {
    ...
    private static DueTimeImpl prototype = null;

    public static void setPrototype(DueTimeImpl prototype) {
        DueTimeImpl.prototype = prototype;
    }
    public static DueTimeImpl newInstance() {
        try {
            return (prototype!=null)?(DueTimeImpl)prototype.clone(): new DueTimeImpl();
    ...
    public static DueTime unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static DueTime unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static DueTime unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static DueTime unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static DueTime unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static DueTime unmarshal(Reader reader, boolean validate) throws IOException {
        DueTimeImpl dueTime = DueTimeImpl.newInstance();
        dueTime.setValidating(validate);
        dueTime.setCurrentUNode(dueTime);
        dueTime.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(dueTime);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", dueTime);

            // Register content handler
            parser.setContentHandler(dueTime);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("DueTime")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
