...
public class PPIDImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, PPID {
    ...
    private static PPIDImpl prototype = null;

    public static void setPrototype(PPIDImpl prototype) {
        PPIDImpl.prototype = prototype;
    }
    public static PPIDImpl newInstance() {
        try {
            return (prototype!=null)?(PPIDImpl)prototype.clone(): new PPIDImpl();
    ...
    public static PPID unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static PPID unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static PPID unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static PPID unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static PPID unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static PPID unmarshal(Reader reader, boolean validate) throws IOException {
        PPIDImpl pPID = PPIDImpl.newInstance();
        pPID.setValidating(validate);
        pPID.setCurrentUNode(pPID);
        pPID.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(pPID);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", pPID);

            // Register content handler
            parser.setContentHandler(pPID);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("PPID")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
