...
public class UnitIDImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, UnitID {
    ...
    private static UnitIDImpl prototype = null;

    public static void setPrototype(UnitIDImpl prototype) {
        UnitIDImpl.prototype = prototype;
    }
    public static UnitIDImpl newInstance() {
        try {
            return (prototype!=null)?(UnitIDImpl)prototype.clone(): new UnitIDImpl();
    ...
    public static UnitID unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static UnitID unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static UnitID unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static UnitID unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static UnitID unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static UnitID unmarshal(Reader reader, boolean validate) throws IOException {
        UnitIDImpl unitID = UnitIDImpl.newInstance();
        unitID.setValidating(validate);
        unitID.setCurrentUNode(unitID);
        unitID.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(unitID);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", unitID);

            // Register content handler
            parser.setContentHandler(unitID);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("UnitID")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
