...
public class MemoImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Memo {
    ...
    private static MemoImpl prototype = null;

    public static void setPrototype(MemoImpl prototype) {
        MemoImpl.prototype = prototype;
    }
    public static MemoImpl newInstance() {
        try {
            return (prototype!=null)?(MemoImpl)prototype.clone(): new MemoImpl();
    ...
    public static Memo unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Memo unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Memo unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Memo unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Memo unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Memo unmarshal(Reader reader, boolean validate) throws IOException {
        MemoImpl memo = MemoImpl.newInstance();
        memo.setValidating(validate);
        memo.setCurrentUNode(memo);
        memo.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(memo);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", memo);

            // Register content handler
            parser.setContentHandler(memo);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("Memo")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
