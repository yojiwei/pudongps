...
public class CodeImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, Code {
    ...
    private static CodeImpl prototype = null;

    public static void setPrototype(CodeImpl prototype) {
        CodeImpl.prototype = prototype;
    }
    public static CodeImpl newInstance() {
        try {
            return (prototype!=null)?(CodeImpl)prototype.clone(): new CodeImpl();
    ...
    public static Code unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static Code unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static Code unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static Code unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static Code unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static Code unmarshal(Reader reader, boolean validate) throws IOException {
        CodeImpl code = CodeImpl.newInstance();
        code.setValidating(validate);
        code.setCurrentUNode(code);
        code.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(code);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", code);

            // Register content handler
            parser.setContentHandler(code);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("Code")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
