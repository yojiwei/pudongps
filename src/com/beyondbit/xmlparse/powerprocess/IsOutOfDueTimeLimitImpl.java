...
public class IsOutOfDueTimeLimitImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, IsOutOfDueTimeLimit {

    private String value;
    private boolean zeus_ValueSet;
    ...
    private static IsOutOfDueTimeLimitImpl prototype = null;

    public static void setPrototype(IsOutOfDueTimeLimitImpl prototype) {
        IsOutOfDueTimeLimitImpl.prototype = prototype;
    }
    public static IsOutOfDueTimeLimitImpl newInstance() {
        try {
            return (prototype!=null)?(IsOutOfDueTimeLimitImpl)prototype.clone(): new IsOutOfDueTimeLimitImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public IsOutOfDueTimeLimitImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    protected void writeXMLRepresentation(Writer writer,
        ...
        writer.write(indent);
        writer.write("<IsOutOfDueTimeLimit");
        ...
        if (getValue() != null) {
            // Handle PCDATA value
            writer.write(">");
            writer.write(escapeTextValue(getValue()));
            writer.write("</IsOutOfDueTimeLimit>\n");
        } else {
            writer.write("/>\n");
        }
    ...
    public static IsOutOfDueTimeLimit unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static IsOutOfDueTimeLimit unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static IsOutOfDueTimeLimit unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static IsOutOfDueTimeLimit unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static IsOutOfDueTimeLimit unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static IsOutOfDueTimeLimit unmarshal(Reader reader, boolean validate) throws IOException {
        IsOutOfDueTimeLimitImpl isOutOfDueTimeLimit = IsOutOfDueTimeLimitImpl.newInstance();
        isOutOfDueTimeLimit.setValidating(validate);
        isOutOfDueTimeLimit.setCurrentUNode(isOutOfDueTimeLimit);
        isOutOfDueTimeLimit.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(isOutOfDueTimeLimit);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", isOutOfDueTimeLimit);

            // Register content handler
            parser.setContentHandler(isOutOfDueTimeLimit);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("IsOutOfDueTimeLimit")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
    ...
    public void endElement(String namespaceURI, String localName,
        ...
        // Make sure we have textual value
        if (this.value == null) {
            this.value = "";
        }
    ...
    public void characters(char[] ch, int start, int len)
        ...
        if (this.value == null) {
            ...
            this.value = new StringBuffer(this.value).append(text).toString();
        }
    }
