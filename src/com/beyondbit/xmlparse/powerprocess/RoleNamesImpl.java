...
public class RoleNamesImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, RoleNames {
    ...
    private static RoleNamesImpl prototype = null;

    public static void setPrototype(RoleNamesImpl prototype) {
        RoleNamesImpl.prototype = prototype;
    }
    public static RoleNamesImpl newInstance() {
        try {
            return (prototype!=null)?(RoleNamesImpl)prototype.clone(): new RoleNamesImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public RoleNamesImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static RoleNames unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static RoleNames unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static RoleNames unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static RoleNames unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static RoleNames unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static RoleNames unmarshal(Reader reader, boolean validate) throws IOException {
        RoleNamesImpl roleNames = RoleNamesImpl.newInstance();
        roleNames.setValidating(validate);
        roleNames.setCurrentUNode(roleNames);
        roleNames.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(roleNames);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", roleNames);

            // Register content handler
            parser.setContentHandler(roleNames);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("RoleNames")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
