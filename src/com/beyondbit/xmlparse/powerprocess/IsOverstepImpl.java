...
public class IsOverstepImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, IsOverstep {
    ...
    private static IsOverstepImpl prototype = null;

    public static void setPrototype(IsOverstepImpl prototype) {
        IsOverstepImpl.prototype = prototype;
    }
    public static IsOverstepImpl newInstance() {
        try {
            return (prototype!=null)?(IsOverstepImpl)prototype.clone(): new IsOverstepImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public IsOverstepImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static IsOverstep unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static IsOverstep unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static IsOverstep unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static IsOverstep unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static IsOverstep unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static IsOverstep unmarshal(Reader reader, boolean validate) throws IOException {
        IsOverstepImpl isOverstep = IsOverstepImpl.newInstance();
        isOverstep.setValidating(validate);
        isOverstep.setCurrentUNode(isOverstep);
        isOverstep.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(isOverstep);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", isOverstep);

            // Register content handler
            parser.setContentHandler(isOverstep);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("IsOverstep")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
