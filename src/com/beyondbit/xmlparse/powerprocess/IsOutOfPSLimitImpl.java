...
public class IsOutOfPSLimitImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, IsOutOfPSLimit {
    ...
    private static IsOutOfPSLimitImpl prototype = null;

    public static void setPrototype(IsOutOfPSLimitImpl prototype) {
        IsOutOfPSLimitImpl.prototype = prototype;
    }
    public static IsOutOfPSLimitImpl newInstance() {
        try {
            return (prototype!=null)?(IsOutOfPSLimitImpl)prototype.clone(): new IsOutOfPSLimitImpl();
        } catch (CloneNotSupportedException e) {
            return null; // never
        }
    }
    public IsOutOfPSLimitImpl() {
        zeus_ValueSet = false;
        docTypeString = "";
        hasDTD = false;
        validate = false;
    ...
    public static IsOutOfPSLimit unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static IsOutOfPSLimit unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static IsOutOfPSLimit unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static IsOutOfPSLimit unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static IsOutOfPSLimit unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static IsOutOfPSLimit unmarshal(Reader reader, boolean validate) throws IOException {
        IsOutOfPSLimitImpl isOutOfPSLimit = IsOutOfPSLimitImpl.newInstance();
        isOutOfPSLimit.setValidating(validate);
        isOutOfPSLimit.setCurrentUNode(isOutOfPSLimit);
        isOutOfPSLimit.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(isOutOfPSLimit);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", isOutOfPSLimit);

            // Register content handler
            parser.setContentHandler(isOutOfPSLimit);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("IsOutOfPSLimit")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
