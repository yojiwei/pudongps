...
public class UserNameImpl extends DefaultHandler implements Cloneable, Unmarshallable, LexicalHandler, UserName {
    ...
    private static UserNameImpl prototype = null;

    public static void setPrototype(UserNameImpl prototype) {
        UserNameImpl.prototype = prototype;
    }
    public static UserNameImpl newInstance() {
        try {
            return (prototype!=null)?(UserNameImpl)prototype.clone(): new UserNameImpl();
    ...
    public static UserName unmarshal(File file) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file));
    }

    public static UserName unmarshal(File file, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new FileReader(file), validate);
    }

    public static UserName unmarshal(InputStream inputStream) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream));
    }

    public static UserName unmarshal(InputStream inputStream, boolean validate) throws IOException {
        // Delegate to the unmarshal(Reader) method
        return unmarshal(new InputStreamReader(inputStream), validate);
    }

    public static UserName unmarshal(Reader reader) throws IOException {
        // Delegate with default validation value
        return unmarshal(reader, false);
    }

    public static UserName unmarshal(Reader reader, boolean validate) throws IOException {
        UserNameImpl userName = UserNameImpl.newInstance();
        userName.setValidating(validate);
        userName.setCurrentUNode(userName);
        userName.setParentUNode(null);
        // Load the XML parser
        ...
        try {
            ...
            // Set error handler
            parser.setErrorHandler(userName);

            // Register lexical handler
            parser.setProperty("http://xml.org/sax/properties/lexical-handler", userName);

            // Register content handler
            parser.setContentHandler(userName);
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        if ((localName.equals("UserName")) && (!zeus_thisNodeHandled)) {
            // Handle ourselves
