...
public class Login extends HttpServlet {
	...
	public void init(ServletConfig servletconfig) throws ServletException {
		...
		loginForm = app.getInitParameter("edu.yale.its.tp.cas.loginForm");// 登录页面
		serviceSuccess = app
	...
	public void doGet(HttpServletRequest httpservletrequest,
		...
		app.getRequestDispatcher(loginForm).forward(httpservletrequest,
				httpservletresponse);
	}
	...
	private void grantForService(HttpServletRequest httpservletrequest,
		...
		try {
			if (s != null) {
				...
				s = "http://31.6.130.122:5203/success.jsp";// 存放服务器的成功页面做为请求页
				ServiceTicket serviceticket = new ServiceTicket(
						ticketgrantingticket, s, flag);
				String s1 = stCache.addTicket(serviceticket);// 产生Ticket
				// 保存开始
