              ...
              <tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td width="250" align="left" valign="middle" class="f14"><font color="#AD0000"><strong>在办事项</strong></font></td>
                                     <!--查询文件导入 -->
...
{
		sqlStr = "select a.wo_isovertime,a.pr_id,a.wo_id,a.wo_status,a.wo_projectname, b.pr_timelimit, b.pr_url from tb_work a,tb_proceeding b where a.us_id='"+us_id+"' and a.pr_id=b.pr_id  and a.wo_status in('0','1','2','8') order by a.wo_applytime desc";
 ...
 %>
                      ...
                      <tr>
                       <!-- <td width="5%" height="22" align="center" valign="middle"><input type="checkbox" name="checkbox2" value="checkbox" /></td>
                        ...
                        <td width="5%" align="center" valign="middle"><%if(Integer.parseInt(l_content.get("wo_status").toString())==3){%><img src="images/usercenterNEW_icon17.gif" width="14" height="12" />
						<%}else {%> <img src="images/usercenterNEW_icon18.gif" width="14" height="12" /><%}%></td>
                      -->
                        <td width="21%" align="center" valign="middle">
						<%
							if(pr_url.equals("")){
														%>
														  <a title="<%=pr_name%>" href="/website/workHall/apply.jsp?pr_id=<%=pr_id%>&wo_id=<%=wo_id%>&asd=<%=CBase64.getEncodeString(us_uid)%>&rfv=<%=CBase64.getEncodeString(user.getPwd())%>" target="_blank"><%if(pr_name.length()>16) out.print("&nbsp;"+pr_name.substring(0,16)+"...");else out.print("&nbsp;"+pr_name);%></a></td>
														<%
															} else {
																...
																if(pr_url.indexOf("?") == -1){
														%>
														  <a title="<%=pr_name%>" href="<%=pr_url%>?pr_id=<%=pr_id%>&ow_id=<%=ow_id%>&wo_status=<%=wo_status%>&wo_id=<%=wo_id%>&owt1_id=<%=owt1_id%>&asd=<%=CBase64.getEncodeString(us_uid)%>&rfv=<%=CBase64.getEncodeString(user.getPwd())%>" target="_blank"><%if(pr_name.length()>8) out.print("&nbsp;"+pr_name.substring(0,8)+"...");else out.print("&nbsp;"+pr_name);%></a></td>
														<%
																} else {
														%>
