...
function st(o,s){
	...
	f.name = "formData";
	f.action = "http://61.129.65.86/website/workHall/workList.jsp?sj_dir="+s+"&pardir=workHall";
	f.method = "post";
	f.appendChild(v);
...
%>
  <table width="519" border="0" cellspacing="0" cellpadding="0">
	<tr>
	  <td class="z_bg_02a"><div align="right" class="STYLE2"><a href="http://61.129.65.86/website/workHall/index.jsp"><font color="#FFFFFF">[进入专栏]</font></a></div></td>
	</tr>
	<tr>
  ...
  <table width="260">
  	...
  	<%
			...
			<%
				...
				out.print("<td><a href='http://61.129.65.86/website/workHall/workList.jsp?sj_dir=perMore&pardir=workHall#listTitle'>[更多]</a></td>");
			}	
			%></tr></table>
  </div>
  <div> 
  	...
  	<tr height="30">
	   ...
	   %>
			...
			<%
				...
				out.print("<td><a href='http://61.129.65.86/website/workHall/workList.jsp?sj_dir=enMore&pardir=workHall#listTitle'>[更多]</a></td>");
			}
			%></tr></table>
  </div>
  <div> 
  	...
  	<tr height="30">
    	<%
			...
			<%	
				...
				out.print("<td><a href='http://61.129.65.86/website/workHall/workList.jsp?sj_dir=speMore&pardir=workHall#listTitle'>[更多]</a></td>");
			}
			%></tr></table>
  </div>
</div>		
...
<div id="online">
    <ul> 
      ...
      <li><a href="http://61.129.65.86/website/usercenter" onMouseOver="show1(2)">状态查询</a></li>
      <!--li><div style="valign:center;margin-top:3px">&nbsp;&nbsp;&nbsp;<img src="../images/index/statusquery.gif" style="cursor:hand" onclick="window.location.href='http://61.129.65.86/website/usercenter/index.jsp'"/></div></li-->
    </ul>
</div>
<div id="online_c">
  <div style="display:block">
	...
	<%
		...
		if (vPage!=null){
			for( int i=0;i<vPage.size();i++){
				...
				title = title.length() > 18 ? title.substring(0,17) + ".." : title;
	%>
		<li>·<A HREF="http://61.129.65.86/website/workHall/Govdownload.jsp?pa_id=<%=content.get("pa_id").toString()%>" title="下载该表格" target="downFrm"><%=title%></a></li>
		<%
			}
		}
		%>
	...
	<td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
		  <tr>
			...
			<form name="formWSearch" action="http://61.129.65.86/website/workHall/SList.jsp" target="_blank" method="post">
				<table width="100%" height="90" border="0" cellpadding="0" cellspacing="0">
				  ...
				  <tr>
					<td><div align="center">
					  ...
					  <select name="sValue" style="width:111px;"><option value="0">--</option>
					  </select>
<%
sqlStr="select d.dt_name as name,d.dt_id as id from tb_deptinfo d where d.dt_iswork=1 and (dt_parent_id = 1129 or d.dt_id = 35) or d.dt_parent_id in (select dt_id from tb_deptinfo where dt_name like '%功能区域%') order by d.dt_sequence";
...
%>
				  ...
				  <tr>
					<td height="22"><table width="224" height="22" border="0" align="center" cellpadding="0" cellspacing="0">
					  <tr>
						<td><div align="right">
						  <input name="kWord" type="text" class="s_02"  value="请输入关键字"/>
						  <input type="hidden" name="sj_dir" value="workHall">
						</div></td>
						<td><div align="left"><img src="../images/index/so1.gif" width="52" height="21" onClick="javascript:document.formWSearch.submit();" style="cursor:hand"/></div></td>
					  </tr>
					</table>
					  <table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
...
<div id="Feedback">
    <ul> 
      ...
      <li style="padding-top:8px" id=more><span style="cursor:hand;" onclick="javascript:window.location.href='http://61.129.65.86/website/workHall/feedbackMore.jsp';">更多&gt;&gt;</span></li>
    </ul>
</div>
<div id="Feedback_c">
  <div style="display:block">
	...
	<%
		...
		if(vPage!=null)
		{
			for(int i=0;i<vPage.size();i++)
			{
				...
				title = title.length() > 18 ? title.substring(0,17) + ".." : title;
	%>
		<li>·<a href="http://61.129.65.86/website/usercenter/index.jsp" class="three"><%=title%></a></li>
		<%
			}
		}
		%>
	...
	<%
		...
		if(vPage!=null)
		{
			for(int i=0;i<vPage.size();i++)
			{
				...
				title = title.length() > 18 ? title.substring(0,17) + ".." : title;
	%>
		<li>·<a href="http://61.129.65.86/website/workHall/contentwzw.jsp?wz_id=<%=content.get("wz_id").toString()%>" class="three"><%=title%></a></li>
		<%
			}
		}
		%>
		  ...
		  <table width="519" border="0" align="center" cellpadding="0" cellspacing="0">
            <tr>
              <td class="z_bg_03"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
