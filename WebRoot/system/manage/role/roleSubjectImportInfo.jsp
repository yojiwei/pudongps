...
<%
 ...
 // String module = manage.getAtModule();
  ...
  moduleIds=ado.getAccessByRole(tr_id,ado.ColumnSelAccess);
%>
  <xml id=xmlDoc>
<%
...
if(!ado.isAdmin(user_id))
  tree.setFilterSql(ado.getAccessSqlByUser(user_id,ado.ColumnSelAccess));

out.print(tree.getXMLByParentID(0)); //输出XML
*/

	String strXml=CTools.dealNull(session.getAttribute("_InfoImportAccess"));
	if(strXml.equals(""))
	{
		if(!ado.isAdmin(user_id))
		  tree.setFilterSql(ado.getAccessSqlByUser(user_id,ado.ColumnAccess));
		
		strXml=tree.getXMLByParentID(0);
		session.setAttribute("_InfoImportAccess",strXml);
	}
...
<form name="formData" method="post" action="roleSubjectImportResult.jsp">
