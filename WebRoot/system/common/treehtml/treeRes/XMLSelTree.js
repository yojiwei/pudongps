...
function LoadNodeContent(p_oSrcElement)
{
    ...
    if (oLabel != null)
    {
        ...
        if (oLink != null)
        {
			...
			var sContent = String(oLink.cont);
			var sEnglishName = String(oLink.englishName);
			var sShowFlag = String(oLink.showFlag);
			if(oLink.target)
            {
				GoLink(sHref, oLink.target,sContent,sEnglishName,sShowFlag) ;
            }
        }
    }
}
