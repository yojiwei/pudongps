...
public String getRedom() {
  	...
  	String sToday = sdf.format(dt);
  	int filenum = 0;
    filenum =(int)(Math.random()*100000);
    String stPath = getServletConfig().getServletContext().getRealPath("/");
  	String reStr = sToday + Integer.toString(filenum);
  	String stFile = stPath + sUploadDir + reStr + "/";
...
{    //远程自动获取
	...
	System.out.println("remote " + sUrl);
	...
	// Retreive the current file
		...
		if (!myFile.isMissing()) {
			...
			if (session.getAttribute("temppath") != null)
				randomStr = (String)session.getAttribute("temppath");
			else
				randomStr = getRedom();	
			session.setAttribute("temppath",randomStr);
			sUploadDir = sUploadDir + randomStr+"/";
			//end modify
			
			FileName=(String)date.Time_Stamp();
			sOriginalFileName=myFile.getFileName();
			sSaveFileName=FileName+"."+myFile.getFileExt();
			String fileNames = sSaveFileName;
			sPathFileName=sUploadDir + sSaveFileName;
			System.out.println("----------===========" + "/"+sPathFileName);
			myFile.saveAs("/"+sPathFileName);
			System.out.println("save " + sPathFileName);
			
			//modify for hh
			sSaveFileName = randomStr + "/" + sSaveFileName;
			//end modify
		}
	}
   out.println("<script language=javascript>");
