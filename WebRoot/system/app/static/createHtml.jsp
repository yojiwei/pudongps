...
try {
 ...
 dImpl = new CDataImpl(dCn); 
	...
	String s_fiid=CTools.dealNull(request.getParameter("fi_id")).trim();
	...
	if(content!=null)
	{
			...
			b_flag = myurl.createHtml(Url_source,request.getRealPath("/"),Url_destination);
	}
	//自动生成互动平台首页信息滚动条 o1370 为滚动条的fi_id  2005-5-11日添加
	Url_sql = "select fi_url,fi_content from tb_frontinfo where fi_id='o1370'";
	content = dImpl.getDataInfo(Url_sql);
	if(content!=null)
	{
			Url_source = content.get("fi_content").toString();
			Url_destination = content.get("fi_url").toString();
	...
	////////////////
	//自动生成公务员门户的最新新闻 o1561 为滚动条的fi_id  2005-12-02日添加
	Url_sql = "select fi_url,fi_content from tb_frontinfo where fi_id='o1561'";
	content = dImpl.getDataInfo(Url_sql);
	if(content!=null)
	{
			Url_source = content.get("fi_content").toString();
