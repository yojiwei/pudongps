...
onlineSql = "select dx_id,dx_num,to_char(dx_applytime,'yyyy-MM-dd hh24:mi:ss') as dx_applytime,dx_name,dx_backtype,dx_tel,dx_address,dx_codenumber,dx_email,dx_content,dx_use,dx_type,dx_status,dx_backcontent from tb_daxx where dx_type='ask' ";
if(!"".equals(dx_id)){
...
<input type="hidden" name="dx_type" value="ask"/>
...
<tr class="line-even">
  ...
  <td colspan="3" align="left" valign="middle">
  	<input name="name" type="text"  readonly id="name" size="47" value="<%=dx_name%>" /></td>
  </tr>
<tr class="line-odd">
...
<tr class="line-odd">
  ...
  <td  colspan="3" align="left" valign="middle"><input name="tel" type="text" readonly id="tel" size="47" value="<%=dx_tel%>" /></td>
</tr>
<tr class="line-even">
  <td  align="right" valign="middle">电子邮件：</td>
  <td colspan="3" align="left" valign="middle"><input name="email" type="text"readonly  id="email" size="47" value="<%=dx_email%>"/></td>
</tr>
<tr class="line-odd">
  <td align="right" valign="middle">联系地址：</td>
  <td colspan="3" align="left" valign="middle"><input name="address" type="text" readonly id="address" size="47"  value="<%=dx_address%>"/></td>
</tr>
<tr class="line-even">
  <td  align="right" valign="middle">邮　&nbsp; 编：</td>
  <td  colspan="3" align="left" valign="middle"><input name="codenum" type="text" readonly id="codenum"  size="47" maxlength="6" value="<%=dx_codenumber%>" /></td>
</tr>
<tr class="line-even">
  <td align="right" valign="middle">主　　题：</td>
  <td colspan="3" align="left" valign="middle"><input name="usertobe" type="text" readonly  id="usertobe" size="47" value="<%=dx_use%>" /></td>
</tr>
<tr class="line-odd">
  <td align="right" valign="middle">咨询内容：</td>
...
<!--附件-->
<%
sqlStr = "select da_imgrealname,da_id from tb_daxxattach where dx_id='"+dx_id+"'";
vPage = dImpl.splitPageOpt(sqlStr,10,1);
if (vPage!=null){
	
%>
<tr class="line-even">
  <td align="right" valign="middle">附　　件：</td>
  <td colspan="3" align="left" valign="middle">
  <%
	for(int i=0;i<vPage.size();i++){
		content = (Hashtable)vPage.get(i);
	%>
	<%=CTools.dealNull(content.get("da_imgrealname"))%>&nbsp;&nbsp;
	<A HREF="onlineDownload.jsp?da_id=<%=CTools.dealNull(content.get("da_id"))%>" target="downFrm" title="下载该文件"><img class="hand" border="0" src="/website/images/pub.gif" title="下载" WIDTH="16" HEIGHT="16"></A></br>
	<%}%></td>
</tr>
<%
}
...
<tr class="outset-table">
  <td colspan="4" align="center" valign="middle"><input type="button" name = "sub1" value="&nbsp;提交&nbsp;" onclick="checkform()"/>
&nbsp;&nbsp;&nbsp;<input type="button" name = "sub5" value="&nbsp;删除&nbsp;" onclick="javascript:window.location.href='onlineDel.jsp?dx_id=<%=dx_id%>';"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="reset" name = "sub2" value="&nbsp;重置&nbsp;"/></td>
  </tr>
<%}else{%>
<tr class="outset-table">
  <td colspan="4" align="center" valign="middle"><input type="button" name = "sub3" value="&nbsp;删除&nbsp;" onclick="javascript:window.location.href='onlineDel.jsp?dx_id=<%=dx_id%>&dx_type=ask';"/>&nbsp;&nbsp;<input type="button" name = "sub4" value="&nbsp;返回&nbsp;" onclick="javascript:window.history.back();"/></td>
  </tr>
<%}%>
</form>
