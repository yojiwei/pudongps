...
try {
 ...
 String aa_id = CTools.dealString(request.getParameter("op_id")).trim();
 ...
 if (!aa_id.equals(""))
 {
	...
	sqlStr = "select op_id,op_imgname,op_imgpath from tb_daxxphoto where op_id='"+aa_id+"'";
	 
	 content = dImpl.getDataInfo(sqlStr);
	 if (content!=null)
	 {
	 	 zipFile = CTools.dealNull(content.get("op_imgname").toString());
		 zipCnt = zipFile.lastIndexOf("\\\\");
		 if(zipCnt != -1)
		 	zipFile = zipFile.substring(zipCnt+2);
			
		 pa_path = CTools.dealString(content.get("op_imgpath").toString());
