...
String dept,start_date,end_date;
dept=CTools.dealString(request.getParameter("dept")).trim();
...
<table border="0" width="100%" cellpadding="3" height="44">
   <tr class="bttn">
	 ...
	 <td width="25%" height="1" class="outset-table">操作部门</td>
...
<%
    String sql="";
   sql="select distinct dt_id,dt_name,dt_parent_id from tb_deptinfo where 1=1 ";
   if(!dept.equals("all"))
     {sql=sql+ "and dt_id =" + dept;
     }
   else {
   sql=sql+ " and (dt_id in (3,10,25,26,27,28,30))";
     }
	// out.println(sql);
	      ...
	      for(int j=0;j<vectorPage.size();j++)
			 {
               ...
               out.println("<td>" + content.get("dt_name") + "</td>");

//内层循环，读出统计数量
      String strSql="select count(cl.ui_id) as countnum,cl.cl_operate as cl_operate from tb_contentlog cl, tb_userinfo ui,tb_deptinfo dt "+sWhere+"and cl.ui_id=ui.ui_id and ui.dt_id=dt.dt_id and dt.dt_id="+content.get("dt_id")+" group by cl_operate";
