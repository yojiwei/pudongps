...
<%
	...
	String getbeginYear = CTools.dealString(
			request.getParameter("reportyear")).trim();
	String getbeginMon = CTools.dealString(
			request.getParameter("reportmonth")).trim();
	...
	try {
		...
		if (!"".equals(dt_id)) {
			...
			String dtSql = "select dt_id from tb_deptinfo where dt_name like '%公安局浦东分局%' or dt_name like '%税务局%' or dt_name like '%质监局%' or dt_name like '%食品药品监督管理局浦东新区分局%' or dt_name like '%工商行政管理局浦东新区分局%'";
	...
	String dd_name = "";
	String strSql_dt ="select dt_name from deptinfo where dt_id='"+dt_id+"'";
	content = dImpl.getDataInfo(strSql_dt);
	if(content!=null) 
	dd_name = content.get("dt_name").toString();
//<!--  分析说明  -->
...
//<!--  结束   -->
		...
		sqlStrYear = "select sum(alleinfo) as alleinfoyear,sum(GONGWENINFO) as GONGWENINFOyear,sum(ZHUDONGTOTALSINFO) as ZHUDONGTOTALSINFOyear,sum(newinfo) as newinfoyear,sum(serviceinfo) as serviceinfoyear,"
		...
		+ "sum(copychargefdisk) as copychargefdiskyear,sum(cfsqs) as cfsqsyear from iostat where reportyear = "
		+ getbeginYear + " and uiid in (select ui_id from tb_userinfo where dt_id = " + dt_id + ") and reportmonth <= " + getbeginMon;
		//out.println(sqlStr);
		if (!"".equals(dt_id)) {
			sqlStr += " and did = " + dt_id;
			sqlStrYear  += " and did = " + dt_id;
		}else{
			sqlStr += " and did not in (" + dt_ids + ")";
			sqlStrYear  += " and did not in (" + dt_ids + ")";
		}
...
<table class="FindAreaTable1" cellspacing="1" rules="cols" border="0" id="ctl00_DataGrid_DataGrid1" style="border-width:0px;">
					...
					<tr class="bttn">
						<td  colspan='5' align="left">
							<div>分析说明：</div><textarea name="FXSM" rows="5" id="FXSM" cols="95" readonly><%=FXSM%></textarea>
						</td>
					</tr>
	...
	<%
			...
			+ cloumnVal + "')</script>");
				...
				if (columnName.equals("DEALCHARGE")) {
			out.print("<script language='javascript'>todo('DEALCHARGEyear','"
			+ cloumnVal + "')</script>");
				}
				if (columnName.equals("DEALPAY")) {
			out.print("<script language='javascript'>todo('DEALPAYyear','"
			+ cloumnVal + "')</script>");
				}if (columnName.equals("ALLCHARGE")) {
			out.print("<script language='javascript'>todo('ALLCHARGEyear','"
			+ cloumnVal + "')</script>");
				}
