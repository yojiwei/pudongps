...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr class="bttn">
		...
		<td width="19%" align="left">
			<select name="reportyear">
				...
				out.print("<option value=" + yea + ">" + yea + "</option>");
					for (int i = 1; i < 10; i++) {
						beginYear = yea - i;%>
						<option value="<%=beginYear%>" <%=getbeginYear.equals(beginYear+"")?"selected":""%>><%=beginYear%></option>
					<%}%>
			</select>
			&nbsp;年
			<select name="reportmonth">
				<%
					for (int i = 1; i < 13; i++) {%>
						<option value="<%=i%>" <%=getbeginMon.equals(i+"")?"selected":""%>><%=i%></option>
				<%}%>
			</select>
			&nbsp;月
		</td>
				...
				<%
					...
					if (vPage != null) {
						for (int i = 0; i < vPage.size(); i++) {
							...
							dt_id_l = content.get("dt_id").toString();
							%>
							<option value="<%=dt_id_l%>" <%=dt_id.equals(dt_id_l)?"selected":""%>><%=dt_name%></option>
				<%}}%>
			</select>
		</td>
		...
		<input class="bttn" value="打印" type="button"  id="button6" name="button6" onclick="print()">
		</td>
	</tr>
	</table>
	</form>
          ...
          <TD>
            ...
            cellPadding=1 width="100%" border=1>
              ...
              <TR class=tr_header height=20>
				...
				<TD vAlign=center align=middle width=20 rowSpan=2>“重复 申请”数</TD>
              ...
              <TR class=tr_header height=20>
                ...
                <TD vAlign=center align=middle width=20>“国家秘密”数</TD>
                <TD vAlign=center align=middle width=20>“商业秘密”数</TD>
                <TD vAlign=center align=middle width=20>“个人隐私”数</TD>
                <TD vAlign=center align=middle width=20>“过程中信息且影响安全稳定”数</TD>
                <TD vAlign=center align=middle width=20>“危及安全和稳定”数</TD>
                <TD vAlign=center align=middle width=20>“法律法规规定的其他情形”数</TD>
...
<%
	//统计数据库中数据的条数
		...
		+ "sum(apartopen) as apartopen,sum(noinfo) as noinfo,sum(nodept) as nodept,sum(nobound) as nobound,sum(foujuegongkai) as foujuegongkai,"
...
//for循环开始	
			...
			shenzong += cloumnVal;
				...
				if ((columnName+""+u).equals("FOUJUEGONGKAI"+u+"")) {
