...
if(intStatus==8)
 {
  //str_sql="select c.cw_id,c.cp_id,c.cw_appliedname,c.cw_applieddept,c.cw_subject,c.cw_isovertime,c.cw_isovertimem,to_char(c.cw_managetime,'yyyy-mm-dd hh24:mi:ss') cw_managetime,to_char(c.cw_applytime,'yyyy-mm-dd hh24:mi:ss') cw_applytime,to_char(c.cw_finishtime,'yyyy-mm-dd hh24:mi:ss') cw_finishtime,p.cp_timelimit,p.cp_timelimiting,q.dt_id,q.pc_timelimit from tb_connwork c,tb_deptinfo d,tb_connproc p,tb_connproccorr q where c.cp_id=p.cp_id and (p.cp_id='o4'or p.cp_upid='o4') and p.cp_id=q.cp_id and c.cw_status=" + cw_status;
  str_sql="select c.cw_id,c.cp_id,c.cw_appliedname,c.cw_applieddept,c.cw_subject,c.cw_isovertime,c.cw_isovertimem,to_char(c.cw_managetime,'yyyy-mm-dd hh24:mi:ss') cw_managetime,to_char(c.cw_applytime,'yyyy-mm-dd hh24:mi:ss') cw_applytime,to_char(c.cw_finishtime,'yyyy-mm-dd hh24:mi:ss') cw_finishtime,p.cp_timelimit,p.cp_timelimiting,p.dt_id from tb_connwork c,tb_deptinfo d,tb_connproc p where c.cp_id=p.cp_id and (p.cp_id='o4'or p.cp_upid='o4') and c.cw_status=" + cw_status;
 }
else
	{
  str_sql="select c.cw_id,c.cp_id,c.cw_appliedname,c.cw_applieddept,c.cw_subject,c.cw_isovertime,c.cw_isovertimem,to_char(c.cw_managetime,'yyyy-mm-dd hh24:mi:ss') cw_managetime,to_char(c.cw_applytime,'yyyy-mm-dd hh24:mi:ss') cw_applytime,to_char(c.cw_finishtime,'yyyy-mm-dd hh24:mi:ss') cw_finishtime,p.cp_timelimit,p.cp_timelimiting from tb_connwork c,tb_deptinfo d,tb_connproc p where c.cp_id=p.cp_id and (p.cp_id='o4'or p.cp_upid='o4') and c.cw_status=" + cw_status;
  if(intStatus == 3){
  	str_sql="select c.cw_id,c.cp_id,c.cw_appliedname,c.cw_applieddept,c.cw_subject,c.cw_isovertime,c.cw_isovertimem,to_char(c.cw_managetime,'yyyy-mm-dd hh24:mi:ss') cw_managetime,to_char(c.cw_applytime,'yyyy-mm-dd hh24:mi:ss') cw_applytime,to_char(c.cw_finishtime,'yyyy-mm-dd hh24:mi:ss') cw_finishtime,p.cp_timelimit,p.cp_timelimiting from tb_connwork c,tb_deptinfo d,tb_connproc p where c.cp_id=p.cp_id and (p.cp_id='o4'or p.cp_upid='o4')  and (c.cw_status ='3' or cw_status='18')";
  }
  }
...
<tr class="bttn">                       
<td width="15%" class="outset-table">被投诉人</td>
<td width="15%" class="outset-table">被投诉单位</td>                     
...
<td width="11%" class="outset-table">发送时间</td>
  <%
   ...
   if(intStatus == 1 ) 
	  out.print("<td width='6%' class='outset-table'>受理时限</td>");                        
   if( intStatus == 2 || intStatus == 8) 
	  out.print("<td width='6%' class='outset-table'>办理时限</td>");	
	%>                        						
<td width="6%" class="outset-table" nowrap>受理超时</td>
<td width="6%" class="outset-table" nowrap>办理超时</td>						
...
if(vectorPage!=null)
{
	      ...
	      if(intStatus==8)
	      {
	        ...
	        if(!deal_dtid.equals(""))
	        {
	         ...
	         if(content_deal!=null){
	         		deal_name = CTools.dealNull(content_deal.get("dt_name"));
	         }
	        }
	      }
      }
  ...
  %>  
		 		 <td align="center"><%=CTools.dealNull(content.get("cw_appliedname"))%></td>
         <td align="center"><%=CTools.dealNull(content.get("cw_applieddept"))%></td>             
         <td align="center"><%=CTools.dealNull(content.get("cw_subject")) %></td>
         <td align="center"><%=CTools.dealNull(content.get("cw_applytime"))%></td>
          <%
            if(intStatus==3)
            out.print("<td  align=\"center\">" + CTools.dealNull(content.get("cw_finishtime")) + "</td>");		    
            if(intStatus==8)
          ...
          <%if (intStatus == 1) 
          		out.println(CTools.dealNull(content.get("cp_timelimit")));
          	else if (intStatus == 2) 
          		out.println(CTools.dealNull(content.get("cp_timelimiting")));
          	else 
          		out.println(15);
          %> 天</td>
          <%}
    ...
    <td width="5%" align="center">
    	...
    	<img class="hand" border="0" src="../../images/modi.gif" title="<%=strAct%>" WIDTH="16" HEIGHT="16">
    	</a></td>
	<%
    }
  %>
