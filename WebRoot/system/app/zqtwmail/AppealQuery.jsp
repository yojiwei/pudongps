...
us_uid = String.valueOf(mySelf.getMyUid());
%>
...
<form name="formData" method="post" action="QueryList.jsp">
	...
	<%
	if(us_uid.equals("administrator")){
	%>
	<tr class="line-even">
		<td width="25%" align="right">部门</td>
		<td align="left">
		<select name="cp_id" style="width:160px">
  		<option value="">选择相关部门</option>
  		<%
  		String deptSql = "";
  		String sdt_id = "";
  		String sdt_name = "";
  		...
  		deptSql = "select cp_id,cp_name,dt_id from tb_connproc where cp_upid = 'o11958'";
  		dept_list = dImpl.splitPageOpt(deptSql,500,1);
  		if (dept_list != null) {
  		  for (int i=0;i<dept_list.size();i++) {
  		  dept_content = (Hashtable)dept_list.get(i);
  		  sdt_id = dept_content.get("cp_id").toString();
  		  sdt_name = dept_content.get("cp_name").toString();
  		%>
	...
	<%}%>
	<tr class="line-odd">
		<td width="25%" align="right">事项状态</td>
		<td align="left">
