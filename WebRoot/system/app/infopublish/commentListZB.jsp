...
String subIds = "";
strSql = "select cc_id,cc_us_ip,cc_us_name,cc_publish_flag,cc_create_time,cc_content,cc_answer_time,cc_answer from tb_contentcomment where " + sWhere + " cc_corid_name = 'ct_id' and cc_corid_value = '"+ Ct_id +"' order by cc_id desc";
 ...
 <table class="main-table" width="100%">
  ...
  <td width="100%">
     ...
     <INPUT TYPE="hidden" name="curpage" value="<%=CTools.dealNumber(request.getParameter("strPage"))%>">
        ...
        <tr class="bttn">
            <td width="40%" class="outset-table">评论内容</td>
            <td width="10%" class="outset-table">评论者</td>
            <td width="8%" class="outset-table">状态</td>
            <td width="12%" class="outset-table">评论日期</td>
            <td width="10%" class="outset-table">评论人IP</td>
            <td width="12%" class="outset-table">答复时间</td>
...
%>
                ...
                <td ><img border="0" src="/system/images/arrow_yellow.gif" WIDTH="15" HEIGHT="13">&nbsp;&nbsp;<a href="commentInfoZB.jsp?cc_id=<%=cc_id%>&ct_id=<%=Ct_id%>" title="点击答复"><%=cc_content%></a></td>
                <td align="center" ><%=cc_us_name%></td>
                <td align="center" ><%if (cc_publish_flag.equals("0")) {out.println("<font color='#FF0000'>待审核</font>");} else {if (cc_answer_time.equals("")) {out.println("<font color='#0000FF'>待回复</font>");} else {out.println("已回复");}}%></td>
                <td align="center"><%=cc_create_time%></td>
                <td align=center><%=cc_us_ip%></td>
                <td align=center><%=cc_answer_time%></td>
                <td align="center" nowrap>
								<a href="commentInfoZB.jsp?cc_id=<%=cc_id%>&ct_id=<%=Ct_id%>">
