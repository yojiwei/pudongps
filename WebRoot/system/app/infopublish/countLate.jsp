...
<form name="formData" method="post" action="updatecounttList.jsp"  target="">
</form>
		...
		for (int i = 0;i < vPage.size();i++) {
			...
			if (sjDir.length > num) {
				for (int k = 0;k < sjDir.length;k++) {
					sun_dir += sjDir[k] + ",";
					if (k % num == 0) {
						chg_sj_dir += " or tb_url in (" + sun_dir.substring(0,sun_dir.length() - 1) + ")";
						sun_dir = "";
					}
				}
				...
				chg_sj_dir = " or tb_url in (" + sj_dir.substring(0,sj_dir.length() - 1) + ")";
			}
			
			count_sql = "select tb_count from tb_countNews where " + chg_sj_dir.substring(3) + " and tb_type = 'count'";
			//out.println(count_sql);
			rs = dImpl.executeQuery(count_sql);
			while (rs.next()) {
				tb_count += Integer.parseInt(rs.getString("tb_count"));
			}
			rs.close();

			tb_name = content.get("sj_name").toString();
			f_sj_dir = "'" + f_sj_dir + "',";
			hrefUrl = f_sj_dir.equals(sj_dir) ? tb_name : "<a href=\"countLate.jsp?sj_id=" + sj_id + "\"> " + tb_name + "</a>";
			if (tb_count != 0) {
				kk++;
  %>
