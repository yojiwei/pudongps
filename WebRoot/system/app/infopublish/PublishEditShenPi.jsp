...
<%
  ...
  //update20080122
  //--------edit by yanker 20081104----------start------------------
  String urID = "";
  String dtID = "";
  Vector sjNameVec = null;
  //--------edit by yanker 20081104----------end--------------------  
...
try {
 ...
 if(mySelf != null){ 
 	urID = String.valueOf(mySelf.getMyID());
  	dtID = String.valueOf(mySelf.getDtId());
 }
  ...
  {
    String sqlStr = "select c.*,ct_content from tb_content c,tb_contentdetail d where d.ct_id=c.ct_id and c.ct_id='" + ctId + "'";
    //out.println(sqlStr);
    ...
    if(content!=null)
    {
      ...
      ct_contentflag = content.get("ct_contentflag").toString();
	   ...
	   //取sj_name--------------------------------------start--------20081104------------------
	   if("".equals(sjName)){
		   sqlStr = "select sj_name from tb_subject where sj_id in ("+ CTools.trimEx(sjId,",") +")";
		   sjNameVec = dImpl.splitPage(sqlStr,20,1);
		   if(sjNameVec != null)
		   {
		   	   for(int cnt = 0; cnt < sjNameVec.size(); cnt++){
		   	   	   content = (Hashtable)sjNameVec.get(cnt);
		   	   	   sjName = (!"".equals(sjName)) ? sjName + "," + CTools.dealNull(content.get("sj_name")) : CTools.dealNull(content.get("sj_name"));
		   	   }
		   }
		}
	   //取sj_name--------------------------------------end----------20081104------------------	  
...
<%
        ...
        <span  id="TdInfo1">
         	...
         	<%
         		String sql_bk = "select * from tb_newslog where ct_id = " + ctId + " order by ct_id desc";
         		Vector vPage_bk = dImpl.splitPage(sql_bk,100,1);
         		if (vPage_bk != null) {
        	%>
     <tr class="line-even">
         <td width="19%" colspan=2 align="left"><a onClick="javascript:Display(1)" style="cursor:hand"><img border="0" src="/system/images/<%=OPType.equals("ShenHeBack") ? "topminus.gif" : "topplus.gif"%>" WIDTH="16" HEIGHT="16" id="InfoImg1"></a>
     	 审批意见：</td>
     </tr>
     <tr id=Info1 style="display:<%=OPType.equals("ShenHeBack") ? "" : "none"%>">
       <td colspan="2">
         <table cellspacing="1" border="0" bgcolor="#ffffff" width="100%">
        	<%
         		for (int kk = 0;kk < vPage_bk.size();kk++) {
         			Hashtable content_bk = (Hashtable)vPage_bk.get(kk);
         			String ns_meno = content_bk.get("ns_meno").toString();
         			String ns_date = content_bk.get("ns_date").toString();
         			String ns_userName = content_bk.get("username").toString();
         			String ns_userId = content_bk.get("userid").toString();
         	%>
		    <tr class="<%=kk%2==0 ? "line-odd" : "line-even"%>">
				 <td width="19%" align="right"></td>
				 <td width="81%" align="left">
				 	<table cellspacing="0" cellpadding="0" border="0" width="100%">
				 	  <tr>
				 	    <td width="72%">
				 	    	<textarea name="sp_opion" cols="60" rows="5" readOnly><%=ns_meno%></textarea>
				 	    </td>
...
<script language='javascript'>
  ...
  function Display(Num)
  {
        ...
        var objImg=eval("document.all.InfoImg"+Num);
