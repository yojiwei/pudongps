...
document.getElementById("mess").innerHTML="您已经超出了";  
document.getElementById("age").innerHTML="  超出部分将分条收费！";    
//field.value   =   field.value.substring(0,   maxlimit);
}
else   
//在记数区文本框内显示剩余的字符数；
document.getElementById("mess").innerHTML="您还可以输入";     
countfield.value   =   maxlimit   -   field.value.length;   
}
function pp(field,countfield,maxlimit){ 
var clob = "";
}
setInterval("textCounter(document.all.content1,document.all.message,55)", 1);
//-->   
</SCRIPT> 
...
<%
  ...
  Hashtable content_ym = new Hashtable();
  ...
  String smm_id=CTools.dealString(request.getParameter("sm_id"));
  String sm_ct_id = "";
...
//**********************************************************开始******************************************************
  String mysql="select sm_ct_id from tb_sms where sm_id="+smm_id+" and sm_tel is null";
  String yousql="";
  String mmt_id="";
  String mmt_con="";
  String mmt_sendtime="";
  ...
  String mmt_sj_id ="";
  String mmt_sj_name = "";
  Hashtable dt_content1 = dImpl.getDataInfo(mysql);
  if (dt_content1 != null);
  sm_ct_id = dt_content1.get("sm_ct_id").toString();
  if(!"".equals(sm_ct_id))
  {
  	 yousql="select s.sm_id,s.sm_con,s.sm_sj_id,s.sm_sendtime,p.ct_content as ct_content,j.sj_name from tb_sms s,tb_contentdetail p,tb_subject j ";
	 yousql+=" where s.sm_ct_id = p.ct_id and s.sm_sj_id=j.sj_id and s.sm_ct_id="+sm_ct_id+"";
  }
  else
  {
  	 yousql="select s.sm_id,s.sm_con,s.sm_sj_id,s.sm_sendtime,s.sm_detail as ct_content,j.sj_name  from tb_sms s,";
	 yousql+="tb_subject j where s.sm_sj_id=j.sj_id and s.sm_id="+smm_id+"";
  }
    //out.println(sm_ct_id+"====="+yousql);
    content_ym = dImpl.getDataInfo(yousql);
    if(content_ym!=null)
    {
      mmt_id=content_ym.get("sm_id").toString() ;
	  mmt_con=content_ym.get("sm_con").toString() ;
	  mmt_sendtime=content_ym.get("sm_sendtime").toString() ;
	  mmt_content=content_ym.get("ct_content").toString() ;
	 // mmt_content=new String(mmt_content1.getBytes("ISO-8859-1"),"gbk");
	  mmt_sj_name=content_ym.get("sj_name").toString() ;
	  mmt_sj_id=content_ym.get("sm_sj_id").toString();
	 }
					...
					<td width="100%">
						<table width="100%" height="1">
							...
							<tr class="line-even">
								...
								<td width="81%" nowrap align='left'>
									...
									<input type="text" size=40 name="Module" class="text-line"
										treeType="Subject" value="<%=mmt_sj_name%>," treeTitle="选择所属栏目"
									...
									<input type=button title="选择所属栏目"
										onClick="chooseTree('Module');formData.Module.value=formData.Module.value.replace(/\x02/ig,',');"
										class="bttn" value=选择...>
									<input type="hidden" name="ModuleDirIds" value="<%=mmt_sj_id%>">
							...
							<tr class="line-even">
								...
								<td width="81%" nowrap align='left'>
								<span id=mess></span><input name="message" value="55" type="text" size="4" readonly/>个字！<span id="age" class="STYLE1"></span></td>
							</tr>
							<tr class="line-even">
								<td align="left" height="20" colspan=2>
									<textarea name="content1" cols="55" style="WIDTH: 100%; HEIGHT: 80px" onbeforepaste="pp(this.form.content1,this.form.message,55)"><%=mmt_con%></textarea>
								</td>
							</tr>
							<tr class="line-even">
								<td align="center" height="20" colspan=2>短信的详细内容(<span class="STYLE1">*可以在前台页面点击显示</span>)</td>
								<input type="hidden" name="sm_id" value="<%=smm_id%>"/>
								<input type="hidden" name="sm_ct_id" value="<%=sm_ct_id%>"/>
							</tr>
							<tr class="line-even">
								<td align="center" height="20" colspan=2>
                <textarea id="content" name="CT_content" style="display:none;WIDTH: 100%; HEIGHT: 400px"><%=mmt_content%></textarea>
