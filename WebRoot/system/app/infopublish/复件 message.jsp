...
isadmin = CTools.dealString(request.getParameter("isadmin"));
...
Vector vectorPage =dImpl.splitPage(strSql,request,18); 
%>
...
<%
  if(vectorPage!=null)
  {
    for(int j=0;j<vectorPage.size();j++)
    {
	  ...
	  String aa="";
	  if(content.get("sm_detail").toString().equals("")){
		aa="1";
	  }else{
		aa="2";
	  }
...
%>
<td >
	<img border="0" src="/system/images/arrow_yellow.gif" WIDTH="15" HEIGHT="13">&nbsp;<%=content.get("sm_con")%></td>
		<td align="center"><%=content.get("dt_name")%></td>
		<td align="center"><%=content.get("sj_name")%></td>
		<td align="center"><%=content.get("sm_sendtime")%></td>
		<td align="center"><img class="hand" border="0" src="images/dialog/modi.gif" WIDTH="16" HEIGHT="16" onclick="editMessage(<%=content.get("sm_id")%>,<%=aa%>)" alt="编辑"></td>
		<%if(!status.equals("2")){%>
		<td align="center">
		...
		%>
		<td>
		<a href="SelectList.jsp?sm_id=<%=CTools.dealNumber(content.get("sm_id"))%>&sm_sj_id=<%=content.get("sm_sj_id")%>">查看发送情况</a>
		</td>
		<%}%>
		</tr>
<%
