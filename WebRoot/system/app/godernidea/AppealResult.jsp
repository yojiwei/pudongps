...
//out.println(receiverId);
  ...
  if(!deptconn.equals(""))
  {
  	...
  	dImpl_conn.setValue("cw_applyingname",CTools.dealString(request.getParameter("applyname").trim()),CDataImpl.STRING);//投诉人姓名
        ...
        dImpl_conn.setValue("cw_applytime",FinishTime.toLocaleString(),CDataImpl.DATE); //投诉时间
		...
		dImpl_conn.setValue("cw_subject","金点子信箱:"+CTools.dealString(request.getParameter("strSubject")),CDataImpl.STRING);//投诉主题
        dImpl_conn.setValue("cw_code",CTools.dealString(request.getParameter("cp_id").trim()),CDataImpl.STRING);//投诉主题
		dImpl_conn.setValue("cw_transmittime",FinishTime.toLocaleString(),CDataImpl.DATE);//部门转发时间
  ...
  if (strStatus.equals("2"))dImpl.setValue("cw_managetime",FinishTime.toLocaleString(),CDataImpl.DATE); //开始处理时间
