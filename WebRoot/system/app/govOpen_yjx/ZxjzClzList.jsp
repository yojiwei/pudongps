...
//
 str_sql="select c.cw_id,c.cp_id,c.cw_applyingname,c.cw_applyingdept,c.cw_subject,to_char(c.cw_managetime,'yyyy-mm-dd hh24:mi:ss') cw_managetime,c.cw_isovertime,c.cw_isovertimem,to_char(c.cw_applytime,'yyyy-mm-dd hh24:mi:ss') cw_applytime,to_char(c.cw_finishtime,'yyyy-mm-dd hh24:mi:ss') cw_finishtime,p.cp_timelimit,p.cp_timelimiting,d.dt_id from tb_connwork c,tb_deptinfo d,tb_connproc p where p.cp_id in((select c.cp_id from tb_connproc c,tb_deptinfo d where c.cp_id <> 'o11893' and c.cp_id <> 'o11890' and c.cp_id <> 'o11835' and c.dt_id=d.dt_id and (c.cp_upid='o7' or c.cp_upid = 'o10000') union (select cp_id from tb_connproc where cp_upid = 'o11919'))) and c.cp_id=p.cp_id and c.cw_status=2 and ROUND(TO_NUMBER(sysdate - cw_applytime))>60 ";
...
<!-- 记录日志 -->
<%
String logstrMenu = "网上超期处理信件（2个月没办理）";
