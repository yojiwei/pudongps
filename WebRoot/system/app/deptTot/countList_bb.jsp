...
try {
 ...
 dImpl = new CDataImpl(dCn); 
	...
	if(!"".equals(begtime)) {
		sqlWhere += " and to_date(to_char(OPERTIME,'YYYY-MM-DD hh:mi:ss'),'YYYY-MM-DD hh:mi:ss')>=to_date('" + begtime + " 00:00:01','YYYY-MM-DD hh24:mi:ss')";
	}
	if(!"".equals(endtime)) {
		sqlWhere += " and to_date(to_char(OPERTIME,'YYYY-MM-DD hh:mi:ss'),'YYYY-MM-DD hh:mi:ss')<=to_date('" + endtime + " 23:59:59','YYYY-MM-DD hh24:mi:ss')";
	}
	...
	if (!"".equals(sqlWhere1)) 
		sql = "select d.dt_id,d.dt_name,sum(s.addnum)as addCount,sum(s.pubnum)as pubCount,sum(s.score) as score from" + 
 		  	  "(select dt_id,dt_name from tb_deptinfo where dt_id in (" + sqlWhere1 + "))  d left join (select addnum,pubnum,score,deptid from tb_infostatic where 1 = 1 and infoid in (select ct_id from tb_contentpublish where sj_id = 371) " + sqlWhere + ") s on d.dt_id = s.deptid where 1 = 1 group by d.dt_id,d.dt_name";
		/*
