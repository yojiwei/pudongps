...
<table width="100%">
		...
		<td>
			<table width="100%">
				<tr class="bttn">
					...
					<td width="10%" <%=us_uid.equals("admin") ? "" : "style='display:none'"%> nowrap class="outset-table" ><div align="center">修改</div></td>
				</tr>
			</table>
		</td>
	<tr>
	...
	%>
	<tr class="line-odd">
		<td align="center" ><b><%if("admin".equals(us_uid)){%><a href ="sortEdit.jsp?OPType=edit&sort_id=<%=sort_id%>"><%=sort_name%></a><%}else{%><%=sort_name%><%}%></b></td>
		<td>
			<table width="100%">
				<tr>
				<%
				if("admin".equals(us_uid)){
				...
				%>
					...
					<td width="10%" align=center <%=us_uid.equals("admin") ? "" : "style='display:none'"%>><div align="center"><img style="cursor:hand;" src='images/modi.gif' onclick="window.location='rework.jsp?sort_id=<%=sort_id%>&board_id=<%=board_id%>&us_uid=<%=us_uid%>&board_master_id=<%=board_master_id%>'"  width="16" height="16" border='0' title='修改' <%=us_uid.equals("admin") ? "" : "disabled"%>></div></td>
				<%
					}
				}else{
