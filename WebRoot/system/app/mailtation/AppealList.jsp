...
if(intStatus==8)
 {
  str_sql="select c.cw_id,c.cp_id,c.cw_applyingname,c.cw_applyingdept,c.cw_subject,c.cw_isovertime,to_char(c.cw_managetime,'yyyy-mm-dd hh24:mi:ss') cw_managetime,c.cw_isovertimem,to_char(c.cw_applytime,'yyyy-mm-dd hh24:mi:ss') cw_applytime,to_char(c.cw_finishtime,'yyyy-mm-dd hh24:mi:ss') cw_finishtime,p.cp_timelimit,q.dt_id from tb_connwork c,tb_deptinfo d,tb_connproc p,tb_connproccorr q where  p.cp_upid='o10000' and c.cp_id=p.cp_id and p.cp_id=q.cp_id and c.cw_status=" + cw_status;
 }
else
 str_sql="select c.cw_id,c.cp_id,c.cw_applyingname,c.cw_applyingdept,c.cw_subject,c.cw_isovertime,to_char(c.cw_managetime,'yyyy-mm-dd hh24:mi:ss') cw_managetime,c.cw_isovertimem,to_char(c.cw_applytime,'yyyy-mm-dd hh24:mi:ss') cw_applytime,to_char(c.cw_finishtime,'yyyy-mm-dd hh24:mi:ss') cw_finishtime,p.cp_timelimit,p.cp_timelimiting from tb_connwork c,tb_deptinfo d,tb_connproc p where p.cp_upid='o10000' and c.cp_id=p.cp_id and c.cw_status=" + cw_status;
...
<table class="FindAreaTable1" cellspacing="1" rules="cols" border="0" id="ctl00_DataGrid_DataGrid1" style="border-width:0px;">
                ...
                <tr class="bttn">                       
                        ...
                        <%
						              if(intStatus==3)
                          out.print("<td width=\"11%\" class=\"outset-table\">办理完成时间</td>");
                         if(intStatus==8)
                          out.print("<td width=\"11%\" class=\"outset-table\">转办单位</td>");
                         if(intStatus == 1 ) 
						...
						%>						
						<td width="11%" class="outset-table">受理超时</td>
						<td width="11%" class="outset-table">办理超时</td>						
