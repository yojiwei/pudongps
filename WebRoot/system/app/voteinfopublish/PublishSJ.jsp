...
%>
<script LANGUAGE="javascript" src="NewtreeJs.jsp"></script>
...
function on_choose()
{
	...
	//alert(formData.deptIds.value);
		...
		//alert(strPowerID);
	sValue = formData.deptIds.value.split(","); //sjid的值
	sname = formData.content.value.split(",");
	j=0
    ...
    if(sValue[i]!="") //过滤为空的记录
    {
	  ...
	  //alert(sValue[i]);
	  //alert(sname[j])
	  if(strPowerID.indexOf("," + sValue[i] +",")!=-1)
      {
        returnValue1 += sValue[i] + ",";
      }
      else
      {
        alert("你没有栏目权限，请重新选择")
	 ...
	 //alert(returnValue1);
    returnValue=returnValue1 + sep2 + formData.content.value;
	...
	try
		{
			...
			objhttp.Open("post","CreatChildTree.jsp",false);
	...
	if (img != 3)
	{
		...
		//alert(chooseValue.value);
			...
			if (img == 0 || img == 1) //要添加
			{
					...
					if (chooseIds.value.search(","+list_id+",")=="-1")
					   {
                         ...
                         var value2=","+chooseValue.value;
                         
                         //if(value2.indexOf("," + node_title + ",")=="-1")
                        // {
                         	//alert(chooseValue.value)
                           //alert(node_title);
				...
				{
					//alert(re);
					//alert(chooseIds.value);
					//var tmpchooseIds = "," + chooseIds.value;

					chooseIds.value   = chooseIds.value.replace(re,",");
					eval("var re = /"+sp1+node_title+sp1+"/;");
					//var dd =chooseValue.value; 
					//alert();
					//alert(re);
