...
sqlstr="select c.*,d.ct_content ,d.cd_id,r.th_powercode,r.cn_id from tb_content c,tb_contentdetail d ,tb_relate r where r.ct_id=c.ct_id and d.ct_id=c.ct_id and c.ct_id=" + ct_id;
	...
	if (content!=null){
		...
		th_powercode1 = content.get("th_powercode").toString();
		cn_id = content.get("cn_id").toString();
		cd_id = content.get("cd_id").toString();
		if(!ctImgpath.equals("")) 
		session.setAttribute("temppath",ctImgpath);
	}
}
%>
...
<html>
	...
	<%
			...
			for(int i=0; i<vPage.size(); i++){
				...
				if(th_powercode1.equals(fs_code)){
	 %>
							 ...
							 if (!filePath.equals(""))
							 {
								...
								if (vPage_ia!=null)
								{
									for(int n=0;n<vPage_ia.size();n++)
									{
										...
										fileName = CTools.dealNull(content_ia.get("im_filename"));
										int imgflag = 0;
										%>
										<p align="left">文件名称：&nbsp;&nbsp;&nbsp;&nbsp;<a href="download.jsp?ia_id=<%=ia_id%>"><%if(!ia_name.equals("")) {out.println(ia_name);} else{out.println(fileName);}%></a>&nbsp;&nbsp;
										<img SRC="../images/dialog/delete.gif" onclick="javascript:deleteFile('<%=ia_name%>');" title='删除该文件'style="cursor:hand">
										</p>
										<%
									}
								}
	...
	function deleteFile(fileName)
{
	...
	{
		var obj = formData.ct_id;
		var thid = formData.th_id;
		var url = "attachDel.jsp?fileName="+fileName;
		url += "&in_id="+obj.value+"&th_id="+thid.value;
		window.location = url;
	}
}
</script>
