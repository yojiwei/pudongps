...
if(!messages.equals(" 详见“上海浦东”"))
{
	...
	dImpl.setValue("sm_flag","2", CDataImpl.STRING);// 发送与否的标志0发送失败1发送成功2没有发送3正在发送中
	dImpl.setValue("sm_flagtoo","10", CDataImpl.STRING);//发送的优先级别1发送验证码快10发送短信内容
	dImpl.setValue("sm_check","1",CDataImpl.STRING);//1待审核2通过3审核没通过
	...
	dImpl.setValue("sj_id", sj_id, CDataImpl.STRING);//发送的优先级别1发送验证码快10发送短信内容
...
dCn.closeCn();
%>
<script language='javascript'>window.location.href='dxpdList.jsp?ct_id=<%=ct_id%>';</script>
  </body>
</html>