...
<%
  ...
  String mytitle = CTools.dealString(request.getParameter("title"));
 //update20080122
...
/*String mysql="select * from subscibesetting s where s.username='"+us_name+"'";
contentmy =(Hashtable) dImpl.getDataInfo(mysql);
if(contentmy!=null)
{
   subscibeid=contentmy.get("id").toString();
						...
						<table width="100%" height="1">
							...
							<tr class="line-even">
								...
								<td width="81%" nowrap align='left'>
									...
									<input type="hidden" name="sjName" class="text-line"
										...
										String moduu=(String)session.getAttribute("Module_name");
										 if(moduu!=null)
										 {
										 	Module_name=moduu;
										 }
										 
											String xxSql = "select sj_id from tb_subject where sj_dir='dxpd'";
											Hashtable contentsh = dImpl.getDataInfo(xxSql);
											if (contentsh != null) {
												myidid = contentsh.get("sj_id").toString();
											}
										 
										 %>
									<input type="text" size=40 name="Module" class="text-line"
										treeType="Subject" value="<%=Module_name%>" treeTitle="选择所属栏目"
										readonly isSupportMultiSelect="-1" isSupportFile="0" parentId="<%=myidid%>"
							...
							<tr class="line-even">
								<td align="left" height="20" colspan=2>
									<textarea name="content1" style="WIDTH: 100%; HEIGHT: 80px"><%=mytitle%></textarea>
								</td>
							</tr>
							<tr class="line-even">
								<td align="center" height="20" colspan=2>
									<input type="submit" value="保存" class="bttn" onClick="save()"/>
									<input type="submit" value="保存并发送" class="bttn" onClick="saveAndsend()"/>
