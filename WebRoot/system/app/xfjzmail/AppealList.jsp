...
if (!"".equals(mailkind_get)) {
	sqlWhere = " and cw_subject = '"+mailkind_get+"' ";
}
...
if(intStatus==8)
 {
  str_sql="select c.cw_id,c.cp_id,c.cw_applyingname,c.cw_applyingdept,c.cw_trans_id,p.dt_id,c.cw_subject,to_char(c.cw_managetime,'yyyy-mm-dd hh24:mi:ss') cw_managetime,c.cw_isovertime,c.cw_isovertimem,to_char(c.cw_applytime,'yyyy-mm-dd hh24:mi:ss') cw_applytime,to_char(c.cw_finishtime,'yyyy-mm-dd hh24:mi:ss') cw_finishtime,p.cp_timelimit,p.cp_timelimiting from tb_connwork c,tb_deptinfo d,tb_connproc p where  p.cp_id='o11100' and c.cp_id=p.cp_id and c.cw_status=" + cw_status + sqlWhere;
 }
else
...
<!--    功能按钮开始    -->
  <img src="../../images/menu_changedept.gif " border="0" onclick="GetAllSelect();" title="批处理转办" style="cursor:hand" align="absmiddle">批处理转办
            ...
            <%
             ...
             if(intStatus==8)
              out.print("<td width=\"16%\" class=\"outset-table\">转办单位</td>");
						...
						if(intStatus == 1 ) 
						{
						out.print("<td width='3%' class='outset-table'><input type='checkbox'   onclick='javascript:SelectAllCheck(\"checkbox1\")'></td>");
						}
						%>
...
if(vectorPage!=null)
{
  ...
  %>
		  ...
		  <%if(intStatus == 1 ) 
				out.print(" <td width='3%' class='outset-table'align='center'><input name='checkbox1' type='checkbox' value='"+CW_ID+"'</td>");
			%>
...
function GetAllSelect(){
	...
	else
	{
		...
		document.formData.action="CorrTypenew.jsp";
