...
<%
 // java.util.Date FinishTime = new java.util.Date();
  ...
  CDataImpl dImpl_source = null; //新建数据接口对象
  try{
  	dCn = new CDataCn();
    dImpl = new CDataImpl(dCn);
    dImpl_conn = new CDataImpl(dCn);
    dImpl_source = new CDataImpl(dCn);
  ...
  String cw_emailtype = request.getParameter("cw_emailtype").toString();//信箱类型
...
if(!receiverId.equals("")&&!strStatus.equals("8")){
...
//out.println(receiverId);
  ...
  if(!deptconn.equals(""))
  {
  	dImpl_conn.addNew("tb_connwork","cw_id",CDataImpl.PRIMARY_KEY_IS_VARCHAR);//新增
  	dImpl_conn.setValue("cp_id",deptconn,CDataImpl.STRING);//事项id
  	dImpl_conn.setValue("cw_applyingname",CTools.dealString(request.getParameter("applyname").trim()),CDataImpl.STRING);//投诉人姓名
        dImpl_conn.setValue("cw_applyingdept",CTools.dealString(request.getParameter("applydept").trim()),CDataImpl.STRING);//投诉人单位
        ...
        dImpl_conn.setValue("cw_telcode",CTools.dealString(request.getParameter("strTel").trim()),CDataImpl.STRING);
        dImpl_conn.setValue("cw_subject",CTools.dealString(request.getParameter("strSubject").trim()),CDataImpl.STRING);//投诉主题
		//dImpl_conn.setValue("cw_subject",,CDataImpl.STRING);//投诉主题
		...
		dImpl_conn.setValue("dd_id",CTools.dealString(request.getParameter("dd_id").trim()),CDataImpl.STRING);
		dImpl_conn.setValue("cw_ispublish",CTools.dealString(request.getParameter("cw_ispublish").trim()),CDataImpl.STRING);
        dImpl_conn.update();
