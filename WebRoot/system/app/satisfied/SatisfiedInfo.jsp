...
dImpl = new CDataImpl(dCn);
    ...
    String cp_id = "";//项目编号
	String cp_name = "";
    String applyname = "";//咨询人姓名
    String applydept = "";//咨询人单位
    String strTel = "";//咨询人电话
    String strEmail = "";//咨询人邮件
    String appealname = "";//被咨询人姓名
    String appealdept = "";//被咨询人单位
    String strSubject = "";//咨询主题
    String strContent = "";//咨询内容
    String strfeedback = "";//咨询反馈
    String feedbackType = "";//咨询反馈
    int cw_status=0;         //咨询状态
    String cw_statusStr = "";
    String strStatus = "";//咨询状态
    String strSql = "";
  ...
  if(vectorPage!=null)
  {
   ...
   strSql += "e.de_senddeptid,e.de_receiverdeptid,e.de_isovertime,e.de_requesttime,f.dt_name,'' as cp_name ";
  ...
  {
   strSql = "select c.cw_id,c.us_id,c.cw_typecode,c.cp_id,c.cw_applyingname,c.cw_applyingdept,c.cw_telcode,c.dd_id,c.cw_ispublish,c.cw_emailtype,c.cw_email,c.cw_appliedname,c.cw_parentid,c.cw_applieddept,c.cw_subject,c.cw_content,c.cw_feedback,p.cp_name as cp_name  from tb_connwork c,tb_connproc p where c.cw_id='"+ cw_id +"' and c.cp_id = p.cp_id ";
  }
...
<form method="post" name="sForm">
    ...
    <td width="100%" align="left" valign="top">
    	<table width="100%">
          ...
          <td width="82%"><input type="text" class="text-line" name="applyname" maxlength="100" size="30" title="咨询人" value="<%=applyname%>" readonly></td>
        </tr>
        <tr class="line-even">
        ...
        <tr class="line-odd">
          <td width="18%" align="right">被咨询人：</td>
          <td width="82%"><input type="text" class="text-line" name="appealname" maxlength="100" size="30" title="咨询人" value="<%=appealname%>"readonly></td>
        </tr>
        <tr class="line-even">
          <td width="18%" align="right">单　　位：</td>
		 ...
		 <tr class="line-even">
          <td width="18%" align="right">办理单位：</td>
          <td width="82%"><input type="text" class="text-line" name="cp_name" maxlength="100" size="30" value="<%=cp_name%>" readonly></td>
        </tr>
        <tr class="line-odd">
        ...
        <tr class="line-even">
          ...
          <td><textarea class="text-area" name="strContent" cols="60" rows="6" title="咨询内容" readonly><%=strContent%></textarea></td>
        </tr>
        <tr class="line-even">
        ...
        <tr class="line-odd" id="Info2" style="display:none">
          <td align="left" height="20" colspan=2><table cellspacing="1" border="0" bgcolor="#ffffff" width="100%">
              <tr class="line-odd">
                ...
                <td><textarea class="text-area" name="strFeedback" cols="60" rows="6" title="咨询反馈" <%=feedbackType%>><%=strfeedback%></textarea></td>
              </tr>
            </table></td>
        </tr>
		<!--用户反馈满意度调查-->
		<tr class="line-even" width="100%">
          <td width="100%" colspan="2" align="center"> <strong>用户反馈满意度调查</strong> </td>
        </tr>
		<%
		String satisSql = "select cs_satis,cs_timesatis,cs_resultsatis,cs_message from tb_connsatisfied where cw_id = '"+cw_id+"'";
		String cs_satis = "";
		String cs_timesatis = "";
		String cs_resultsatis = "";
		String cs_message = "";
		vPage = dImpl.splitPage(satisSql,request,20);
		if(vPage!=null)
		{
			for(int j=0;j<vPage.size();j++)
			{
				 content = (Hashtable)vPage.get(j);
				 cs_satis = CTools.dealNull(content.get("cs_satis"));
				 if("1".equals(cs_satis)){
				 	cs_satis = "满意";
				 }else if("2".equals(cs_satis)){
			    	cs_satis = "基本满意";
				 }else if("3".equals(cs_satis)){
					cs_satis = "不满意";			 
				 }
				 cs_timesatis = CTools.dealNull(content.get("cs_timesatis"));
				 if("1".equals(cs_timesatis)){
				 	cs_timesatis = "满意";
				 }else if("2".equals(cs_timesatis)){
			    	cs_timesatis = "基本满意";
				 }else if("3".equals(cs_timesatis)){
					cs_timesatis = "不满意";			 
				 }
				 cs_resultsatis = CTools.dealNull(content.get("cs_resultsatis"));
				 if("1".equals(cs_resultsatis)){
				 	cs_resultsatis = "满意";
				 }else if("2".equals(cs_resultsatis)){
			    	cs_resultsatis = "基本满意";
				 }else if("3".equals(cs_resultsatis)){
					cs_resultsatis = "不满意";			 
				 }
				 cs_message = CTools.dealNull(content.get("cs_message"));
		%>
		<tr class="line-odd" id="Info2">
          <td align="left" height="20" colspan=2><table cellspacing="1" border="0" bgcolor="#ffffff" width="100%">
              <tr class="line-odd">
                <td align="right" width="18%">对办理部门工作作风、工作态度的评价：</td>
                <td><strong><%=cs_satis%></strong></td>
              </tr>
			  <tr class="line-odd">
                <td align="right" width="18%">对信访事项办理时效的评价：</td>
                <td><strong><%=cs_timesatis%></strong></td>
              </tr>
			  <tr class="line-odd">
                <td align="right" width="18%">对信访事项办理结果的意见：</td>
                <td><strong><%=cs_resultsatis%></strong></td>
              </tr>
            </table></td>
        </tr>
		<tr class="line-odd" id="Info2">
          <td align="left" height="20" colspan=2><table cellspacing="1" border="0" bgcolor="#ffffff" width="100%">
              <tr class="line-odd">
                <td align="right" width="18%">用户建议和意见：</td>
                <td><textarea class="text-area" name="cs_message" readonly="readonly" cols="60" rows="6" title="用户建议和意见"><%=cs_message%></textarea></td>
              </tr>
            </table></td>
        </tr>
		<%
