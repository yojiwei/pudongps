<%@page contentType="text/html; charset=GBK"%>
<%@include file="/system/app/skin/head.jsp"%>
<%
String sqlStr = "";
String strTitle = "";
String userKind = "";
String question = "";
String sequence = "";
String qu_id = "";
String OPType = "";

userKind = CTools.dealString(request.getParameter("userKind")).trim();
question = CTools.dealString(request.getParameter("question")).trim();
OPType = CTools.dealString(request.getParameter("OPType")).trim();

if(OPType.equals("Search"))
{
	strTitle = "问题解答>>搜索结果";
}
else
{
	strTitle = "问题解答>>列表";
}

//update20080122

CDataCn dCn=null;   //新建数据库连接对象
CDataImpl dImpl=null;  //新建数据接口对象

try {
 dCn = new CDataCn(); 
 dImpl = new CDataImpl(dCn); 

sqlStr = "select a.qu_id,a.qu_title,a.qu_sequence,b.uk_name from tb_question a,tb_userkind b where a.us_kind=b.uk_id ";
sqlStr += "and a.qu_title like '%"+question+"%' ";
if (!userKind.equals(""))
{
	sqlStr += "and a.us_kind='"+userKind+"'";
}
sqlStr += " order by a.qu_sequence ";
%>
<SCRIPT LANGUAGE="JavaScript">
var sFlag = false;
function selectAll()
{
	var obj = formData.del;
	sFlag = !sFlag;
	if (typeof(obj)=="undefined") return false;
	if (typeof(obj.length)=="undefined")
	{
		obj.checked = sFlag;
	}
	else
	{
		var length = obj.length;
		for(var i=0;i<length;i++)
		{
			obj[i].checked = sFlag;
		}
	}
}

function delSelect()
{
	var obj = formData.del;
	var obj1 = formData.qu_id;
	if (typeof(obj)=="undefined") return false;
	if (typeof(obj.length)=="undefined")
	{
		if(obj.checked)
		{
			obj1.value = obj.value;
		}
	}
	else
	{
		var length = obj.length;
		var str = "";
		for(var i=0;i<length;i++)
		{
			if(obj[i].checked)
			{
				str += obj[i].value + ",";
			}
		}
		if (str.length>0)
		{
			str = str.substring(0,str.length);
		}
		obj1.value = str;
	}
	if (obj1.value!="")
	{
		if (!confirm("确定要删除您选择的记录吗？")) {
			return false;
		}
		formData.action = "QuestionDel.jsp";
		formData.submit();
	}
	else
	{
		alert("没有选择要删除的项！");
	}
}

function setSequence()
{
        var form = document.formData ;
        form.action = "setSequence.jsp";
        form.submit();
}
</SCRIPT>
<table class="main-table" width="100%">
<form name="formData" method="post">
	<tr class="title1" width="100%">
		<td colspan="9">
			<table cellspacing="0" border="0" width="100%">
				<tr>
					<td align="left" valign="middle">
						<%=strTitle%>
					</td>
					<td align="right" valign="middle">
						<img src="/system/images/split.gif" align="middle" border="0" WIDTH="5" HEIGHT="8">
						<img src="/system/images/delete.gif" border="0" width="16" height="16" onclick="delSelect()" style="cursor:hand" title="删除选中项">
						<img src="/system/images/split.gif" align="middle" border="0" WIDTH="5" HEIGHT="8">
						<input type="checkbox" id="selAll" name="selAll" onclick="selectAll()" style="cursor:hand">
						<label for="selAll" style="cursor:hand">全选</label>
						<img src="/system/images/split.gif" align="middle" border="0" WIDTH="5" HEIGHT="8">
						<img src="/system/images/new.gif" border="0" onclick="window.location='QuestionInfo.jsp'" title="新建特别推荐" style="cursor:hand" align="absmiddle" WIDTH="16" HEIGHT="16">
						<img src="/system/images/split.gif" align="middle" border="0" WIDTH="5" HEIGHT="8">
						<img src="/system/images/sort.gif" title="修改排序" border="0" style="cursor:hand" onclick="javascript:setSequence();" width="16",height="16" align="absmiddle">
						<img src="/system/images/split.gif" align="middle" border="0" WIDTH="5" HEIGHT="8">
						<img src="/system/images/goback.gif" border="0" onclick="javascript:history.back();" title="返回" style="cursor:hand" align="absmiddle">
						<img src="/system/images/split.gif" align="middle" border="0" WIDTH="5" HEIGHT="8">
					</td>
				</tr>
			</table>
		</td>
	</tr>
	<tr class="bttn" width="100%">
		<td align="center" class="outset-table" width="10%">ID</td>
		<td align="center" class="outset-table" width="15%">用户类型</td>
		<td align="center" class="outset-table" width="35%">特别推荐</td>
		<td align="center" class="outset-table" width=10%">编辑</td>
		<td align="center" class="outset-table" width="10%">排序</td>
		<td align="center" class="outset-table" width="10%">删除</td>
	</tr>
	<%
	Vector vPage = dImpl.splitPage(sqlStr,request,20);
	if (vPage!=null)
	{
		for (int i=0;i<vPage.size();i++)
		{
			Hashtable content = (Hashtable)vPage.get(i);
			qu_id = content.get("qu_id").toString();
			question = content.get("qu_title").toString();
			sequence = content.get("qu_sequence").toString();
			userKind = content.get("uk_name").toString();
			%>
	<tr <%if(i%2==0) out.print("class='line-even'"); else out.print("class='line-odd'");%> width="100%">
		<td align="center"><%=qu_id%></td>
		<td align="center"><%=userKind%></td>
		<td align="left"><%=question%></td>
		<td align="center"><a href="QuestionInfo.jsp?qu_id=<%=qu_id%>"><img src="/system/images/modi.gif" border="0"></a></td>
		<td align="center"><input class="text-line" size="4" name="module<%=sequence%>" value="<%=sequence%>"></td>
		<td align="center"><input type="checkbox" name="del" class="checkbox1" value="<%=qu_id%>"></td>
	</tr>
			<%
		}
	}
	else
	{
		out.print("<tr class='line-even'><td  colspan=9>没有记录</td></tr>");
	}
	%>
	<input type="hidden" name="qu_id" value="">
	</form>
	<tr>
		<td colspan="9" align="right">
			<%=dImpl.getTail(request)%>
		</td>
	</tr>
</table>
<%
dImpl.closeStmt();
dCn.closeCn();

} catch (Exception ex) {
	System.out.println(new java.util.Date() + "--"
			+ request.getServletPath() + " : " + ex.getMessage());
} finally {
	if(dImpl != null)
	dImpl.closeStmt();
	if(dCn != null)
	dCn.closeCn();
}
%>
<%@include file="/system/app/skin/bottom.jsp"%>