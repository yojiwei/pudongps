...
try{
	dCn = new CDataCn();
  Note_Title=CTools.dealString(request.getParameter("title"));
  //Note_Title=yy.ex_chinese(Note_Title);
  Note_Content=CTools.dealString(request.getParameter("content"));
 //Note_Content=yy.ex_chinese(Note_Content);
  ...
  String guestName = CTools.dealString(request.getParameter("Guest_Sign"));
...
//out.close();
  ...
  }else
  {
   ...
   sql=sql+"values(SEQ_FORUM_POST.nextval,"+Board_Id+","+Return_Id+",'"+Note_Title+"','"+Note_Content+"',to_date('"+datestr+"','YYYY-MM-DD HH24:MI:SS'),'"+userName+"',"+Note_Content.length()+","+Note_Singid+",'"+Note_Icon+"','"+request.getRemoteHost()+"','"+userName+"',to_date('"+datestr+"','YYYY-MM-DD HH24:MI:SS'),'"+guestName+"',0,0)";
...
stmt.executeUpdate(sql);
  ...
  // out.close();
   ...
   if (Return_Id=="0")
   {
   	response.sendRedirect("board_smzx.jsp?fid="+Board_Id);
%>
<html>

<body>
<p>&nbsp;</p>
<meta http-equiv='refresh' content='2;url=board_smzx.jsp?fid=<%=Board_Id%>'>
...
{
	response.sendRedirect("shownote_smzx.jsp?fid="+Board_Id+"&noteid="+Return_Id);
%>
<html>
<meta http-equiv='refresh' content='2;url=shownote_smzx.jsp?fid=<%=Board_Id%>&noteid=<%=Return_Id%>'>
<body>
<p>&nbsp;</p>
<table width="75%" border="1" align="center" cellpadding="5" cellspacing="0">
  <tr>
