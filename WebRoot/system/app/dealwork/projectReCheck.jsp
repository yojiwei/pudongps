...
try{
	...
	if(owList != null){
		for(int countLsh = 0 ; countLsh < owList.size(); countLsh++){
			...
			//------------生成一个新的文件名---------------------start---------------
			String fileNameNew = ow_contentOld.substring(0,ow_contentOld.indexOf(".xml")) + "123.xml";
			if(!CFile.Exist(attPath + "\\\\" + fileNameNew)){
				CFile.write(attPath + "\\\\" + fileNameNew,ow_contentNew);
			}else{
				fileNameNew = ow_contentOld.substring(0,ow_contentOld.indexOf(".xml")) + "456.xml";
				CFile.write(attPath + "\\\\" + fileNameNew,ow_contentNew);
			}
			//------------生成一个新的文件名---------------------end-----------------
			//------------将附件 存到另一个服务器
			CWsbXml wsbXml = new CWsbXml();
			wsbXml.saveOneFile(attPath + "\\\\" + fileNameNew);
			...
			dImplWSB.setValue("ow_content",fileNameNew,CDataImpl.SLONG);
			dImplWSB.update();
			dCnWSB.commitTrans();
