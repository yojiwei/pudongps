...
String nowpage = CTools.dealNumber(("".equals(request.getParameter("strPage"))||request.getParameter("strPage")==null)?"1":request.getParameter("strPage"));
...
sqlStr = "select a.wo_status,a.wo_id,a.wo_sparehour,a.pr_id,a.wo_isovertime,a.wo_applypeople,a.wo_projectname,to_char(a.wo_applytime,'yyyy-mm-dd hh24:mi:ss') wo_applytime,b.pr_timelimit ";
sqlStr += "from tb_work a,tb_proceeding b,tb_deptinfo c ";
sqlStr += "where a.wo_status in(" +status+ ")  and a.pr_id=b.pr_id and b.dt_id=c.dt_id and c.dt_id="+dtId ;
sqlStr += " and a.wo_projectname is not null and a.wo_applypeople is not null";
if(!"".equals(pr_name)){
	sqlStr += " and a.wo_projectname like '%"+pr_name+"%'";
}
if(!"".equals(wo_applyPeople)){
	sqlStr += " and a.wo_applypeople like '%"+wo_applyPeople+"%'";
}
if (!beginTime.equals(""))
{
	...
	for (int i=0;i<vPage.size();i++)
	{
		...
		%>
				<tr width="100%" <%if (i%2==0) out.print("class='line-even'");else out.print("class='line-odd'");%>>
					...
					{
					%>
						<span style="cursor:hand" onclick="javascript:window.location='AppWorkDetail.jsp?wo_id=<%=wo_id%>&nowpage=<%=nowpage%>'"><%=pr_task%></span>
					<%
					if("34".equals(dtId)){
					%>
					...
					{
					%>
						<a href="../cooperate/CorrForm.jsp?OPType=Corragain&wo_id=<%=wo_id%>&pr_id=<%=content.get("pr_id")%>">
