...
sqlDept =" select x.dt_id,y.dt_name,x.pc_timelimit from tb_proceedingcorr x,tb_deptinfo y where x.dt_id=y.dt_id and x.pr_id='"+pr_id+"'";
  ...
  <td width="100%">
       <table width="100%" class="content-table" height="1">
	     <tr>
			...
			{
				out.println("<tr><td><table width='100%'><tr><td width='40%' align='left' valign='center'>协办部门:");
...
function checkCorr(choice)
{
	...
	formData.action="/system/app/new_cooperate/CorrResult.jsp?choice="+choice;
	formData.submit();
}
</script>
<script>
function checkRecorr(choice)
{
	...
	{
		formData.action="/system/app/new_cooperate/ReCorrResult.jsp?choice="+choice;
		formData.submit();
	}
	...
	if(choice==2)
	{
		var w=500;
		var h=300;
		var url="/system/app/new_cooperate/Frozen.jsp?OPType=Frozen&co_id=<%=co_id%>&de_id=<%=de_id%>&de_senddeptid=<%=de_senddeptid%>";
