...
{
	var id = obj.value;
	var status = document.getElementById("cw_status").value;
    location.reload("AppealList.jsp?cp_id="+id+"&cw_status="+status+"");
...
String cp_id =  request.getParameter("cp_id")==null?"o140":request.getParameter("cp_id").toString();
...
if(intStatus==8)
 {
  str_sql="select c.cw_id,c.cp_id,c.cw_applyingname,c.cw_applyingdept,c.cw_subject,c.cw_isovertime,c.cw_isovertimem,to_char(c.cw_applytime,'yyyy-mm-dd hh24:mi:ss') cw_applytime,to_char(c.cw_finishtime,'yyyy-mm-dd hh24:mi:ss') cw_finishtime,p.cp_timelimit,p.cp_timelimiting,q.dt_id,q.pc_timelimit from tb_connwork c,tb_deptinfo d,tb_connproc p,tb_connproccorr q where p.cp_id='"+cp_id+"'  and c.cp_id=p.cp_id and p.cp_id=q.cp_id and c.cw_status=" + cw_status;
 }
else
 str_sql="select c.cw_id,c.cp_id,c.cw_applyingname,c.cw_applyingdept,c.cw_subject,c.cw_isovertime,c.cw_isovertimem,to_char(c.cw_applytime,'yyyy-mm-dd hh24:mi:ss') cw_applytime,to_char(c.cw_finishtime,'yyyy-mm-dd hh24:mi:ss') cw_finishtime,p.cp_timelimit,p.cp_timelimiting from tb_connwork c,tb_deptinfo d,tb_connproc p where p.cp_id='"+cp_id+"'  and c.cp_id=p.cp_id and c.cw_status=" + cw_status ;
...
<table class="FindAreaTable1" cellspacing="1" rules="cols" border="0" id="ctl00_DataGrid_DataGrid1" style="border-width:0px;">
 <input type="hidden" value="<%=intStatus%>" id="cw_status"/>
				<tr>
						<td colspan="2" align="left" >信件类型:
						<select name="cp_id" onchange="selectType(this)">
						<option value="o140" <%="o140".equals(cp_id)?"selected":""%>>党务咨询</option>
						<option value="o141" <%="o141".equals(cp_id)?"selected":""%>>投诉监督</option>
						</select>
						</td>
						<td  align="left" >&nbsp;</td>
						<td  align="left" >&nbsp;</td>
