...
<input type="hidden" name="vde_type" value="33">
...
if(OType.equals("Edit"))
{
  ...
  vde_type=CTools.dealNull(content_editid.get("vde_type"),"0");
	vde_sort=CTools.dealNull(content_editid.get("vt_sort"));
...
if(upperid.equals("0"))//根节点
{
	...
	<tr class="line-odd">
	  ...
	  <tr class="line-odd">
         <td align='right' width='16%'>部门类别</td><td align='left'>
         <select name="vde_sort" class=text-line >
         <%
         	sqlStr = "select dd_id,dd_name,dd_code from tb_datatdictionary where dd_parentid = (select dd_id from tb_datatdictionary where dd_code = 'zhcp')";
					Vector vt = dImpl.splitPage(sqlStr,request,100);
					if (vt!= null) {
						for (int i = 0;i < vt.size();i++) {
							Hashtable hash = (Hashtable)vt.get(i);
				 %>
         <option value="<%=hash.get("dd_code").toString()%>" <%if(vde_sort.equals(hash.get("dd_code").toString())) out.print("selected");%>><%=hash.get("dd_name").toString()%></option>
         <%		
				}
			}
         %>
...
%>
<tr class="outset-table" width="100%" id="btnObj" style="display:">
...
if(OType.equals("Edit"))
{
	...
	out.print("<input type=button name=btnType value=发布 onclick=checkFtype(0) class=bttn>");
	}
	if(vde_status.equals("1")){
	out.print("<input type=button name=btnType value=暂存 onclick=checkFtype(1) class=bttn>");
	}
	}
	%>
<input type='hidden' name='vde_status'value='<%=vde_status%>'>
<input type='hidden' name='type' value='<%=vt_type%>'>
<input type='hidden' name='a' value='0'>
<%
	if(!Typepp.equals("2")){%>
<input type="button" name="btnDel"  value="删除" class="bttn" onclick="deleteThis('')">&nbsp;&nbsp;&nbsp;
<%
}}
%>
...
function deleteThis(val1)
{
	  if(confirm("确实要删除吗？"))
	  {
			formData.action = "state.jsp?del=1";
			formData.submit();
	  }
}
