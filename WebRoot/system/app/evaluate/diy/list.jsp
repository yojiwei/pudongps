...
if(Typepp.equals("1")&&!upperid.equals("0")){//edit by arin
sqlStr = "select * from tb_votediy where vt_upperid = "+upperid+" order by vt_sequence,vt_id desc";
}else{
sqlStr = "SELECT * FROM tb_votediy a,tb_votediyext b WHERE a.vt_id=b.vt_id AND b.vde_status="+vde_status+" AND vt_upperid = "+upperid+" order by vt_sequence,a.vt_id desc";
}
...
if(vPage != null)
{
	for(int i=0;i<vPage.size();i++)
	{
		...
		if(upperid.equals("0"))
			...
			out.print("<td><a href='vote.jsp?id="+content.get("vt_id").toString()+"' target='_black'>投票页面</a></td>");
		if(upperid.equals("0")){
			if(content.get("vde_type").toString().equals("0")){
			out.print("<td><a href='votestat.jsp?id="+content.get("vt_id").toString()+"'>统计页面</a></td>");
			}
			else if(content.get("vde_type").toString().equals("1")){
			out.print("<td><a href='votestit.jsp?id="+content.get("vt_id").toString()+"'>统计页面</a></td>");
			}
			else {//add by arin 2006-12-30
			 out.print("<td></td>");
			}
...
if(!tempid_showvote.equals(""))
{
	...
	String strSQLs="select * from tb_votediyext where vt_id="+tempid_showvote+"";
