...
String vde_img = request.getParameter("vde_img");
String DL_file_name = "";
String vde_pp="";
vde_pp=request.getParameter("vde_pp");
long CT_file_size = 0;
if (vde_img == null)
vde_img = "";
if (!vde_img.equals(""))
...
java.io.File oDir=new java.io.File(images_save_path+vde_img);
if(oDir.exists())
{
	int n;
	String HttpPath = images_http_path+vde_img;
...
-->				
		   ...
		   <%if(vde_pp.equals("a")){
				...
				out.print("图片名称：<input type='file' name='fj11 size=30 class='text-line' id=fj11>");
				}
			}else{
				if(vde_pp.equals("")){
					out.print("图片名称：<input type='file' name='fj11 size=30 class='text-line' id=fj11>");
	...
	<input type="hidden" name="vde_pp" value="e">
	<tr>
		<td>
...
<!--
window.opener.formData.vde_img.value = document.formData.vde_img.value;

function fnCheck()
{
    formData.action="ExplainInfoImgResult.jsp?vde_img=<%=vde_img%>&vde_pp=e";
    return true;
...
function deleteFile(filename)
{
  ...
  if (d)
  {
  window.location="ExplainInfoImgDel.jsp?vde_img=<%=vde_img%>&Filename="+filename+"&vde_pp=e";
  }
}
//-->
</script>
