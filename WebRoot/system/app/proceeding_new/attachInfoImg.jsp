...
String images_save_path = ""; //获取
String images_http_path = ""; //获取

try{
	dCn = new CDataCn(); //新建数据库连接对象
	dImpl = new CDataImpl(dCn); //新建数据接口对象
	images_save_path = dImpl.getInitParameter("images_save_path"); //获取
	images_http_path = dImpl.getInitParameter("images_http_path"); //获取
} catch (Exception ex) {
...
String pr_imgpath = request.getParameter("pr_imgpath");
String DL_file_name = "";
long CT_file_size = 0;
if (pr_imgpath == null)
	pr_imgpath = "";
if (!pr_imgpath.equals(""))
{
%>
...
java.io.File oDir=new java.io.File(images_save_path+pr_imgpath);
if(oDir.exists())
{
	int n;
	String HttpPath = images_http_path+pr_imgpath;
...
%>
<form name="formData" action="attachInfoImgResult.jsp" method="post" enctype="multipart/form-data" onsubmit="return fnCheck()">
<table>			 
	<tr class="line-even">
	...
	<input type="hidden" name="pr_imgpath"  value="<%=pr_imgpath%>">
	<tr>
		<td>
...
<!--
window.opener.formData.pr_imgpath.value = document.formData.pr_imgpath.value;

function fnCheck()
{
    formData.action="attachInfoImgResult.jsp?pr_imgpath=<%=pr_imgpath%>";
    return true;
...
function deleteFile(filename)
{
  ...
  if (d)
  {
  window.location="attachImgDel.jsp?pr_imgpath=<%=pr_imgpath%>&Filename="+filename;
  }
}
//-->
</script>
