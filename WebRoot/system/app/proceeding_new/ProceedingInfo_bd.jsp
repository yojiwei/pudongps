...
session.setAttribute("mySelf",mySelflogin);
//update by yo--------over
*/
...
if ((OType.equals("Edit")||mySelf.getMyUid().equals("admin"))||!projectId.equals(""))
{
	...
	if (content!=null){
		...
		//update by shiweijun 20091221
		pr_gisaddress = CTools.dealNull(content.get("pr_gisaddress"));
...
<script language="javascript" src="/system/include/change_bd.js"></script>
...
function checkForm()
{
	...
	getCWInfo();
	formData.action = "ProceedingInfoResult_bd.jsp";
			...
			<table width="100%" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td valign="bottom" width="90">
						<table cellspacing="0" cellpadding="0">
							<tr>
								<td id="bdform_bt" class="title_down" align="center" onClick="javascript:ChangeC('bdform');">绑定表单</td>
								</tr>
						</table>
					</td>
					<!--在线表单绑定开始-->
...
<script language="javascript">isAccept();</script>
	...
	<table width="100%" class="table_main"><tr><td><table cellpadding="3" width="100%" align="center">
	<tr>
       <td class="row" id="TdInfo3" colspan="2" align="left">请选择需要绑定的表单</td>
    </tr>
	<%
	String formSql = "";
	String centerSql = "";
	String pc_id = "";
	Vector vform = null;
	Hashtable formContent = null;
	Hashtable centerContent = null;
	formSql = "select pc_id,pc_formname,pc_formguid,pc_formgroup,pc_formgroupcode from tb_proceeding_form ";
	vform = dImpl.splitPage(formSql,200,1);
	if(vform!=null){
		for(int v=0;v<vform.size();v++){
			formContent = (Hashtable)vform.get(v);
			pc_id = CTools.dealNull(formContent.get("pc_id"));
			String ischeck = "";
			centerSql = "select pi_id from tb_proceeding_center where pr_id = '"+projectId+"' and pc_id="+pc_id;
			centerContent = dImpl.getDataInfo(centerSql);
	...
	%>
	<tr>
	   <td width="5%" class="row"><input type="checkbox" name="vf_id" <%=ischeck%> value="<%=pc_id%>"/>&nbsp;</td>
	   <td width="95%" align="left" class="row"><%=CTools.dealNull(formContent.get("pc_formname"))%>&nbsp;</td>
   </tr>
    <%
		}
	}
