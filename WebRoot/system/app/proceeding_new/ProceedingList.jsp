...
String sqlStr = "select p.pr_id,p.pr_name,p.pr_code,p.dt_id,p.pr_isdel,pr_sequence,to_char(pr_edittime,'yyyy-mm-dd') pr_edittime from tb_proceeding_new p";
sqlStr+=" where 1=1 "; 
...
if(!(departId.equals("")||curUid.equals("")))
{
	...
	{   
		if(curUid.equals("czj"))
		{
			sqlStr +="and pr_id in (select pr_id from tb_worklinksort_new where gw_id in (select gw_id from tb_gasortwork where dt_id = "+departId+"))";	
		}else{
			if(!departId.equals("57")){ 
				sqlStr += " and (dt_id="+departId+" or pr_sourcedtid="+departId+")";
				}
			else {
				sqlStr += " and dt_id=57";
				}
...
sqlStr += " and pr_isdel is null and pr_id_old is null order by pr_sequence,pr_edittime desc,pr_id desc";
	...
	for (int i=0;i<vPage.size();i++)
	{
		...
		<tr width="100%" <%if (i%2==0) out.print("class='line-odd'");else out.print("class='line-even'");%>>
		   ...
		   <span style="cursor:hand" onclick="javascript:window.location='../corrdeptmanage/CorrDeptList.jsp?pr_id=<%=content.get("pr_id").toString()%>'"><img src="/system/images/clip.gif" title="协办部门维护" width="15" height="15" border="0"></span>			</td>
			...
			<td align="center">
				<a href="ProceedingInfo.jsp?OType=Edit&curpage=<%=CTools.dealNumber(request.getParameter("strPage"))%>&projectId=<%=projectId%>"><img class="hand" border="0" src="../../images/modi.gif" title="项目编辑" WIDTH="16" HEIGHT="16"></a>
			</td>
