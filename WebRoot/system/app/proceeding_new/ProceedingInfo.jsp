...
if ((OType.equals("Edit")||mySelf.getMyUid().equals("admin"))||!projectId.equals(""))
{
	...
	sqlStr += "pr_isaccept,pr_sequence,to_char(pr_edittime,'yyyy-mm-dd') pr_edittime,pr_isdel,uk_id,pr_gut,pr_cc,pr_root,pr_table,pr_by,pr_area,pr_stuff,pr_blcx,pr_money,pr_telephone,pr_tstype,pr_sxtype,pr_address,pr_tstel,pr_tsemail,pr_qt,pr_bc,pr_gisaddress,pr_outurl from tb_proceeding_new where pr_id = '" + projectId + "'  ";
	strTitle     = "网上办事>>管理";
	...
	if (content!=null){
		...
		pr_outurl = CTools.dealNull(content.get("pr_outurl"));
...
{
	//得到单选按钮是否有链接的值
	var isoutUrl = formData.isoutUrl;
	var theRadioLen = isoutUrl.length;
    var theRadioValue = false;
    if (theRadioLen == undefined){
        if (isoutUrl.checked){
            theRadioValue = isoutUrl.value;
        }
    }else{
        for (theRadioI=0;theRadioI<theRadioLen;theRadioI++){
            if (isoutUrl[theRadioI].checked){
                theRadioValue = isoutUrl[theRadioI].value;
                break;
            }
        }
    }
	...
	formData.pr_blcx.value=eWebEditor4.getHTML();
		...
		if (formData.pr_outurl.value=="")
		{
			alert("请填写外网链接地址！");
			formData.pr_outurl.focus();
			return false;
		}
	}
	getCWInfo();//用户类型选择
...
function isoutwork(isoutwork){
	if(isoutwork == 1){
		document.getElementById("isout").style.display = "none"; 
		document.getElementById("outurl").style.display = "";
	}else{
		document.getElementById("isout").style.display = "";
				...
				<tr>
					<td>
						...
						<tr class="line-even" width="100%">
						  ...
						  <td align="left"><input type="radio" name="isoutUrl" value="1" onclick="isoutwork(1)" <%=!"".equals(pr_outurl)?"checked":""%>/>&nbsp;是<input type="radio" name="isoutUrl" value="0" onclick="isoutwork(0)" <%="".equals(pr_outurl)?"checked":""%>/>&nbsp;否&nbsp;<br />
						     &nbsp;<FONT COLOR="#FF0000">*</FONT>&nbsp;<span id="checkTip" style="COLOR:#FF0000">（注：如链接外部网站办事地址，选择后只需填写以下显示内容，办事指南无需填写上传）</span></td>
						</tr>
						<!--办事事项是否有外部链接1是0否-->
						<tr class="line-even" width="100%">
...
<!----------------------没有外网办事链接-------------------->
<table width="100%" cellpadding="3" cellspacing="2" id="isout" style="display:">
<tr class="line-even" width="100%">
	<td width="100" align="right">办事依据：</td>
	<td align="left"><textarea <%=isReadonly%> name="pr_by" value="1" <%=isCheck%> style="display:none;WIDTH: 100%; HEIGHT: 200px"><%=pr_by%></textarea>	
<IFRAME ID="eWebEditor1" src="/system/common/edit/eWebEditor.jsp?id=pr_by&style=standard" frameborder="0" scrolling="no" width="100%" height="200"></IFRAME>
</tr>
<tr class="line-odd" width="100%">
	<td width="100" align="right">申请范围：</td>
	<td align="left"><textarea <%=isReadonly%> name="pr_area" value="1" <%=isCheck%> style="display:none;WIDTH: 100%; HEIGHT: 200px"><%=pr_area%></textarea>	
<IFRAME ID="eWebEditor2" src="/system/common/edit/eWebEditor.jsp?id=pr_area&style=standard" frameborder="0" scrolling="no" width="100%" height="200"></IFRAME>	
</tr>
...
<!----------------------没有外网办事链接-------------------->
<!----------------------有外网办事链接---------------------->
<table width="100%" id="outurl" style="display:none" cellpadding="0" cellspacing="0">
<tr class="line-even" width="100%">
	<td width="100" align="right">链接：</td>
	<td align="left">
		<input <%=isReadonly%> size="50" class="text-line" name="pr_outurl" value="<%=pr_outurl%>" maxlength="200">
		&nbsp;<FONT COLOR="#FF0000">*（注：请输入事项外网链接地址如：http://www.pudong.gov.cn）</FONT></td>
</tr>
</table>
<!----------------------有外网办事链接---------------------->
	<tr class="line-even" width="100%" style="display:none">
	...
	<tr class="outset-table" width="100%">
		<td colspan="5" align="center">
			<input <%=isDisabled%> type="button" name="btnSubmit" id="tt" value="确定" onClick="checkForm()" class="bttn">&nbsp;
	...
	<input type="hidden" name="pr_address_old" value="<%=pr_address%>">
	<!---update by dongliang 20090106-->
...
<script type="text/javascript" for=window event=onload>
	...
	initCommonWork();
	ChangeC("baseinfo");
	isoutwork(<%=!"".equals(pr_outurl)?"1":"0"%>);
</script>
<!--    表单提交结束    -->
