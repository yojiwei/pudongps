...
CDataCn cdCn = null; //原来数据库链接用户权限等
CDataImpl cdImpl = null;//原来数据库链接用户权限等
try{
dCn = new CDataCn("pddjw");
dImpl = new CDataImpl(dCn);

cdCn = new CDataCn();//原来数据库链接用户权限等
cdImpl = new CDataImpl(cdCn);//原来数据库链接用户权限等
if (mySelf!=null)
{
...
if (!us_id.equals(""))
{
	...
	Hashtable ccontent = cdImpl.getDataInfo(ssqlStr);
	if (ccontent!=null)
	{
...
<!--    功能按钮开始    -->
<img  src="/system/images/new.gif" border="0" onClick="window.location='sortEdit.jsp?OPType=add'" <%=us_uid.equals("djwbbs_01") ? "" : "style='display:none'"%>  title="新增栏目" style="cursor:hand" >
新增栏目
<img src="/system/images/new.gif" border="0" style="cursor:hand;" onClick="submitAdd()" alt="新建主题"  <%=us_uid.equals("djwbbs_01") ? "" : "style='display:none'"%>>
新建主题
...
<table class="FindAreaTable1" cellspacing="1" rules="cols" border="0" id="ctl00_DataGrid_DataGrid1" style="border-width:0px;">
	...
	<!-- 加入新建主题功能 postEdit.jsp为新建主题页面 -->
				<form action="/system/app/pddjwbbs/postEdit.jsp?type_post=Add&us_id=<%=us_id%>" method="post" name="formrepAdd">
...
<table width="100%">
		...
		<td>
			<table width="100%">
				<tr class="bttn">
					...
					<td width="10%" <%=us_uid.equals("djwbbs_01") ? "" : "style='display:none'"%> nowrap class="outset-table" ><div align="center">修改</div></td>
				</tr>
			</table>
		</td>
	<tr>
	...
	%>
	<tr class="line-odd">
		<td align="center" ><b><%if("djwbbs_01".equals(us_uid)){%><a href ="sortEdit.jsp?OPType=edit&sort_id=<%=sort_id%>"><%=sort_name%></a><%}else{%><%=sort_name%><%}%></b></td>
		<td>
			<table width="100%">
				<tr>
				<%
				if("djwbbs_01".equals(us_uid)){
				...
				%>
					...
					<td width="10%" align=center <%=us_uid.equals("djwbbs_01") ? "" : "style='display:none'"%>><div align="center"><img style="cursor:hand;" src='images/modi.gif' onclick="window.location='rework.jsp?sort_id=<%=sort_id%>&board_id=<%=board_id%>&us_uid=<%=us_uid%>&board_master_id=<%=board_master_id%>'"  width="16" height="16" border='0' title='修改' <%=us_uid.equals("djwbbs_01") ? "" : "disabled"%>></div></td>
				<%
					}
				}else{
