...
//更新forum_post_pd表中的话题的关注数
dImpl.edit("forum_post_pd","post_id",post_id);
dImpl.setValue("post_click_count",post_click_count,CDataImpl.STRING);//更新forum_post_pd表中的话题的关注数
dImpl.update();
...
<table width="100%" height="108" border="0"   cellpadding="6" cellspacing="1">
  ...
  <tr>
    <td rowspan="2" valign="top" bgcolor="#F7FBFF"><div align="center"><%=revert_author%></div><p align="center">
	IP:<%=revert_ip%></td>
	...
	<a href='delRevert.jsp?strPage=<%=newdImpl.getPageNo()%>&sort_id=<%=sort_id%>&board_id=<%=board_id%>&post_id=<%=post_id%>&revert_id=<%=revert_id%>'><img src='images/DELETE.GIF' width="16" height="16" border='0' title='删除' onclick="return window.confirm('确认要删除该记录么?');"/></a></div></td></form>
  </tr>
  <tr>
