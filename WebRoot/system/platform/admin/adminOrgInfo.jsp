...
<%
  ...
  try{
  	...
  	tree = new CDeptXML(dCn);

  id = java.lang.Long.parseLong(AT_id); //角色的ID
  //tree
  moduleIds = jdo.getAccess(id,jdo.ORGANIZATION_ACCESS); //获取模块的ID列表

  String title="操作：设置机构集";
%>
  <xml id=xmlDoc>
   <%
  ...
  <legend>&nbsp;
          [<%=CTools.dealNull(session.getAttribute("_platform_AT_realname"))%>]的机构集&nbsp;
  </legend>

<br><span id=TreeRoot></span><br><br>
<form name="formData" method="post" action="adminOrgInfoResult.jsp">
